...
export function computeLeachingFractionRunoffEc({ volume_applied = 0, volume_drained = 0, feed_ec = 0, measured_leachate_ec = 0, alternative_leaching_fraction = 0, target_leachate_ec = 0 } = {}) {
  ...
  return {
    ...
    note: "The bound is NOT a prediction -- it is what the leachate would reach if the crop absorbed no salt at all, and nothing grows there, which is the clue that it is far from the truth. The gap between the bound and a measurement IS the crop's uptake, which is why this calculation's error term is its product. Raising the leaching fraction moves the bound at the cost of nearly three times the water and fertiliser down the drain (drainage goes as LF / (1 - LF): 0.25 to 0.67 of crop use from LF 0.2 to 0.4). And the bound must never be used as a target: holding a leachate reading on the bound alone would take a leaching fraction no one irrigates at and no one needs, because the crop is removing most of the salt. MANAGE TO THE MEASUREMENT, NOT TO THE BOUND. A pour-through or saturated-media procedure done consistently, and the crop's own published root-zone range, govern.",
  };
}
