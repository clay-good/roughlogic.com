...
// dims: in { clearing_time_s: T, surface_resistivity: dimensionless, native_resistivity: dimensionless, layer_thickness_m: L, body_weight: dimensionless } out: { cs: dimensionless, e_step_v: M L^2 T^-3 I^-1, e_touch_v: M L^2 T^-3 I^-1 }
...
// dims: in { grid_current_a: I, grid_resistance_ohm: M L^2 T^-3 I^-2, tolerable_touch_v: M L^2 T^-3 I^-1 } out: { gpr_v: M L^2 T^-3 I^-1, safe_by_gpr: dimensionless, margin_v: M L^2 T^-3 I^-1 }
...
// dims: in { tolerable_touch_v: M L^2 T^-3 I^-1, grid_current_a: I } out: { max_grid_resistance_ohm: M L^2 T^-3 I^-2 }
...
export function computeRollingSphereProtection({ mast_height_ft = 0, sphere_radius_ft = 150 } = {}) {
  ...
  return {
    ...
    note: "NFPA 780 rolling-sphere method: imagine a sphere of radius R (150 ft for the standard protection level; smaller radii apply for higher protection -- 100 ft for explosives under NFPA 780, down to 20 m for IEC 62305 LPL I) rolled over the structure -- lightning strikes wherever the sphere touches, and a mast or air terminal shields the ground circle the sphere cannot reach. For a mast of height h no taller than R, the protected ground radius is d = sqrt(2 R h - h^2); this is a single-mast estimate, and two or more masts protect the overlapping zone between them, which reaches higher than either alone. "
