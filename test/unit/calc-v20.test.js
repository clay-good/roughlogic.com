...
// ---------------------------------------------------------------------------
import {
  computeFederalPostJudgmentInterest, federalPostJudgmentInterestExample,
  computeLeaseRentProration, leaseRentProrationExample,
} from "../../calc-legal.js";

test("federal-post-judgment-interest: $100k 5% 2yr -> $10,250", () => {
  const r = computeFederalPostJudgmentInterest({ principal: 100000, rate_pct: 5.0, days_elapsed: 730 });
  assert.ok(Math.abs(r.accrued_interest - 10250) < 1);
  assert.ok(Math.abs(r.total_owed - 110250) < 1);
});
test("federal-post-judgment-interest: example", () => { assert.ok(computeFederalPostJudgmentInterest(federalPostJudgmentInterestExample.inputs).accrued_interest > 0); });
test("federal-post-judgment-interest: compounds annually", () => {
  const oneYr = computeFederalPostJudgmentInterest({ principal: 100000, rate_pct: 5.0, days_elapsed: 365 });
  assert.ok(Math.abs(oneYr.accrued_interest - 5000) < 1);
});
test("federal-post-judgment-interest: partial year accrues", () => {
  const r = computeFederalPostJudgmentInterest({ principal: 100000, rate_pct: 5.0, days_elapsed: 547 });
  assert.ok(r.accrued_interest > 5000 && r.accrued_interest < 10250);
});
test("federal-post-judgment-interest: zero days -> zero", () => {
  const r = computeFederalPostJudgmentInterest({ principal: 100000, rate_pct: 5.0, days_elapsed: 0 });
  assert.strictEqual(r.accrued_interest, 0);
});
test("federal-post-judgment-interest: negative principal rejected", () => {
  assert.ok("error" in computeFederalPostJudgmentInterest({ principal: 0, rate_pct: 5, days_elapsed: 365 }));
});

test("lease-rent-proration: $1500 March 17 days actual -> $822.58", () => {
  const r = computeLeaseRentProration({ monthly_rent: 1500, method: "actual", days_in_month: 31, occupied_days: 17 });
  assert.ok(Math.abs(r.prorated_amount - 822.58) < 0.05);
  assert.ok(Math.abs(r.daily_rate - 1500 / 31) < 1e-6);
});
test("lease-rent-proration: example", () => { assert.ok(computeLeaseRentProration(leaseRentProrationExample.inputs).prorated_amount > 0); });
test("lease-rent-proration: 30-day method", () => {
  const r = computeLeaseRentProration({ monthly_rent: 1500, method: "thirty", occupied_days: 17 });
  assert.ok(Math.abs(r.daily_rate - 50) < 1e-6);
  assert.ok(Math.abs(r.prorated_amount - 850) < 1e-6);
});
test("lease-rent-proration: 365-day method", () => {
  const r = computeLeaseRentProration({ monthly_rent: 1500, method: "threesixtyfive", occupied_days: 17 });
  assert.ok(Math.abs(r.daily_rate - 1500 * 12 / 365) < 1e-6);
});
test("lease-rent-proration: methods differ", () => {
  const a = computeLeaseRentProration({ monthly_rent: 1500, method: "actual", days_in_month: 31, occupied_days: 17 });
  const b = computeLeaseRentProration({ monthly_rent: 1500, method: "thirty", occupied_days: 17 });
  assert.ok(a.prorated_amount !== b.prorated_amount);
});
test("lease-rent-proration: zero rent rejected", () => {
  assert.ok("error" in computeLeaseRentProration({ monthly_rent: 0, method: "actual", days_in_month: 31, occupied_days: 17 }));
});
...
// ---------------------------------------------------------------------------
import {
  computeVetBodySurfaceArea, vetBodySurfaceAreaExample,
  computeVetCorrectedReticulocyte, vetCorrectedReticulocyteExample,
  computeVetFluidDeficit, vetFluidDeficitExample,
  computeVetAnionGap, vetAnionGapExample,
} from "../../calc-vet.js";

test("vet-body-surface-area: dog 20 kg -> 0.744 m2", () => {
  const r = computeVetBodySurfaceArea({ species: "dog", weight_kg: 20 });
  assert.ok(Math.abs(r.bsa_m2 - 0.744) < 0.005);
});
test("vet-body-surface-area: example + dose", () => {
  const r = computeVetBodySurfaceArea({ species: "dog", weight_kg: 20, dose_mg_m2: 100 });
  assert.ok(Math.abs(r.total_dose_mg - r.bsa_m2 * 100) < 1e-6);
});
test("vet-body-surface-area: cat uses K=10.0", () => {
  const dog = computeVetBodySurfaceArea({ species: "dog", weight_kg: 5 });
  const cat = computeVetBodySurfaceArea({ species: "cat", weight_kg: 5 });
  assert.ok(cat.bsa_m2 < dog.bsa_m2);
});
test("vet-body-surface-area: small dog flagged", () => {
  assert.strictEqual(computeVetBodySurfaceArea({ species: "dog", weight_kg: 5 }).small_dog_flag, true);
});
test("vet-body-surface-area: scales with weight^2/3", () => {
  const a = computeVetBodySurfaceArea({ species: "dog", weight_kg: 10 });
  const b = computeVetBodySurfaceArea({ species: "dog", weight_kg: 80 });
  assert.ok(Math.abs(b.bsa_m2 / a.bsa_m2 - Math.pow(8, 2 / 3)) < 0.01);
});
test("vet-body-surface-area: zero weight rejected", () => {
  assert.ok("error" in computeVetBodySurfaceArea({ species: "dog", weight_kg: 0 }));
});

test("vet-corrected-reticulocyte: dog 5% PCV20 normal45 -> 2.22%, 150k", () => {
  const r = computeVetCorrectedReticulocyte({ retic_pct: 5, patient_pcv: 20, normal_pcv: 45, rbc: 3000000 });
  assert.ok(Math.abs(r.corrected_pct - 2.222) < 0.01);
  assert.ok(Math.abs(r.absolute_ul - 150000) < 1);
});
test("vet-corrected-reticulocyte: example", () => { assert.ok(computeVetCorrectedReticulocyte(vetCorrectedReticulocyteExample.inputs).corrected_pct > 0); });
test("vet-corrected-reticulocyte: regenerative band", () => {
  const r = computeVetCorrectedReticulocyte({ retic_pct: 5, patient_pcv: 20, normal_pcv: 45, rbc: 3000000 });
  assert.strictEqual(r.regenerative, true);
});
test("vet-corrected-reticulocyte: species default PCV", () => {
  const r = computeVetCorrectedReticulocyte({ retic_pct: 5, patient_pcv: 20, normal_pcv: 0, species: "cat" });
  assert.ok(Math.abs(r.corrected_pct - 5 * 20 / 37) < 1e-6);
});
test("vet-corrected-reticulocyte: no RBC -> band prompt", () => {
  const r = computeVetCorrectedReticulocyte({ retic_pct: 5, patient_pcv: 20, normal_pcv: 45 });
  assert.strictEqual(r.absolute_ul, null);
});
test("vet-corrected-reticulocyte: zero PCV rejected", () => {
  assert.ok("error" in computeVetCorrectedReticulocyte({ retic_pct: 5, patient_pcv: 0, normal_pcv: 45 }));
});

test("vet-fluid-deficit: 10kg 8% 60mL/kg -> 1400 mL/day, 58 mL/hr", () => {
  const r = computeVetFluidDeficit({ weight_kg: 10, dehydration_pct: 8, maintenance_ml_kg_day: 60 });
  assert.ok(Math.abs(r.total_24h_ml - 1400) < 1);
  assert.ok(Math.abs(r.rate_ml_hr - 58.33) < 0.5);
});
test("vet-fluid-deficit: example", () => { assert.ok(computeVetFluidDeficit(vetFluidDeficitExample.inputs).deficit_ml > 0); });
test("vet-fluid-deficit: deficit = %*kg*1000/100", () => {
  const r = computeVetFluidDeficit({ weight_kg: 10, dehydration_pct: 8, maintenance_ml_kg_day: 60 });
  assert.strictEqual(r.deficit_ml, 800);
});
test("vet-fluid-deficit: severe dehydration flagged", () => {
  assert.strictEqual(computeVetFluidDeficit({ weight_kg: 10, dehydration_pct: 14, maintenance_ml_kg_day: 60 }).severe_dehydration, true);
});
test("vet-fluid-deficit: ongoing losses add", () => {
  const a = computeVetFluidDeficit({ weight_kg: 10, dehydration_pct: 8, maintenance_ml_kg_day: 60 });
  const b = computeVetFluidDeficit({ weight_kg: 10, dehydration_pct: 8, maintenance_ml_kg_day: 60, ongoing_ml_day: 200 });
  assert.ok(Math.abs(b.total_24h_ml - a.total_24h_ml - 200) < 1e-6);
});
test("vet-fluid-deficit: zero weight / bad hours rejected", () => {
  assert.ok("error" in computeVetFluidDeficit({ weight_kg: 0, dehydration_pct: 8 }));
  assert.ok("error" in computeVetFluidDeficit({ weight_kg: 10, dehydration_pct: 8, hours: 0 }));
});

test("vet-anion-gap: dog 145/4/110/20 -> AG 19", () => {
  const r = computeVetAnionGap({ na: 145, k: 4.0, cl: 110, hco3: 20 });
  assert.strictEqual(r.anion_gap, 19);
  assert.match(r.band, /within/);
});
test("vet-anion-gap: example", () => { assert.strictEqual(computeVetAnionGap(vetAnionGapExample.inputs).anion_gap, 19); });
test("vet-anion-gap: high gap flagged", () => {
  assert.match(computeVetAnionGap({ na: 150, k: 5, cl: 100, hco3: 12 }).band, /above/);
});
test("vet-anion-gap: cat reference differs", () => {
  const r = computeVetAnionGap({ na: 145, k: 4.0, cl: 110, hco3: 20, species: "cat" });
  assert.strictEqual(r.reference_high, 27);
});
test("vet-anion-gap: includes potassium", () => {
  const withK = computeVetAnionGap({ na: 145, k: 4.0, cl: 110, hco3: 20 });
  assert.strictEqual(withK.anion_gap, (145 + 4) - (110 + 20));
});
test("vet-anion-gap: non-finite rejected", () => {
  assert.ok("error" in computeVetAnionGap({ na: 145, k: 4, cl: 110, hco3: Infinity }));
  assert.ok("error" in computeVetAnionGap({ na: 0, k: 4, cl: 110, hco3: 20 }));
});
...
// ---------------------------------------------------------------------------
import {
  computeCockcroftGaultCrcl, cockcroftGaultCrclExample,
  computeWintersExpectedPco2, wintersExpectedPco2Example,
  computeAaGradient, aaGradientExample,
  computeFena, fenaExample,
} from "../../calc-ems.js";

test("cockcroft-gault-crcl: 70yr 72kg male SCr1.2 -> 58.3", () => {
  const r = computeCockcroftGaultCrcl({ age_yr: 70, weight_kg: 72, sex: "male", scr_mgdl: 1.2 });
  assert.ok(Math.abs(r.crcl_ml_min - 58.33) < 0.1);
});
test("cockcroft-gault-crcl: example", () => { assert.ok(computeCockcroftGaultCrcl(cockcroftGaultCrclExample.inputs).crcl_ml_min > 0); });
test("cockcroft-gault-crcl: female x0.85", () => {
  const m = computeCockcroftGaultCrcl({ age_yr: 70, weight_kg: 72, sex: "male", scr_mgdl: 1.2 });
  const f = computeCockcroftGaultCrcl({ age_yr: 70, weight_kg: 72, sex: "female", scr_mgdl: 1.2 });
  assert.ok(Math.abs(f.crcl_ml_min - m.crcl_ml_min * 0.85) < 1e-6);
});
test("cockcroft-gault-crcl: round SCr toggle", () => {
  const r = computeCockcroftGaultCrcl({ age_yr: 80, weight_kg: 60, sex: "male", scr_mgdl: 0.6, round_scr: true });
  assert.strictEqual(r.scr_rounded, true);
});
test("cockcroft-gault-crcl: lower with higher SCr", () => {
  const a = computeCockcroftGaultCrcl({ age_yr: 70, weight_kg: 72, sex: "male", scr_mgdl: 1.2 });
  const b = computeCockcroftGaultCrcl({ age_yr: 70, weight_kg: 72, sex: "male", scr_mgdl: 2.4 });
  assert.ok(b.crcl_ml_min < a.crcl_ml_min);
});
test("cockcroft-gault-crcl: zero inputs rejected", () => {
  assert.ok("error" in computeCockcroftGaultCrcl({ age_yr: 0, weight_kg: 72, scr_mgdl: 1.2 }));
  assert.ok("error" in computeCockcroftGaultCrcl({ age_yr: 70, weight_kg: 72, scr_mgdl: 0 }));
});

test("winters-expected-pco2: HCO3 12 -> 26", () => {
  const r = computeWintersExpectedPco2({ hco3: 12 });
  assert.strictEqual(r.expected_pco2, 26);
  assert.strictEqual(r.expected_low, 24);
  assert.strictEqual(r.expected_high, 28);
});
test("winters-expected-pco2: example", () => { assert.strictEqual(computeWintersExpectedPco2(wintersExpectedPco2Example.inputs).expected_pco2, 26); });
test("winters-expected-pco2: superimposed respiratory acidosis", () => {
  assert.match(computeWintersExpectedPco2({ hco3: 12, measured_pco2: 35 }).flag, /acidosis/);
});
test("winters-expected-pco2: concordant", () => {
  assert.match(computeWintersExpectedPco2({ hco3: 12, measured_pco2: 26 }).flag, /concordant/);
});
test("winters-expected-pco2: respiratory alkalosis when low", () => {
  assert.match(computeWintersExpectedPco2({ hco3: 12, measured_pco2: 18 }).flag, /alkalosis/);
});
test("winters-expected-pco2: zero HCO3 rejected", () => {
  assert.ok("error" in computeWintersExpectedPco2({ hco3: 0 }));
});

test("aa-gradient: FiO2.21 PaO2 70 PaCO2 40 age40 -> 29.7, expected 14", () => {
  const r = computeAaGradient({ fio2: 0.21, pao2: 70, paco2: 40, age_yr: 40 });
  assert.ok(Math.abs(r.aa_gradient - 29.73) < 0.1);
  assert.ok(Math.abs(r.expected_normal - 14) < 0.01);
});
test("aa-gradient: example", () => { assert.ok(computeAaGradient(aaGradientExample.inputs).aa_gradient > 0); });
test("aa-gradient: elevated flag", () => {
  assert.strictEqual(computeAaGradient({ fio2: 0.21, pao2: 70, paco2: 40, age_yr: 40 }).elevated, true);
});
test("aa-gradient: PAO2 uses 0.8 RQ", () => {
  const r = computeAaGradient({ fio2: 0.21, pao2: 70, paco2: 40, patm: 760, age_yr: 40 });
  assert.ok(Math.abs(r.pao2_alveolar - (0.21 * (760 - 47) - 40 / 0.8)) < 1e-6);
});
test("aa-gradient: higher FiO2 raises PAO2", () => {
  const a = computeAaGradient({ fio2: 0.21, pao2: 70, paco2: 40, age_yr: 40 });
  const b = computeAaGradient({ fio2: 0.5, pao2: 70, paco2: 40, age_yr: 40 });
  assert.ok(b.pao2_alveolar > a.pao2_alveolar);
});
test("aa-gradient: invalid FiO2/PaO2 rejected", () => {
  assert.ok("error" in computeAaGradient({ fio2: 1.5, pao2: 70, paco2: 40 }));
  assert.ok("error" in computeAaGradient({ fio2: 0.21, pao2: 0, paco2: 40 }));
});

test("fena: SNa140 UNa10 SCr3 UCr100 -> 0.21%", () => {
  const r = computeFena({ serum_na: 140, urine_na: 10, serum_cr: 3.0, urine_cr: 100 });
  assert.ok(Math.abs(r.fena_pct - 0.214) < 0.005);
  assert.match(r.band, /pre-renal/);
});
test("fena: example", () => { assert.ok(computeFena(fenaExample.inputs).fena_pct > 0); });
test("fena: intrinsic band when high", () => {
  assert.match(computeFena({ serum_na: 140, urine_na: 60, serum_cr: 1.5, urine_cr: 30 }).band, /intrinsic/);
});
test("fena: formula identity", () => {
  const r = computeFena({ serum_na: 140, urine_na: 10, serum_cr: 3.0, urine_cr: 100 });
  assert.ok(Math.abs(r.fena_pct - (10 * 3.0) / (140 * 100) * 100) < 1e-9);
});
test("fena: zero serum/urine creatinine rejected", () => {
  assert.ok("error" in computeFena({ serum_na: 140, urine_na: 10, serum_cr: 0, urine_cr: 100 }));
  assert.ok("error" in computeFena({ serum_na: 140, urine_na: 10, serum_cr: 3.0, urine_cr: 0 }));
});
...
// ---------------------------------------------------------------------------
import {
  computeIsaTempCorrection, isaTempCorrectionExample,
  computeWeightShiftCg, weightShiftCgExample,
  computeLandingTakeoffDaCorrection, landingTakeoffDaCorrectionExample,
} from "../../calc-aviation.js";

test("isa-temp-correction: MDA 1000 above station, OAT -20 -> 140 ft", () => {
  const r = computeIsaTempCorrection({ oat_c: -20, station_elev_ft: 0, published_alt_ft: 1000 });
  assert.ok(Math.abs(r.correction_ft - 140) < 1);
  assert.ok(Math.abs(r.corrected_alt_ft - 1140) < 1);
});
test("isa-temp-correction: example", () => { assert.ok(computeIsaTempCorrection(isaTempCorrectionExample.inputs).correction_ft > 0); });
test("isa-temp-correction: warmer than ISA -> no correction", () => {
  const r = computeIsaTempCorrection({ oat_c: 20, station_elev_ft: 0, published_alt_ft: 1000 });
  assert.strictEqual(r.correction_ft, 0);
  assert.strictEqual(r.meaningful, false);
});
test("isa-temp-correction: colder -> larger correction", () => {
  const a = computeIsaTempCorrection({ oat_c: -10, station_elev_ft: 0, published_alt_ft: 1000 });
  const b = computeIsaTempCorrection({ oat_c: -30, station_elev_ft: 0, published_alt_ft: 1000 });
  assert.ok(b.correction_ft > a.correction_ft);
});
test("isa-temp-correction: height drives magnitude", () => {
  const r = computeIsaTempCorrection({ oat_c: -20, station_elev_ft: 0, published_alt_ft: 2000 });
  assert.ok(Math.abs(r.correction_ft - 280) < 1);
});
test("isa-temp-correction: published below station rejected", () => {
  assert.ok("error" in computeIsaTempCorrection({ oat_c: -20, station_elev_ft: 1000, published_alt_ft: 500 }));
});

test("weight-shift-cg: empty 1500@36 + stations -> CG 38.25, in envelope", () => {
  const r = computeWeightShiftCg({ empty_weight_lb: 1500, empty_arm_in: 36, stations: [{ weight: 340, arm: 37 }, { weight: 50, arm: 70 }, { weight: 228, arm: 48 }], fwd_limit_in: 35, aft_limit_in: 47, max_gross_lb: 2550 });
  assert.strictEqual(r.total_weight_lb, 2118);
  assert.ok(Math.abs(r.cg_in - 38.25) < 0.02);
  assert.strictEqual(r.in_envelope, true);
});
test("weight-shift-cg: example", () => { assert.ok(computeWeightShiftCg(weightShiftCgExample.inputs).cg_in > 0); });
test("weight-shift-cg: CG = moment/weight", () => {
  const r = computeWeightShiftCg({ empty_weight_lb: 1500, empty_arm_in: 36, stations: [] });
  assert.ok(Math.abs(r.cg_in - 36) < 1e-9);
});
test("weight-shift-cg: over gross flagged", () => {
  const r = computeWeightShiftCg({ empty_weight_lb: 2500, empty_arm_in: 36, stations: [{ weight: 400, arm: 40 }], max_gross_lb: 2550 });
  assert.strictEqual(r.within_gross, false);
  assert.ok(r.lbs_over_gross > 0);
});
test("weight-shift-cg: aft CG out of limit", () => {
  const r = computeWeightShiftCg({ empty_weight_lb: 1500, empty_arm_in: 36, stations: [{ weight: 400, arm: 90 }], fwd_limit_in: 35, aft_limit_in: 47, max_gross_lb: 2550 });
  assert.strictEqual(r.within_aft, false);
});
test("weight-shift-cg: zero empty weight rejected", () => {
  assert.ok("error" in computeWeightShiftCg({ empty_weight_lb: 0, empty_arm_in: 36, stations: [] }));
});

test("landing-takeoff-da-correction: 1000ft PA5000 OAT25 -> DA 7388, roll 1739", () => {
  const r = computeLandingTakeoffDaCorrection({ ref_roll_ft: 1000, pressure_alt_ft: 5000, oat_c: 25 });
  assert.ok(Math.abs(r.density_alt_ft - 7388) < 1);
  assert.ok(Math.abs(r.corrected_roll_ft - 1739) < 1);
});
test("landing-takeoff-da-correction: example", () => { assert.ok(computeLandingTakeoffDaCorrection(landingTakeoffDaCorrectionExample.inputs).density_alt_ft > 0); });
test("landing-takeoff-da-correction: grass penalty increases roll", () => {
  const paved = computeLandingTakeoffDaCorrection({ ref_roll_ft: 1000, pressure_alt_ft: 5000, oat_c: 25 });
  const grass = computeLandingTakeoffDaCorrection({ ref_roll_ft: 1000, pressure_alt_ft: 5000, oat_c: 25, grass: true });
  assert.ok(grass.corrected_roll_ft > paved.corrected_roll_ft);
});
test("landing-takeoff-da-correction: headwind shortens roll", () => {
  const no = computeLandingTakeoffDaCorrection({ ref_roll_ft: 1000, pressure_alt_ft: 5000, oat_c: 25 });
  const hw = computeLandingTakeoffDaCorrection({ ref_roll_ft: 1000, pressure_alt_ft: 5000, oat_c: 25, headwind_kt: 10 });
  assert.ok(hw.corrected_roll_ft < no.corrected_roll_ft);
});
test("landing-takeoff-da-correction: tailwind warning", () => {
  assert.strictEqual(computeLandingTakeoffDaCorrection({ ref_roll_ft: 1000, pressure_alt_ft: 5000, oat_c: 25, headwind_kt: -5 }).tailwind_warning, true);
});
test("landing-takeoff-da-correction: zero ref roll rejected", () => {
  assert.ok("error" in computeLandingTakeoffDaCorrection({ ref_roll_ft: 0, pressure_alt_ft: 5000, oat_c: 25 }));
});
