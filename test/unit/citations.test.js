...
test("Group G audit coverage: every cross-trade tile has a CITATIONS entry", async () => {
  ...
  assert.ok(ids.length === 32, "expected 32 Group G tile ids, got " + ids.length);
...
test("Group M audit coverage: every water tile id has a CITATIONS entry", async () => {
  ...
  assert.ok(ids.length === 34, "expected 34 Group M tile ids, got " + ids.length);
  for (const id of ids) {
...
test("Group J audit coverage: every trucking tile id has a CITATIONS entry", async () => {
  ...
  assert.ok(ids.length === 19, "expected 19 Group J tile ids, got " + ids.length);
  for (const id of ids) {
