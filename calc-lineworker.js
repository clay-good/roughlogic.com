...
export function computePoleClassGroundlineMoment({ groundline_circumference_in = 0, fiber_stress_psi = 8000, load_1_lb = 0, height_1_ft = 0, load_2_lb = 0, height_2_ft = 0, load_3_lb = 0, height_3_ft = 0, check_height_ft = 0 } = {}) {
  ...
  if (Number(arguments[0]?.fiber_stress_psi) > 0 && Number(arguments[0]?.fiber_stress_psi) < 1000) return { error: "Enter the fiber stress in psi (8000), not ksi." };
...
export function computeGuyAnchorHoldingCapacity({ helix_diameter_in = 0, installed_depth_ft = 0, cohesion_psf = 0, friction_bearing_factor = 0, soil_unit_weight_pcf = 0, factor_of_safety = 2, installing_torque_ftlb = 0, torque_factor_per_ft = 10, guy_tension_lb = 0 } = {}) {
  const _g = _finiteGuard(arguments[0]); if (_g) return _g;
  // Unit / range guard added 2026-09-26 after printed-example probing.
  if (Number(arguments[0]?.factor_of_safety) > 0 && Number(arguments[0]?.factor_of_safety) < 1) return { error: "Factor of safety must be at least 1 (2.0 typical); below 1 the allowable exceeds the ultimate." };
...
LINEWORKER_RENDERERS["transverse-wind-load-conductor"] = _simpleRenderer({
  citation: "Citation: the transverse wind load relations by name -- force on a conductor = pressure x (diameter / 12) x wind span, force on a pole = pressure x its tapered projected area with the resultant at the trapezoid centroid, h/3 x (B + 2T)/(B + T) above the groundline, and the groundline moment = the sum of each force times its height. A wind speed entered instead of a pressure is converted with the ASCE 7 constant 0.00256 V^2, the same relation the wind-pressure calculator uses. Round shape factor 1.0; no gust, height, or exposure factor is applied. The applicable NESC edition, the utility's construction standards, and a qualified line designer govern.",
...
LINEWORKER_RENDERERS["feeder-loss-load-factor"] = _simpleRenderer({
  ...
  outputs: [
    ...
    { key: "c", id: "flf-out-c", label: "What the shortcuts would have said", value: (r) => Math.abs(r.load_factor_overstatement_pct) < 0.5 && Math.abs(r.peak_overstatement_x - 1) < 0.005 ? "all three agree at a flat load: " + fmt(r.peak_all_year_kwh, 0) + " kWh" : "peak loss for all 8,760 hours reads " + fmt(r.peak_all_year_kwh, 0) + " kWh, " + fmt(r.peak_overstatement_x, 1) + "x too high; the load factor alone reads " + fmt(r.using_load_factor_kwh, 0) + " kWh, still " + fmt(r.load_factor_overstatement_pct, 0) + "% high" },
