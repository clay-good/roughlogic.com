...
[roughlogic.com](https://roughlogic.com) is 1,921 small, single-purpose calculators for electricians, plumbers, HVAC techs, carpenters, restoration techs, firefighters, surveyors, and dozens of other trades. Everything runs in your browser. Every answer comes from a published formula and cites its source.
...
The hard part of a calculator catalog is not the arithmetic. It is proving, at scale, that every tile stays correct as the catalog grows. That is a build problem here: `npm run lint` runs 58 static gates before a change can land. Fifty-six of them run for anyone who clones this repository; the fifty-seventh, `check-ngrams`, compares text against a private hash list of licensed code spans that is deliberately not published, and skips with a message when the list is absent.
...
| `check-worked-examples` | every tile's example reproduces a reference number it names a source for -- a published worked example where the publisher prints one, and the project's own derivation where none exists. **525 of them are first-principles**: nobody publishes a worked example for Ohm's law or a footing area, so the derivation is the check |
...
| `check-citation-coverage` | every tile names a real, dated source with all four required fields and no orphans. Freshness tracking is narrower: 1,422 tiles cite a source on a recheck calendar, and **499 cite a source no freshness tracker covers** |
...
npm run lint       # the full static-gate chain (58 checks) -- 56 run without the private n-gram list
