...
> **414 deterministic tools across 24 trade groups. 0 dependencies. 0 trackers. 0 LLM calls. 4,751 unit tests. Works offline.**
...
414 tiles across 24 active group letters. The letters are stable identifiers; new groups append, retired tiles keep their IDs.
...
The home document renders the SPA, and per-tile state lives in the URL hash. But fragments are not part of URL canonicalization, and most non-Google crawlers do not execute JavaScript, so the cited reference content of 414 tiles would otherwise be invisible to general web search. Spec-v13 fixes this with a build-time prerender step.
...
- **`sitemap.xml`** carries 440 URLs (home + changelog + 24 groups + 414 tiles), regenerated at every build.
...
|---|---|---|
| A | Every exported function has a formula-corpus row in `docs/derivations.md` | Complete (691 rows; lint fails on a stale section) |
...
| D | Every function passes the bounds-and-edge-case fuzzer | Complete (691/691) |
...
| G | Every tile maps to a tracked published source | Measurement mode (359/414 tiles tracked, 86.7%) |
| H | Every active non-exempt group has a current reviewer signoff | Pending external reviewers (H and Q are exempt) |
| I | One derivation-index row per tile | Complete (414/414) |
...
| `npm run build` | Produce the static `dist/` for deployment (copies the SPA, prerenders 414 tile + 24 group shells, regenerates `sitemap.xml`). |
| `npm test` / `npm run test:unit` | Run the unit suite under Node's built-in test runner (4,751 tests). |
