...
> **435 deterministic tools across 24 trade groups. 0 dependencies. 0 trackers. 0 LLM calls. 4,939 unit tests. Works offline.**
...
roughlogic.com/
  ...
  tools-data.js         catalog registry (TOOLS, 435 tiles); lazy-loaded (~98 KB)
...
435 tiles across 24 active group letters. The letters are stable identifiers; new groups append, retired tiles keep their IDs.
...
| X | Real Estate | 18 | LTV, DTI, PITI, cap rate/DSCR, 1031 timeline, closing costs, discount-point break-even, per-diem closing interest, reserves requirement |
...
The home document renders the SPA, and per-tile state lives in the URL hash. But fragments are not part of URL canonicalization, and most non-Google crawlers do not execute JavaScript, so the cited reference content of 435 tiles would otherwise be invisible to general web search. Spec-v13 fixes this with a build-time prerender step.
...
- **`sitemap.xml`** carries 461 URLs (home + changelog + 24 groups + 435 tiles), regenerated at every build.
...
| G | Every tile maps to a tracked published source | Measurement mode (376/435 tiles tracked, 86.4%; 163 tracked sources) |
| H | Every active non-exempt group has a current reviewer signoff | Pending external reviewers (H and Q are exempt) |
| I | One derivation-index row per tile | Complete (435/435) |
...
| `npm run build` | Produce the static `dist/` for deployment (copies the SPA, prerenders 435 tile + 24 group shells, regenerates `sitemap.xml`). |
| `npm test` / `npm run test:unit` | Run the unit suite under Node's built-in test runner (4,939 tests). |
