...
> **437 deterministic tools across 24 trade groups. 0 dependencies. 0 trackers. 0 LLM calls. 4,990 unit tests. Works offline.**
...
roughlogic.com/
  ...
  tools-data.js         catalog registry (TOOLS, 437 tiles); lazy-loaded (~98 KB)
...
437 tiles across 24 active group letters. The letters are stable identifiers; new groups append, retired tiles keep their IDs.
...
| Y | Educators and K-12 | 18 | readability scores, bell-curve CDF, standards-based grading, statistics quick-read, confidence interval, Pearson correlation, chi-square goodness-of-fit, linear regression |
...
The home document renders the SPA, and per-tile state lives in the URL hash. But fragments are not part of URL canonicalization, and most non-Google crawlers do not execute JavaScript, so the cited reference content of 437 tiles would otherwise be invisible to general web search. Spec-v13 fixes this with a build-time prerender step.
...
- **`sitemap.xml`** carries 463 URLs (home + changelog + 24 groups + 437 tiles), regenerated at every build.
...
| I | One derivation-index row per tile | Complete (437/437) |
...
| `npm run build` | Produce the static `dist/` for deployment (copies the SPA, prerenders 437 tile + 24 group shells, regenerates `sitemap.xml`). |
| `npm test` / `npm run test:unit` | Run the unit suite under Node's built-in test runner (4,990 tests). |
...
| `npm run check:shell-mobile` | Audit every prerendered shell (437 tools + 24 groups + changelog) for zero 320px horizontal scroll (needs a built `dist/` + Playwright; skips cleanly otherwise). |
...
| `check-tile-contract` | Every registered compute function honors the spec-v18 §2 output contract: no throw, hang, OOM, impurity, input mutation, or `NaN`/`Infinity` on its canonical input; the perturbed-input non-finite-leak backlog is ratcheted (no regressions). |
