...
> **555 deterministic tools across 24 trade groups. 0 dependencies. 0 trackers. 0 LLM calls. 5,482 unit tests. Works offline.**
...
555 tiles across 24 active group letters. The letters are stable identifiers; new groups append, retired tiles keep their IDs.
...
The home document renders the SPA, and per-tile state lives in the URL hash. But fragments are not part of URL canonicalization, and most non-Google crawlers do not execute JavaScript, so the cited reference content of 555 tiles would otherwise be invisible to general web search. Spec-v13 fixes this with a build-time prerender step.
...
- **`sitemap.xml`** carries 581 URLs (home + changelog + 24 groups + 555 tiles), regenerated at every build.
...
|---|---|---|
| A | Every exported function has a formula-corpus row in `docs/derivations.md` | Complete (859 rows; lint fails on a stale section) |
...
| C | Every function carries a dimensional-analysis annotation that parses and balances | Complete (862/862) |
| D | Every function passes the bounds-and-edge-case fuzzer | Complete (859/859) |
...
| G | Every tile maps to a classified source: a tracked published authority, or an explicit first-principles / public-domain / author-original class with no edition cycle | Complete (555/555 tiles classified, 100%; 206 tracked sources) |
| H | Every active non-exempt group has a current reviewer signoff | Pending external reviewers (H and Q are exempt) |
| I | One derivation-index row per tile | Complete (555/555) |
...
| `npm run build` | Produce the static `dist/` for deployment (copies the SPA, prerenders 555 tile + 24 group shells, regenerates `sitemap.xml`). |
| `npm test` / `npm run test:unit` | Run the unit suite under Node's built-in test runner (5,482 tests). |
...
| `npm run check:shell-mobile` | Audit every prerendered shell (555 tools + 24 groups + changelog + home) for zero horizontal scroll: all at the 320px floor, plus a representative sample at 568x320 landscape and 200% text zoom (needs a built `dist/` + Playwright; skips cleanly otherwise). |
