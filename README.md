...
> **385 deterministic tools across 24 trade groups. 0 dependencies. 0 trackers. 0 LLM calls. 4,469 unit tests. Works offline.**
...
roughlogic.com/
  ...
  app.js                SPA entry: TOOLS registry, router, renderers (~132 KB)
...
385 tiles across 24 active group letters. The letters are stable identifiers; new groups append, retired tiles keep their IDs.

| Letter | Group | Tiles | Representative tools |
|---|---|--:|---|
| A | Electrical | 33 | voltage drop, wire ampacity, service load, motor FLA, PoE budget |
...
| G | Cross-Trade Utilities | 27 | unit conversion, mileage cost, NIOSH lifting, heat stress, haversine |
...
The home document renders the SPA, and per-tile state lives in the URL hash. But fragments are not part of URL canonicalization, and most non-Google crawlers do not execute JavaScript, so the cited reference content of 385 tiles would otherwise be invisible to general web search. Spec-v13 fixes this with a build-time prerender step.
...
- **`sitemap.xml`** carries 411 URLs (home + changelog + 24 groups + 385 tiles), regenerated at every build.
...
| G | Every tile maps to a tracked published source | Measurement mode (335/385 tiles tracked) |
| H | Every active non-exempt group has a current reviewer signoff | Pending external reviewers (H and Q are exempt) |
| I | One derivation-index row per tile | Complete (385/385) |
...
| `npm run build` | Produce the static `dist/` for deployment (copies the SPA, prerenders 385 tile + 24 group shells, regenerates `sitemap.xml`). |
| `npm test` / `npm run test:unit` | Run the unit suite under Node's built-in test runner (4,469 tests). |
