...
> **549 deterministic tools across 24 trade groups. 0 dependencies. 0 trackers. 0 LLM calls. 5,474 unit tests. Works offline.**
...
roughlogic.com/
  ...
  tools-data.js         catalog registry (TOOLS, 549 tiles); lazy-loaded (~130 KB raw / ~43 KB gz)
...
549 tiles across 24 active group letters. The letters are stable identifiers; new groups append, retired tiles keep their IDs.

| Letter | Group | Tiles | Representative tools |
|---|---|--:|---|
| A | Electrical | 55 | voltage drop (with reactance), power triangle, EV charger load, service load (220.82), PV interconnection busbar, conduit offset / saddle / 90-stub bending, **motor-feeder sizing (430.24/430.62)**, **transformer conductor/protection (450.3(B))**, **fiber loss budget**, **cable-tray fill (392.22)**, **CCTV/NVR storage**, **70 V speaker line**, **fire-alarm standby battery (NFPA 72)**, **coax attenuation** |
| B | Plumbing and Gas | 41 | friction loss, pipe sizing, water hammer, water-heater recovery, sanitary DFU, Spitzglass gas pressure drop, **mixing-valve blend temp (ASSE)**, **well pressure-tank drawdown**, **pipe velocity / copper erosion** |
| C | HVAC | 46 | duct sizing, Manual J (simplified), refrigerant P-T, chiller tonnage, LMTD/NTU, superheat/subcool charge verdict, economizer free-cooling hours, **round-to-rectangular duct equivalent (ASHRAE)** |
...
| E | Carpentry and Construction | 56 | joist/beam spans, header sizing (R602.7), deck beam/post (R507), wind/snow load, wall bracing, column buckling (Cp), welding heat input, metal weight, 3-4-5 layout squaring, horizontal / vertical curve layout, earthwork end-area volume, slope-stake cut/fill, **fillet-weld strength (AWS D1.1 / AISC J2)** |
...
| G | Cross-Trade Utilities | 37 | unit conversion, mileage cost, NIOSH lifting, heat stress, haversine, rolling offset (pipefitting), **fitting take-out cut length**, **multi-piece miter elbow**, **pipe wraparound template**, **flange bolt-up torque (ASME PCC-1)**, **center of gravity from two scales** |
...
The home document renders the SPA, and per-tile state lives in the URL hash. But fragments are not part of URL canonicalization, and most non-Google crawlers do not execute JavaScript, so the cited reference content of 549 tiles would otherwise be invisible to general web search. Spec-v13 fixes this with a build-time prerender step.
...
- **`sitemap.xml`** carries 575 URLs (home + changelog + 24 groups + 549 tiles), regenerated at every build.
...
|---|---|---|
| A | Every exported function has a formula-corpus row in `docs/derivations.md` | Complete (853 rows; lint fails on a stale section) |
...
| C | Every function carries a dimensional-analysis annotation that parses and balances | Complete (856/856) |
| D | Every function passes the bounds-and-edge-case fuzzer | Complete (853/853) |
...
| G | Every tile maps to a classified source: a tracked published authority, or an explicit first-principles / public-domain / author-original class with no edition cycle | Complete (549/549 tiles classified, 100%; 205 tracked sources) |
| H | Every active non-exempt group has a current reviewer signoff | Pending external reviewers (H and Q are exempt) |
| I | One derivation-index row per tile | Complete (549/549) |
...
| `npm run build` | Produce the static `dist/` for deployment (copies the SPA, prerenders 549 tile + 24 group shells, regenerates `sitemap.xml`). |
| `npm test` / `npm run test:unit` | Run the unit suite under Node's built-in test runner (5,474 tests). |
...
| `npm run check:shell-mobile` | Audit every prerendered shell (549 tools + 24 groups + changelog + home) for zero horizontal scroll: all at the 320px floor, plus a representative sample at 568x320 landscape and 200% text zoom (needs a built `dist/` + Playwright; skips cleanly otherwise). |
