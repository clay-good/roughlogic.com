...
[roughlogic.com](https://roughlogic.com) is 1,915 small, single-purpose calculators for electricians, plumbers, HVAC techs, carpenters, restoration techs, firefighters, surveyors, and dozens of other trades. Everything runs in your browser. Every answer comes from a published formula and cites its source.
...
| `check-worked-examples` | every tile's example reproduces a reference number it names a source for -- a published worked example where the publisher prints one, and the project's own derivation where none exists. **519 of them are first-principles**: nobody publishes a worked example for Ohm's law or a footing area, so the derivation is the check |
...
| `check-citation-coverage` | every tile names a real, dated source with all four required fields and no orphans. Freshness tracking is narrower: 1,419 tiles cite a source on a recheck calendar, and **496 cite a source no freshness tracker covers** |
