...
| `integrity-banner` (e2e) | the threat model's T4 promise -- if bundled data is tampered with, the reader gets **a visible warning, never silent corruption** -- exercised against a browser rather than asserted. It rewrites `data/integrity.json` in flight so a real manifest fails its recorded hash, then checks the reader is warned, that the warning is announced (`role="alert"`) and names the dataset, and that the site keeps working -- because a control that blocks the page on one corrupted byte has become a denial of service. Until 2026-09-04 only the absence of the banner was ever asserted, which a deleted banner satisfies |
| `answer-above-inputs` (e2e) | step 3 above -- "the answer sits at the top of the page, above the inputs that produced it" -- measured as a reader experiences it. It compares rendered POSITION, not DOM order, because a stylesheet can invert the page without touching the markup: seeding `order: -1` on the input region flips it while `app.js` is unchanged, and this catches that. Nothing asserted the page's reading order until 2026-09-04, though it was chosen deliberately in the 2026-08-16 presentation overhaul |
...
| `check-fixture-keys` / `check-guard-only-inputs` / `check-render-output-keys` | each of these parses source rather than running it, so each has a set it cannot see into: a fixture whose compute takes a rest parameter, a compute with a named object signature, a compute that returns a spread. All three now **name that set and fail when it grows** -- 30, 4 and 7 respectively, each budget lowered as a signature is brought under analysis rather than left where it was found. Before 2026-09-04 they printed the count in a green summary and nothing read it, so coverage could fall one tile at a time. The last seven are covered a second way rather than rewritten: `spread-return-outputs` RUNS them against their worked examples and compares the keys actually returned to the ones the renderer reads, which is more than a static parse can know -- for the one input path a fixture exercises, which it says. |
...
| `check-feedback-loop` | every current and future calculator retains the shared defensive D1 reporting path, and the three places a size limit is stated -- what the browser lets you type, what the Worker re-checks because the browser is not trusted, and what the D1 `CHECK` constraint enforces because the Worker is code that can change -- agree. Three layers is the design; until 2026-09-04 each named its own number and nothing compared them, and the drift would have landed on a reader's bug report as a 500 |
