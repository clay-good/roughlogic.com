...
> **425 deterministic tools across 24 trade groups. 0 dependencies. 0 trackers. 0 LLM calls. 4,867 unit tests. Works offline.**
...
roughlogic.com/
  ...
  tools-data.js         catalog registry (TOOLS, 425 tiles); lazy-loaded (~96 KB)
...
425 tiles across 24 active group letters. The letters are stable identifiers; new groups append, retired tiles keep their IDs.
...
The home document renders the SPA, and per-tile state lives in the URL hash. But fragments are not part of URL canonicalization, and most non-Google crawlers do not execute JavaScript, so the cited reference content of 425 tiles would otherwise be invisible to general web search. Spec-v13 fixes this with a build-time prerender step.
...
- **`sitemap.xml`** carries 451 URLs (home + changelog + 24 groups + 425 tiles), regenerated at every build.
...
| G | Every tile maps to a tracked published source | Measurement mode (367/425 tiles tracked, 86.8%) |
| H | Every active non-exempt group has a current reviewer signoff | Pending external reviewers (H and Q are exempt) |
| I | One derivation-index row per tile | Complete (425/425) |
...
| `npm run build` | Produce the static `dist/` for deployment (copies the SPA, prerenders 425 tile + 24 group shells, regenerates `sitemap.xml`). |
