...
[roughlogic.com](https://roughlogic.com) is 1,856 small, single-purpose calculators for electricians, plumbers, HVAC techs, carpenters, restoration techs, firefighters, surveyors, and dozens of other trades. Everything runs in your browser. Every answer comes from a published formula and cites its source.
...
| `check-worked-examples` | every tile's example reproduces a reference number it names a source for -- a published worked example where the publisher prints one, and the project's own derivation where none exists. **460 of them are first-principles**: nobody publishes a worked example for Ohm's law or a footing area, so the derivation is the check |
| `check-example-parity` | the example a page prints is the example its calculator opens (1,673 tiles statically; the 131 that declare theirs inline are driven in a real browser by `test/integration/example-parity-runtime.test.js`, so the claim covers all 1,856) |
| `check-citation-coverage` | every tile names a real, dated source with all four required fields and no orphans. Freshness tracking is narrower: 1,404 tiles cite a source on a recheck calendar, and **452 cite a source no freshness tracker covers** |
...
| `check-tile-registries` | the new-tile checklist names every registry that holds all 1,856 ids, and calls no partial one mandatory |
