...
|---|---|
| `check-dimensions` | all 2,337 exported calculator functions carry a machine-parsed `// dims:` annotation declaring each input's and each output's SI dimensions, and a malformed one fails the build. 1,942 of those keys are *named* for a unit (`_psi`, `_gpm`, `_rpm`, `_hp`, `_ft3`, `_fps`) and are additionally held to the dimension that unit implies -- the check that caught 136 wrong declarations in one pass. It still checks the **declaration**, not the arithmetic against it -- verifying the expression would need a CAS, which this repo has no dependency on |
