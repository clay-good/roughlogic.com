...
> **561 deterministic tools across 24 trade groups. 0 dependencies. 0 trackers. 0 LLM calls. 5,498 unit tests. Works offline.**
...
roughlogic.com/
  ...
  tools-data.js         catalog registry (TOOLS, 561 tiles); lazy-loaded (~130 KB raw / ~43 KB gz)
  ...
  specs/                spec.md .. spec-v37.md (inheriting build specs)
...
561 tiles across 24 active group letters. The letters are stable identifiers; new groups append, retired tiles keep their IDs.
...
| G | Cross-Trade Utilities | 41 | unit conversion, mileage cost, NIOSH lifting, heat stress, haversine, rolling offset (pipefitting), **fitting take-out cut length**, **multi-piece miter elbow**, **pipe wraparound template**, **flange bolt-up torque (ASME PCC-1)**, **center of gravity from two scales**, **bolt-circle hole layout**, **decimal to fraction / feet-inches**, **sine bar angle setup** |
...
The home document renders the SPA, and per-tile state lives in the URL hash. But fragments are not part of URL canonicalization, and most non-Google crawlers do not execute JavaScript, so the cited reference content of 561 tiles would otherwise be invisible to general web search. Spec-v13 fixes this with a build-time prerender step.
...
- **`sitemap.xml`** carries 587 URLs (home + changelog + 24 groups + 561 tiles), regenerated at every build.
...
|---|---|---|
| A | Every exported function has a formula-corpus row in `docs/derivations.md` | Complete (865 rows; lint fails on a stale section) |
...
| C | Every function carries a dimensional-analysis annotation that parses and balances | Complete (868/868) |
| D | Every function passes the bounds-and-edge-case fuzzer | Complete (865/865) |
...
| G | Every tile maps to a classified source: a tracked published authority, or an explicit first-principles / public-domain / author-original class with no edition cycle | Complete (561/561 tiles classified, 100%; 206 tracked sources) |
| H | Every active non-exempt group has a current reviewer signoff | Pending external reviewers (H and Q are exempt) |
| I | One derivation-index row per tile | Complete (561/561) |
...
| `npm run build` | Produce the static `dist/` for deployment (copies the SPA, prerenders 561 tile + 24 group shells, regenerates `sitemap.xml`). |
| `npm test` / `npm run test:unit` | Run the unit suite under Node's built-in test runner (5,498 tests). |
...
| `npm run check:shell-mobile` | Audit every prerendered shell (561 tools + 24 groups + changelog + home) for zero horizontal scroll: all at the 320px floor, plus a representative sample at 568x320 landscape and 200% text zoom (needs a built `dist/` + Playwright; skips cleanly otherwise). |
