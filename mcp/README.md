...
server that exposes the roughlogic catalog of **1,908 trades calculators**
...
clean through `run_calculator`. All three are checked for all 1,908 tiles on
every build.
...
`check-both-doors.mjs` holds that. **1,908 of 1,908 calculators name their
...
exist for 1,868 calculators. For the other 40 it projects the descriptors from
