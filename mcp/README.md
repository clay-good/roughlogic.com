...
server that exposes the roughlogic catalog of **1,811 trades calculators**
...
clean through `run_calculator`. All three are checked for all 1,811 tiles on
every build.
...
`check-both-doors.mjs` holds that. **1,811 of 1,811 calculators name their
...
exist for 1,771 calculators. For the other 40 it projects the descriptors from
