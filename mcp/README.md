...
server that exposes the roughlogic catalog of **2,098 trades calculators**
...
clean through `run_calculator`. All three are checked for all 2,098 tiles on
every build.
...
`check-both-doors.mjs` holds that. **2,098 of 2,098 calculators name their
...
exist for 2,062 calculators. For the other 36 it projects the descriptors from
...
every other kind of evidence, and counts as its own corroboration. All 2,098
...
declares no unit in its label at all. Recovery across all 2,062 tiles is
