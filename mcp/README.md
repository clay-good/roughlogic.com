...
server that exposes the roughlogic catalog of **2,170 trades calculators**
...
clean through `run_calculator`. All three are checked for all 2,170 tiles on
every build.
...
`check-both-doors.mjs` holds that. **2,170 of 2,170 calculators name their
...
exist for 2,134 calculators. For the other 36 it projects the descriptors from
...
every other kind of evidence, and counts as its own corroboration. All 2,170
...
declares no unit in its label at all. Recovery across all 2,134 tiles is
