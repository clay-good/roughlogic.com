...
server that exposes the roughlogic catalog of **2,080 trades calculators**
...
clean through `run_calculator`. All three are checked for all 2,080 tiles on
every build.
...
`check-both-doors.mjs` holds that. **2,080 of 2,080 calculators name their
...
exist for 2,040 calculators. For the other 40 it projects the descriptors from
