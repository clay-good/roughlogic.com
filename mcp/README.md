...
server that exposes the roughlogic catalog of **1,958 trades calculators**
...
clean through `run_calculator`. All three are checked for all 1,958 tiles on
every build.
...
`check-both-doors.mjs` holds that. **1,958 of 1,958 calculators name their
...
exist for 1,918 calculators. For the other 40 it projects the descriptors from
