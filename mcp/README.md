...
server that exposes the roughlogic catalog of **2,133 trades calculators**
...
clean through `run_calculator`. All three are checked for all 2,133 tiles on
every build.
...
`check-both-doors.mjs` holds that. **2,133 of 2,133 calculators name their
...
exist for 2,097 calculators. For the other 36 it projects the descriptors from
...
every other kind of evidence, and counts as its own corroboration. All 2,133
...
declares no unit in its label at all. Recovery across all 2,097 tiles is
