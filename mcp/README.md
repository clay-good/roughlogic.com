...
server that exposes the roughlogic catalog of **2,006 trades calculators**
...
clean through `run_calculator`. All three are checked for all 2,006 tiles on
every build.
...
`check-both-doors.mjs` holds that. **2,006 of 2,006 calculators name their
...
exist for 1,966 calculators. For the other 40 it projects the descriptors from
