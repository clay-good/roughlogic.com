...
server that exposes the roughlogic catalog of **1,902 trades calculators**
...
clean through `run_calculator`. All three are checked for all 1,902 tiles on
every build.
...
`check-both-doors.mjs` holds that. **1,902 of 1,902 calculators name their
...
exist for 1,862 calculators. For the other 40 it projects the descriptors from
