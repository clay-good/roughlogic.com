...
server that exposes the roughlogic catalog of **1,968 trades calculators**
...
clean through `run_calculator`. All three are checked for all 1,968 tiles on
every build.
...
`check-both-doors.mjs` holds that. **1,968 of 1,968 calculators name their
...
exist for 1,928 calculators. For the other 40 it projects the descriptors from
