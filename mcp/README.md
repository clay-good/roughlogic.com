...
server that exposes the roughlogic catalog of **2,157 trades calculators**
...
clean through `run_calculator`. All three are checked for all 2,157 tiles on
every build.
...
`check-both-doors.mjs` holds that. **2,157 of 2,157 calculators name their
...
exist for 2,121 calculators. For the other 36 it projects the descriptors from
...
every other kind of evidence, and counts as its own corroboration. All 2,157
...
declares no unit in its label at all. Recovery across all 2,121 tiles is
