...
server that exposes the roughlogic catalog of **2,042 trades calculators**
...
clean through `run_calculator`. All three are checked for all 2,042 tiles on
every build.
...
`check-both-doors.mjs` holds that. **2,042 of 2,042 calculators name their
...
exist for 2,002 calculators. For the other 40 it projects the descriptors from
