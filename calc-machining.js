...
// dims: in { tpi: L^-1 } out: { pitch_in: L, single_depth_in: L, compound_infeed_in: L }
...
export function computeStubAcmeThreadDepth({ major_dia_in = 0, tpi = 0 } = {}) {
  ...
  return {
    pitch_in, thread_depth_in, pitch_dia_in, minor_dia_in, crest_flat_in,
    note: "Stub Acme (29-degree) thread dimensions, the shallower form the general-purpose acme-thread-depth tile names as different. It keeps the 29-degree flank of general Acme but cuts the thread height to h = 0.3 P (against P/2), giving a stronger root and a shallower engagement where axial space or thread strength matters more than travel. The pitch P = 1/TPI; the basic thread depth h = 0.3 P; the basic pitch diameter = D - 0.3 P; the basic minor diameter = D - 2h (the internal thread's minimum minor; ASME B1.8 puts the external thread's maximum minor below it, 0.8600 against 0.8800 at 1-5) = D - 0.6 P; and the crest flat = P/2 - h tan(14.5 deg) = 0.4224 P (wider than the general form's 0.3707 P because the shorter flanks leave more crest). A 1 in, 5-TPI Stub Acme (\"1-5 Stub Acme\") has a 0.200 in pitch, a 0.060 in thread depth, a 0.940 in pitch diameter, a 0.880 in minor diameter, and a 0.0845 in crest flat. Use the stub form for hardened or heavily loaded lead screws, valve stems, and thin-wall parts where a full-depth Acme would over-weaken the root. Cut with a 29-degree tool ground to the crest-flat width and check the pitch diameter over wires. ASME B1.8 and the thread gauge govern the finished fit.",
  };
}
...
function renderStubAcmeThreadDepth(inputRegion, outputRegion, citationEl) {
  citationEl.textContent = "Citation: Stub Acme (29-degree) thread dimensions (Machinery's Handbook / ASME B1.8): pitch = 1/TPI, basic thread depth = 0.3 P, pitch dia = D - 0.3 P, basic minor (internal thread minimum; the external thread's maximum minor is smaller per ASME B1.8) = D - 0.6 P, crest flat = P/2 - 0.3P tan(14.5 deg) = 0.4224 P. The shallower companion to general-purpose Acme (P/2 depth). Cut with a 29-degree tool and verify over wires. ASME B1.8 and a thread gauge govern.";
