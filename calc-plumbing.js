...
export function computeRecircLoopSizing({
  ...
  // at its efficiency (0.98 electric resistance, 0.8 gas when none is entered). Gas is
  // billed per therm (100,000 BTU); electric per kWh (3,412 BTU). The fuel price is
  // optional, so the cost is null until the user supplies one.
  const isElectric = fuel === "electric";
  const _he = Number(heater_efficiency);
  const eff = heater_efficiency != null && heater_efficiency !== "" && _he > 0 ? _he : (isElectric ? 0.98 : 0.8);
...
function _v9p_renderRecircLoopSizing(inputRegion, outputRegion, citationEl) {
  const isElec = () => fuel.select.value === "electric";
  ...
  // Swap the efficiency field to the new fuel's default unless the user typed their own.
  fuel.select.addEventListener("change", () => {
    if (eff.input.value === "0.8" || eff.input.value === "0.98") eff.input.value = isElec() ? "0.98" : "0.8";
  });
...
// spec-v1030 cap-relief split: the plumbing TAKEOFF / materials bench
// (solder-joint-quantity, pipe-insulation-takeoff, heat-trace-sizing,
// pipe-purge-volume, hydronic-system-volume, pex-homerun-takeoff,
// solar-thermal-collector) relocated to calc-plumbingtakeoff.js. That block was
// fully self-contained -- compute and renderer co-located, no shared constant
