...
> **COMPLETE. All 38 bands have landed: railroad track, elevator and escalator, door hardware and locksmithing, mining and quarry, trenchless, sawmill, wind energy, diving, steam plant with commercial laundry, overhead line and distribution, millwright alignment, vibration, drives and compressed air, the distribution side of line work, traffic, work zone and pavement, concrete placement and rail logistics, sheet metal, masonry and scaffold, groundwater, survey and kitchen, arboriculture and abatement, industrial refrigeration in the new `calc-refrigeration.js`, oil, gas and pipeline in the new `calc-oilgas.js`, HVAC test-and-balance and hydronic systems, air quality and emissions in the new `calc-airquality.js`, the marine and aviation band, building performance and envelope diagnostics in the new `calc-buildingperf.js`, plastics processing with foundry practice in the new `calc-process.js`, nondestructive examination with heat treatment in the new `calc-inspection.js`, industrial hygiene in the new `calc-hygiene.js`, water systems in `calc-water.js`, and pool and spa service in the new `calc-pool.js`, radon, acid waste and laboratory containment in the new `calc-containment.js`, the propane and LP-gas tank side in `calc-gas.js`, the tank battery, separation and flare surface facilities in `calc-oilgas.js`, HVAC acoustics with rooftop anchorage in `calc-hvacsystems.js`, auto body paint and panel bonding in `calc-mechanic.js`, mechanical insulation with refractory and cryogenic service in `calc-hvacsystems.js`, dispersion, community noise and odour closing `calc-airquality.js`, and the final band of underground duct-bank ampacity and ground loop pumping, 1,804 -> 2,082. Twenty-two of the 300 specs were cut as duplicates; the other 278 shipped.**
...
| v1505 | `framing-factor-whole-wall` | **CUT** -- duplicate of `assembly-r-value`, which gained the area-weighted wrong method and a second framing factor | `calc-hvac.js` |
...
| v1675 | `personnel-protection-thickness` | **CUT** -- duplicate of `insulation-thickness`, which gained the forward surface-temperature solve, the second film condition, and the jacket-material criterion | `calc-hvac.js` |
| v1676 | `jacketing-fitting-quantity` | **CUT** -- duplicate of `pipe-insulation-takeoff`, which gained the derived jacket OD, the lap allowance, and the fitting and valve covers counted as pieces | `calc-plumbingtakeoff.js` |
...
| v1660 | `paint-booth-airflow-cure` | **CUT** -- duplicate of `spray-booth-airflow`, which gained the air changes, the bake cycle, and the filter-loading velocity check | `calc-shop.js` |
| v1661 | `frame-diagonal-tolerance` | **CUT** -- duplicate of `layout-squaring`, which gained the tolerance verdict and the paired left/right symmetry check | `calc-construction.js` |
| v1662 | `wet-film-for-dry-build` | **CUT** -- duplicate of `coating-coverage-dft`, which gained the reduction term and the dry build a wet gauge reading leaves | `calc-construction.js` |
...
| 8 | v1550-v1556 | `calc-wind.js` (new) | 7 | wind energy; catalog 1,862 -> 1,869. No cut. **spec-v1556's pole compression is degenerate** and spec-v1554's barometric relation disagreed with the sibling `air-density-correction`; both corrected |
| 9 | v1557-v1562 | `calc-diving.js` (new) | 6 | diving; catalog 1,869 -> 1,875. No cut. **THREE internally wrong specs**, two of them about OXYGEN LIMITS, plus a blend-rounding error running through both nitrox tiles |

**Fourteen of the seventy-three specs built so far were internally wrong and
shipped corrected, and THREE have been cut as duplicates.** Band 9 alone
added three, two of them about oxygen partial-pressure limits: spec-v1559
calls 1.096 ata "past 1.4" while its own MOD says 111 ft, spec-v1560 calls
EAN35 usable at 100 ft where it is 1.411 ata, and BOTH nitrox specs round a
blend figure UP, past the limit they were solving for. A blend figure floors. Band 7 was clean in both
respects. Band 8 added spec-v1556, whose pole compression `C = W + T cos(alpha)`
is identically 2W at every angle because its own `T = W / cos(alpha)` uses the
same angle; the correct sheave statics are `2 W cos(beta/2)`. Band 6 added spec-v1597, whose rule line says a steel
