...
| `npm run lint` (includes `check-spec-status`) | Fails if a spec's `**Status:**` line disagrees with the catalog: PROPOSED while every tile it names is in `tools-data.js` (or a calc module carries its `// spec-vN` comment); LANDED while a named id is missing and no as-built id is given; CUT while a named id is live and the status does not name the collision. Platform specs, which name no tile ids, are not read. |
