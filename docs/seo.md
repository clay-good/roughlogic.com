...
already pinned by `check-readme-counts`: "2,157 free calculators for
...
- Sideways to 3-6 related tiles via the curated `RELATED` registry
  ...
  growing with the editorial map). The registry covers 1,991 of the
  2,157 tiles; the catalog outgrew the Phase E expansion that once
...
- Sideways *into* every tile, which the per-tile lists alone do not
  ...
  six. 296 of the 297 find a host; `historical-pricing` cannot,
  being the only tile in group Q. The graph now carries 7,475 edges
  across 2,157 tiles, a mean of 3.5 inbound links per tile, and the
...
default: no wordmark, no search, no way back into 2,157 calculators.
