...
- **New tests (Phase E + Phase F scaffolding + follow-ups + calc-module extensions + Phase D scaffolding + Phase D calc-module extensions)**: [test/unit/numerical-stability.test.js](../test/unit/numerical-stability.test.js) (33 tests), [test/unit/cross-tile-invariants.test.js](../test/unit/cross-tile-invariants.test.js) (20 tests), and (2026-05-19) [test/unit/bounds-fuzzer.test.js](../test/unit/bounds-fuzzer.test.js) (85 tests, of which 30 are the pure-math scaffolding, 28 are the calc-fire Phase D extension (14 initial + 14 same-day expansion), 14 are the calc-aviation Phase D expansion (`computePressureAltitude`, `computeCrosswind`, `computeETE`, `computeHypoxiaAltitude`, `computeTopOfDescent`, `computeTrueAirspeed` with identity pins for ISA pressure-altitude, Pythagorean HW/CW, 3-to-1 descent, and TAS = CAS / sqrt(rho_ratio)), and 13 are the calc-hvac Phase D expansion (`computeSHR`, `computeSeerEer`, `computeBalancePoint`, `computeCfmPerTon`, `computeCombustionAir`, `computeApproachDeltaT`, `computeEvaporativeCooling`, `computeAffinityLaws`, `computeOutdoorAirMix` with identity pins for fan-affinity Q~N / SP~N^2 / kW~N^3, SEER = 1.12 * EER round-trip, per-climate CFM-per-ton factors, evaporative-cooling Q = m * hfg, and the outdoor-air mass-balance temperature mix), on top of the 3 calc-hvac Phase D scaffolding rows for Manual J + duct sizing. All passing. Test-suite total: 3,585.
