...
- **New tests (Phase E + Phase F scaffolding + follow-ups + calc-module extensions)**: [test/unit/numerical-stability.test.js](../test/unit/numerical-stability.test.js) (33 tests, all passing after the §9.1 NaN-guard hardening and the Group F / W / C calc-module extensions) and [test/unit/cross-tile-invariants.test.js](../test/unit/cross-tile-invariants.test.js) (20 tests, all passing after the CFM<->m^3/s cross-group invariant landed). Test-suite total: 3,496 (was 3,443 before Phase E / F scaffolding; +20 since the 2026-05-18 scaffolding close, of which +15 are the calc-module Phase E extensions for `computePDP`, `computeStandpipeFriction`, `computeDensityAltitude`, `manualJCooling`, and `manualJHeating`).
