...
| 4. Every calculator function has a dimension annotation that parses and balances | **partial** | Phase C scaffolding landed 2026-05-19. [scripts/check-dimensions.mjs](../scripts/check-dimensions.mjs) reads each calc-*.js and [pure-math.js](../pure-math.js) source, walks each export, and parses any `// dims: in { name: <expr>, ... } out: name: <expr>` annotation per spec-v14 §7.1 against the SI base-dimension grammar (L / M / T / I / N / J / `dimensionless`, with `^<int>` exponents and `*` / `/` / whitespace as operators). 15 of 658 functions (2.3%) carry the seed annotation: AWG primitives (`awgDiameterInches`, `awgAreaCmils`, `awgAreaM2`), the electrical chain (`conductorResistance`, `voltageDrop`), the hydraulic primitives (`feetOfHeadToPsi`, `fireHoseFrictionLoss`, `hydrantFlow`), the psychrometric primitives (`saturationVaporPressure_hPa`, `dewPointFromVaporPressure_C`), `colebrookFrictionFactor` (dimensionless in / dimensionless out), and the four temperature converters (`F_to_C`, `C_to_F`, `C_to_K`, `K_to_C`). The lint fails on a malformed annotation today; it warns (single-line summary) on a missing annotation. Per spec-v14 §16.2 Phase C, the warn-on-missing graduates to fail-on-missing once corpus + annotation coverage land in lockstep. Lint wired into `npm run lint`; the `audit:dimensions` script exposes it as a standalone target. |
