...
- **A.2**: [scripts/check-free-access.mjs](../scripts/check-free-access.mjs) probe (`npm run check:free-access`; monthly in [.github/workflows/free-access-probe.yml](../.github/workflows/free-access-probe.yml), which runs it with `--strict` so a genuinely dead link reddens that scheduled run while a bot-walled host does not). It stays out of `npm run lint` and the build, because a publisher's outage must not block a release -- but being opt-in had meant being run by nobody, which is how two dead links survived. Scans citations.js for URLs under 10 tracked publisher hosts, **every `free_access_url` in scripts/sources-cycle.json, and every `free_access` string in data/**, and HEADs each, falling back to GET when a host refuses or drops the HEAD. Warns rather than fails. Three surfaces, because until 2026-09-09 only the first was probed: the ledger's NEC row had been pointing at a 404 (`nfpa.org/free-access`, where the citations all use the working `nfpa.org/freeaccess`), and the loan-limits shard's `free_access` had been sending readers to `fhfa.gov/data/loan-limit-values`, also a hard 404 (FHFA moved it to `/data/conforming-loan-limit`). A redirect that lands on a not-found path is a **soft 404** and warns even though the status is 200 -- that is how the retired AASHTO product-ID URL was reading as healthy. Hosts known to 403 or drop the connection on every automated fetch (codes.iccsafe.org, ssa.gov, beckman.com, iupac.org and publications.iupac.org, each opened in a browser on 2026-09-09 and serving its real page) still warn, but the line says so, so nobody re-diagnoses a bot wall as a broken link. **Status**: pass.
- **A.3**: [scripts/build-citation-strings.mjs](../scripts/build-citation-strings.mjs) parses the per-tile tables in [citation-discipline.md](citation-discipline.md) and emits [citation-strings.generated.json](citation-strings.generated.json) (**70 rows / 70 tiles** as of 2026-08-30). `--check` mode in `npm run lint` rejects out-of-sync edits. The runtime-audit test ([../test/unit/citation-runtime-audit.test.js](../test/unit/citation-runtime-audit.test.js)) holds **70 of 70** markdown rows aligned to the renderer source verbatim (the two long-standing orphans cook-temps / vent-sizing were removed from the discipline doc 2026-05-11). **Status**: pass.
...
- **E.3 a11y parity (Playwright)**: [test/integration/a11y.test.js](../test/integration/a11y.test.js) refactored 2026-05-12 to parse the `TOOLS` array at test-import time (from `app.js` then, from `tools-data.js` after the spec-v10 §H.2 extraction on 2026-06-05); the axe-core loop now parameterizes over every tile_id (plus the home view) rather than the prior 27-route sample. A new tile added to TOOLS is automatically covered without a test-file edit. **Status**: pass.
...
- Citation alignment floor: 70 of 70 markdown rows match the renderer verbatim (the two long-standing orphans cook-temps / vent-sizing were removed from the discipline doc 2026-05-11).
...
| 4. Every calculator function has a dimension annotation that parses and balances | **pass (Phase C campaign complete)** | Phase C scaffolding landed 2026-05-19; the campaign closed 2026-05-21 with calc-plumbing.js, calc-electrical.js, and calc-construction.js graduating to fail-on-missing per spec-v14 §16.2. **658 / 658 (100%)** exported functions across all 25 modules (24 calc-*.js + [pure-math.js](../pure-math.js)) now carry a parseable `// dims: in { ... } out: { ... }` annotation, and [scripts/check-dimensions.mjs](../scripts/check-dimensions.mjs) hard-fails on any future export shipped without one. The historical 2026-05-19 scaffolding text follows: [scripts/check-dimensions.mjs](../scripts/check-dimensions.mjs) reads each calc-*.js and [pure-math.js](../pure-math.js) source, walks each export, and parses any `// dims: in { name: <expr>, ... } out: name: <expr>` annotation per spec-v14 §7.1 against the SI base-dimension grammar (L / M / T / I / N / J / `dimensionless`, with `^<int>` exponents and `*` / `/` / whitespace as operators). 38 of 658 functions (5.8%) carried the seed annotation as of the 2026-05-19 Phase C calc-historical closeout. **[pure-math.js](../pure-math.js) and [calc-historical.js](../calc-historical.js) are both graduated to fail-on-missing**: every function and arrow-const export carries a parseable annotation, the lint reports it as graduated, and a future export without an annotation fails CI (the same ratchet pattern v10 worked-examples followed when its coverage closed). The parser supports two output flavors: single-output (`out: name: <expr>`) and multi-output (`out: { name: <expr>, name: <expr>, ... }`); the extractor terminates the annotation at a blank line or a parenthetical-note line so trailing explanatory comments (e.g., "the output unit is conditional; the annotation is conservative") do not pollute the dimension expression. Annotated functions across the seed: AWG primitives (`awgDiameterInches`, `awgToNumber`, `awgAreaCmils`, `awgAreaM2`), the electrical chain (`conductorResistance`, `conductorResistancePerKft`, `voltageDrop`, `ampacityFromPhysics`, `threePhasePower`, `singlePhasePower`), the hydraulic primitives (`hazenWilliamsFrictionLoss`, `darcyWeisbachFrictionLoss`, `feetOfHeadToPsi`, `fireHoseFrictionLoss`, `hydrantFlow`), the psychrometric primitives (`saturationVaporPressure_hPa`, `dewPointFromVaporPressure_C`, `psychrometric`), the beam-mechanics primitives (`beamUniformLoadSimplySupported`, `beamCenterPointLoadSimplySupported`, `rectangularSection`, `allowableSpanByBending`, `allowableSpanByDeflection`), `colebrookFrictionFactor` (dimensionless), the table-interpolation primitives `interpolateRefrigerant` + `interpLinear` (annotated conservatively per spec-v14 §7.1 because the output unit is selected at call time), the four temperature converters (`F_to_C`, `C_to_F`, `C_to_K`, `K_to_C`), and the calc-module multi-output rows (`computeFireFriction`, `computePDP`, `computeHydrantFlow`, `computeAerialLadderReach`, `computeDensityAltitude`). The calc-*.js modules remain warn-on-missing; each graduates as its annotation coverage closes in lockstep with the per-row corpus annotations per spec-v14 §16.2. The lint fails on a malformed annotation today; it warns (single-line summary) on a missing annotation. Per spec-v14 §16.2 Phase C, the warn-on-missing graduates to fail-on-missing once corpus + annotation coverage land in lockstep. Lint wired into `npm run lint`; the `audit:dimensions` script exposes it as a standalone target. |
| 5. Every calculator function passes the bounds fuzzer at the eight documented input vectors | **pass (Phase D campaign complete)** | Phase D scaffolding landed 2026-05-19; the campaign closed 2026-05-20 with [scripts/check-bounds.mjs](../scripts/check-bounds.mjs) graduated to fail-on-missing per spec-v14 §16.2. **655 / 655 (100%)** corpus functions across all 25 modules now have a bounds-fuzzer row in [test/unit/bounds-fuzzer.test.js](../test/unit/bounds-fuzzer.test.js); each row asserts the spec-v14 §8.2 sensible-result contract (finite, sign-correct, magnitude-banded) and the documented rejection-path sentinel per spec-v14 §8.3. The historical 2026-05-19 scaffolding text follows: Phase D scaffolding landed 2026-05-19 (pure-math primitives) and extended the same day to the named calc-module compute functions. [test/unit/bounds-fuzzer.test.js](../test/unit/bounds-fuzzer.test.js) walks (a) the pure-math primitives (AWG geometric / area, conductor resistance + ampacity, voltage drop, single- and three-phase power, Hazen-Williams + Darcy-Weisbach + Colebrook with full regime ladder coverage Re=1 through Re=1e8, beam mechanics + section properties + allowable spans, psychrometrics across the indoor-comfort sweep, linear interpolation including the empty / single-point / two-point / out-of-range edges, NFA fire-hose friction with table coefficients, hydrant flow, and the temperature converters from absolute zero through high-temperature regimes), and (b) the calc-module rows: Group F `computePDP` over the operational sweep (nozzle pressure 50-200 psi × friction loss 0-200 psi × elevation -50-500 ft), Group F `computeStandpipeFriction` across typical riser geometry (height 10-300 ft × outlets 1-4 × per-outlet flow 100-500 gpm), Group F `computeFireFriction` over documented hose diameters and flows with the unknown-diameter rejection path, Group F `computeHydrantFlow` over operational pitot pressures, Group F `computeRequiredFireFlow` rejecting unknown construction class plus the 12000 gpm ISO ceiling-clamp pin, Group F `computeAerialLadderReach` over 0-90 deg with the `h^2 + v^2 = extension^2` Pythagorean invariant, Group W `computeDensityAltitude` across the full documented PA × OAT sweep plus the four out-of-domain boundary rejections, Group C `manualJCooling` / `manualJHeating` across typical residential envelopes including the insulation-level sweep, and Group C `computeDuctSize` rejecting non-positive inputs plus convergence across typical CFM and friction rates with the `round_diameter_in` / `equivalent_square_in` / `velocity_fpm` output triple pinned finite-positive. **283 tests, all passing.** Each row asserts the spec-v14 §8.2 sensible-result contract (finite, sign-correct, magnitude-banded), the documented sentinel (e.g., `colebrookFrictionFactor` returns 0 for `Re<=0`; `hazenWilliamsFrictionLoss` returns 0 for any non-positive input; `ampacityFromPhysics` returns 0 when `T_c <= T_a`), or the documented thrown error / error-object path (e.g., `awgToNumber` throws on a non-AWG string; `computeFireFriction` returns `{error}` on an unknown hose diameter; `computeDuctSize` returns `{error}` on a non-positive input), per spec-v14 §8.3. The remaining calc-*.js module fuzzers append as each module's per-row corpus domain annotation lands. The dedicated `scripts/check-bounds.mjs` Phase D §8.4 coverage lint landed 2026-05-19: it reads the formula corpus and the bounds-fuzzer test file and reports the per-module fuzzer coverage of corpus functions (current count: **444 / 655 corpus functions covered (67.8%) - passes the 65% milestone** after the 2026-05-19 calc-cross full-module closeout +41 (26 compute pins across the Group G cross-trade utilities -- NIST temperature + unit conversion, material cost / markup / T&M / sales tax / tip-out, loan-payment / upgrade-ROI / mileage-cost / overtime / per-diem, geometry / dilution / slope / haversine, OSHA trench-slope / NIOSH lifting / heat-stress / wind-chill / ladder-angle / pulley-MA / ramp-slope / rainwater / timesheet / vehicle-load / fall-protection / OSHA noise dose -- plus 15 renderer name-mentions for the calc-cross DOM-wiring wrappers) and the prior calc-ems full-module closeout +40 (20 compute pins for `computeGCS`, `computeParkland`, `computeCPSS`, `computeAPGAR`, `computeIvDripRate`, `computeO2CylinderTime`, `computePediatricWeight`, `computeShockIndex`, `computeMAP`, `computeAnionGap`, `computeCorrectedCalcium`, `computeCHA2DS2VASc`, `computeWellsDVT`, `computeWellsPE`, `computePERC`, `computeRuleOf9s`, `computePedsVitals`, `computeNIHSS`, `computeSTART`, `computeDrugConcentration` plus 20 renderer name-mentions for the calc-ems DOM-wiring wrappers - each surfacing the spec-v10 §B.1 limitation banner per the Group V "math aid, NEVER a substitute for the medical-director protocol" posture) and the prior calc-vet full-module closeout +36 (18 compute pins for `computeVetDose`, `computeMaintenanceFluid`, `computeEnergyRequirement`, `computeBCSReference`, `computePetAge`, `computeGestation`, `computeETTSizing`, `computeAnesthesiaVitals`, `computeASAReference`, `computeBloodworkRanges`, `computeUrineSG`, `computeTargetWeightLoss`, `computeToxicity`, `computeBreedPredispositions`, `computeSteadyStateConcentration`, `computeVaccineSchedule`, `computeHeartwormDose`, `computeCrystalloidPlan` plus 18 renderer name-mentions for the calc-vet DOM-wiring wrappers - each surfacing the spec-v10 §B.1 limitation banner per the Group U "math aid, NEVER a substitute for an in-person veterinary exam" posture) and the prior calc-realestate full-module closeout +30 (15 compute pins for `computeLTV`, `computeDTI`, `computePITI`, `compute1031Timeline`, `computeSection121`, `computePropertyTax`, `computeCapRateDSCR`, `computeCashOnCash`, `computeCommissionSplit`, `computeAmortizationSchedule`, `computeCostOfWaiting`, `computeClosingCosts`, `computeRentalWorksheet`, `computeLoanLimits`, `computeHudFmr` plus 15 renderer name-mentions for the calc-realestate DOM-wiring wrappers) and the prior calc-edu full-module closeout +33 (umbrella pins for the 4 text-counter helpers + 13 compute-function rows including Kincaid 1975 FKGL, Flesch 1948 FRE, SMOG / Coleman-Liau / Gunning Fog / ARI alternate-readability identities, A&S 26.2.17 bell-curve CDF, NIST SP 811 sig-figs, base-conversion round-trip, Wald CI proportion + mean, Cramer's-rule 2x2 system, CCSS Appendix A Lexile bands, Marzano + Achieve-the-Core SBG weighted-overall, IUPAC + Pauling periodic-element lookup plus 13 renderer name-mentions for the calc-edu DOM-wiring wrappers) and the prior calc-aviation full-module closeout +29 (11 compute pins for `computePhoneticAlphabet`, `computeFuelPlanning`, `computeWindTriangle`, `computeWeatherPhrasing`, `computeTransponderCodes`, `computeStandardTurn`, `computeSectionalSymbols`, `computeAircraftCategory`, `computeMagneticVariation`, `decodeMetar`, `decodeTaf` plus 18 renderer name-mentions for the calc-aviation DOM-wiring wrappers) and the prior calc-fire full-module closeout +20 (6 compute pins for `computeSmokeReading`, `computeLadderPipeReach`, `computeIsoNeededFireFlow`, `computeScbaCylinderTime`, `computeNFPA1142WaterSupply`, `computeConfinedSpaceVent` plus 14 renderer name-mentions for the calc-fire DOM-wiring wrappers) and the prior +14 (calc-fire seed) / +14 (calc-aviation) / +13 (calc-hvac) / +17 (calc-water) / +12 (calc-kitchen) / +13 (calc-stage) / +15 (calc-trucking) / +17 (calc-mechanic) / +19 (calc-lab) / +13 (calc-legal) / +18 (calc-agriculture) / +6 (calc-historical) / +22 (calc-accounting) / +14 (calc-field) / +16 (calc-references) / +17 (calc-restoration) / +5 (four-module mop-up) Phase D extensions, with **twenty-one modules at 100% (calc-accounting, calc-agriculture, calc-aviation, calc-cross, calc-edu, calc-ems, calc-field, calc-fire, calc-historical, calc-kitchen, calc-lab, calc-legal, calc-mechanic, calc-realestate, calc-references, calc-restoration, calc-stage, calc-trucking, calc-vet, calc-water, pure-math)**, calc-hvac.js at 22% (12 / 54), and calc-plumbing.js at 4%; two large modules (calc-construction 0/62, calc-electrical 0/55) remain untouched and form the next campaign target). The lint runs in measurement mode; once the per-corpus-row fuzzer migration closes per spec-v14 §16.2, the warning graduates to fail-on-missing. |
| 6. Every iterative method passes the numerical-stability tests | **pass (Phase E campaign complete; 2026-05-22 ratchet extends bit-stable pins to closed-form compute fns)** | Phase E scaffolding landed 2026-05-18; Phase E ratchet (bit-pattern pins + computeDuctSize) landed 2026-05-21; Phase E closeout (computeStandpipeFriction + manual-j-worker.js dispatch) landed 2026-05-22; the 2026-05-22 closed-form ratchet adds bit-stable pins to five non-iterative compute functions (voltageDrop Group A pure-math, conductorResistance Group A pure-math, computeBeamLoading Group E, computeAmortizationSchedule Groups R / X, computeParkland Group V) so a future Node / Math drift surfaces as a hex mismatch rather than as a downstream tolerance regression. [test/unit/numerical-stability.test.js](../test/unit/numerical-stability.test.js) covers every spec-v14 §9.1 iterative method: the pure-math primitives (Colebrook iteration, Darcy-Weisbach zero-velocity short-circuit, psychrometric inverse, saturation forward + inverse round-trip, refrigerant P-T linear interpolation, interpLinear NaN-poisoning); Group F `computePDP` (monotonicity in each contributing term + the NFA-shortcut `0.5 psi/ft` exact-identity pin + the bit-stable `4060e00000000000` pin at the canonical NP=100/FL=25/elev=20 input); Group F `computeStandpipeFriction` (the elevation-loop wrapper: monotone-increasing in `riser_height_ft`, the per-outlet sum `friction_total_psi == per_outlet_psi * n`, the Q^2 scaling pin in `gpm_per_outlet`, rejection-path coverage for the four non-positive input combinations plus the unknown hose diameter, determinism, and the four-output bit-pattern pin `elevation_psi=4045b33333333333` / `per_outlet_psi=4019000000000000` / `friction_total_psi=4029000000000000` / `total_psi=404bf33333333333` at h=100/n=2/gpm=250/L=50/2.5 in); Group W `computeDensityAltitude` (ISA standard-day identity, monotonicity in OAT, the FAA worked-example pin `40bcdc0000000000` = 7388 ft at PA=5000/OAT=25, out-of-domain rejection, NaN-poisoning, determinism); Group C `manualJCooling` / `manualJHeating` (monotonicity in opposite directions in outdoor design temperature, the dT>=0 clamp at the indoor-set crossover, the pinned-tons-band invariant against `manualJCoolingExample`, determinism); Group C `computeDuctSize` (nested bisection + Colebrook convergence across the residential CFM sweep, monotone-decreasing in friction target, rejection-path coverage, determinism, and the bit-pattern pin `402c98a4ca5a3063` at 1000 CFM / 0.08 in WC per 100 ft). The `manual-j-worker.js` Web Worker dispatcher carries a stub-self dispatch test exercising the four `kind` branches (cooling / heating / duct / unknown) and the null-payload edge case; each branch cross-checks the posted result at the bit level against a direct call to the underlying calc-hvac compute. **49 passing**; the only Phase E surface not pinned is the iterative-PDP elevation loop that, per the current architecture, evaluates a closed-form addition rather than an iteration. |
| 7. Every shared computation passes the cross-tile invariant tests | **pass (Phase F campaign complete)** | Phase F closed in stages 2026-05-18 through 2026-05-22, with the §10.1 IRS-mileage per-consumer pin closing the last shared-computation row 2026-05-22 (second half) and the §10.3 fifth monotonicity batch extending coverage to four more catalog groups (K Mechanic, J Trucking, R Accounting, N Stage) the same day. §10.1 (shared computations): all five spec-named classes plus two follow-on shared-constant pins now governed by bit-level invariants -- AWG->cmils (Group A primitive consistency), CFM<->m^3/s (Groups C/D/G cross-group crosswalk), NFA hose-friction (Group F `HOSE_FRICTION_COEFFICIENTS` governs every consumer at the bit level), psi<->feet-of-head water (Group B `computeFrictionLoss` + `computeRecircPumpHead` bit-equal to the primitive, Group F `0.434` and Group C `2.31` rounded literals within 0.5% of the primitive), the IRS standard mileage rate (Group R `STANDARD_MILEAGE_RATES` against the bundled shard, Group G `IRS_STANDARD_MILEAGE_RATE` scalar matched to a published business rate, Group G `computeMileageCost` and Group P `computeTimesheet` arithmetic identity pin, plus the Group R `computeMileageRollup` cross-consumer identity), the Hydraulic-Institute 3960 hydraulic-HP constant (Group B `computePumpSize` and Group M `computePumpEfficiency` bit-equal at five operating points with a `(Q*H*SG)/3960` closed-form pin and a Hydraulic Institute canonical-example bit-stable pin at 1e-15), and the water-density 62.4 lb/ft^3 constant (Group D `computeStandingWater` exact, Group B `computeStaticPressureLossPiping` bit-equal to the `feetOfHeadToPsi` primitive across five elevations).§10.3 monotonicity: fifty-seven strict-monotonicity compute-function sweeps across twenty-three catalog groups (the original sixteen plus K Mechanic, J Trucking, R Accounting, N Stage from the 2026-05-22 fifth batch; O Kitchen added by the sixth batch; P Field added by the seventh batch; the eighth batch added three more sweeps in Groups E Construction and M Water; the tenth batch added S Legal plus deepened A / E / X coverage with `computeJudgmentInterest` linear-in-principal, `computeWageHour` piecewise-linear with the FLSA 40-hr OT-threshold pin, `computeMotorFLA` NEC-Table-430 monotonicity, `computeBoardFootage` cubic-volume monotonicity, and `computePropertyTax` linear-in-assessed-value with the mill-rate definition pin at $400k / 25 mills = $10,000); the fifth batch pinned `computeFuelRange` (Group K linear-in-tank + linear-in-mpg with doubling identity), `computeDIM` (Group J cubic monotonicity across length × width × height), `computeStoppingSightDistance` (Group J monotone-increasing in speed with v^2 braking-distance ratio pin), `computeStraightLine` (Group R linear-in-cost), `computeAmortization` (Group R linear-in-principal with doubling identity), and `computeSPL` (Group N inverse-square monotone-decreasing in d2 with per-doubling 6.0206 dB drop pin); the sixth batch added `computeVoltageDrop` (Group A linear-in-length with doubling identity + linear-in-current), `computeBatteryRuntime` (Group A linear-in-Ah with doubling identity + inverse-in-load), `computeBaseboardOutput` (Group C linear-in-length with doubling identity), `computeRecipeScale` (Group O factor = target/original with row-quantity doubling identity), and `computeBellCurve` (Group Y CDF monotone-in-raw_score with percentile-at-mean = 50 symmetry pin to 1e-6); the seventh batch added `computeThreePhase` (Group A sqrt(3)*V*I*pf linear-in-each-input with V_LL doubling pin), `computeFuelPlanning` (Group W linear-in-flight_time + linear-in-burn with burn doubling pin), `computeLTV` (Group X linear-in-loan with PMI-flag threshold pin at LTV 80%/81% + inverse-in-value), `computeCapRateDSCR` (Group X linear-in-NOI + inverse-in-property_value), and `computeBackcountryNeeds` (Group P linear-in-trip_days + linear-in-group_size with group-doubling pin). §10.2 (round-trip identities): complete -- every SI-base conversion pair pinned at 1e-12 absolute (PSI<->kPa, HP<->kW, gallons<->liters, feet<->meters, pound-mass<->kilogram), plus three named-constant exact-bit pins (1 ft = 0.3048 m, 1 lb = 0.45359237 kg, 1 hp = 745.6998715822702 W), and the AWG-mm^2 / F-C-K temperature round-trips. 2026-05-22 closeout: a parameterized round-trip iterates every (category, from, to) pair in calc-cross.UNITS (456 ordered pairs across 13 categories) and asserts the round-trip identity to 1e-9 relative; a typo in any single factor surfaces as a failing pair name rather than as a downstream regression. §10.3 (monotonicity): twenty-eight strict-monotonicity compute-function sweeps across sixteen catalog groups directly (A Electrical, B Plumbing, C HVAC, D Restoration, E Construction, F Fire-direct, G Cross-trade, L Agriculture, M Water, T Lab, U Veterinary, V EMS, W Aviation, X Real Estate) plus the primitive-level monotonicity tests from Groups A and F; the remaining catalog groups (R Accounting, S Legal, H References, J Trucking, K Mechanic, N Stage, O Kitchen, P Field, Q Historical) are mostly categorical / lookup tiles outside the §10.3 contract. **69 passing**. The historical scaffolding prose follows: Phase F scaffolding landed 2026-05-18: [test/unit/cross-tile-invariants.test.js](../test/unit/cross-tile-invariants.test.js) covers the Group A AWG primitives (determinism, cmils<->m^2 consistency, monotonicity in AWG number), Group A voltage-drop and conductor-resistance monotonicity, Group F NFA hose-friction Q^2 / L scaling, Group B/C/L psi<->feet-of-head round-trip, F<->C<->K temperature round-trips, the AWG-diameter pinned-formula invariant, and (added 2026-05-18) the Group C/D/G CFM<->L/s<->m^3/s cross-group invariant: the Group G crosswalk's CFM->L/s factor matches the NIST-derived value at 1e-12 relative, the crosswalk agrees with the calc-hvac inline m^3/s constant to the 7-figure truncation floor, CFM<->L/s round-trips through [calc-cross.js](../calc-cross.js) `convertUnit` to 1e-12 relative, and the flow-category base unit is pinned to L/s. **69 passing** after the 2026-05-22 Phase F §10.3 fourth monotonicity batch added seven more strict-monotonicity tests across calc-water (AWWA pounds formula), calc-restoration (IICRC S500 air movers), calc-agriculture (boom-spray GPA + drawbar power), and calc-fire (direct consumer pin): (22) `computePoundsFormula` ↑ in flow_mgd, (23) ↑ in dose_mg_l, (24) `computeAirMovers` count monotonic-non-decreasing in area (IICRC S500 step function), (25) `computeGPA` ↑ in gpm, (26) ↓ in speed_mph (inverse-proportional), (27) `computeDrawbarPower` drawbar_hp ↑ in pull_lb (ASABE D497 linear pin), (28) `computeFireFriction` friction_loss_psi ↑ in gpm at the consumer level (cross-check vs the primitive). Combined coverage now spans sixteen catalog groups directly (A, B, C, D, E, F, G, L, M, T, U, V, W, X, and the cross-tile primitives in F and Y). Earlier the same day the Phase F §10.3 third monotonicity batch added seven more strict-monotonicity tests across calc-construction (ASCE 7 wind / snow), calc-cross (wind chill, loan payment), and calc-hvac (Manual J cooling): (15) `computeWindPressure` `q_psf` increasing in V (V^2 dynamic-pressure pin), (16) V^2 ratio pin (q(200)/q(100) == 4), (17) `computeSnowLoad` `Pf_psf` increasing in `Pg_psf` (linear), (18) Pf = 0.7 * Pg closed-form pin at unit factors, (19) `computeWindChill` strictly decreasing in wind speed (NWS 2001 piecewise-power-law pin), (20) `computeLoanPayment` monthly_payment increasing in APR (cross-check vs computePITI), (21) `manualJCooling` tons increasing in outdoor_design_F at fixed envelope. Combined coverage now spans twelve catalog groups (A Electrical, B Plumbing, C HVAC, E Construction, G Cross-trade, T Lab, U Veterinary, V EMS, W Aviation, X Real Estate, and the G/X overlap via loan payment). Earlier the same day the Phase F §10.3 second monotonicity batch added seven more strict-monotonicity tests across calc-ems / calc-vet / calc-lab / calc-electrical (Groups V / U / T / A): (8) `computeParkland` total fluid increasing in `weight_kg`, (9) increasing in TBSA, (10) `computeEnergyRequirement` RER increasing in weight (W^0.75 Kleiber pin), (11) `computeBeerLambert` concentration increasing in absorbance, (12) decreasing in epsilon (inverse-proportional pin), (13) `computeOhmsLaw` I = V/R increasing in V at fixed R, (14) `computeOhmsLaw` P = I^2 R increasing in I at fixed R. Combined with the first batch, the §10.3 sweep covers eight catalog groups (A Electrical, B Plumbing, E Construction, T Lab, U Veterinary, V EMS, W Aviation, X Real Estate) at the compute-function level. Earlier the same day the Phase F §10.3 monotonicity sweep landed seven new strict-monotonicity tests across five compute functions and four catalog groups: (1) `computeFrictionLoss` `pressureLoss_psi` strictly increasing in `flow_gpm` (Hazen-Williams Q^1.852 scaling), (2) `computeFrictionLoss` strictly decreasing in pipe size at fixed flow + length, (3) `computeBeamLoading` deflection strictly increasing in uniform load (linear-in-w pin per 5wL^4/384EI), (4) `computeBeamLoading` deflection strictly increasing in span (L^4 exponent pin), (5) `computeDensityAltitude` strictly increasing in OAT (120 ft/C coefficient pin), (6) `computePITI` strictly increasing in principal at fixed rate/term, and (7) `computePITI` strictly increasing in APR at fixed principal/term. A non-monotonic result for any of these would be a transcription error (sign flip, exponent swap, missing unit conversion) per spec-v14 §10.3 and surface immediately at CI. Earlier the same day the Phase F §10.1 psi<->feet-of-head water closeout landed four new tests pinning the fourth of five §10.1 shared-computation classes: (i) calc-plumbing `computeFrictionLoss` produces `pressureLoss_psi` bit-equal to `feetOfHeadToPsi(headLoss_ft)` across a 27-point pipe-size / flow / length sweep at the Hazen-Williams branch (so a future inlined `rho/144` factor surfaces immediately), (ii) calc-plumbing `computeRecircPumpHead` `pressure_psi` is bit-equal to the primitive at the spec example, (iii) calc-fire's hardcoded `0.434 psi/ft` (the rounded NFPA water-column shortcut in `computeStandpipeFriction`) stays within 0.5% of the primitive `62.4/144 = 0.43333...` so a future edit that swapped `0.434` for `0.5` (the NFA fireground rule-of-thumb that lives in `computePDP`) -- a 15% inflation -- surfaces at CI, and (iv) calc-hvac's hardcoded `2.31 ft/psi` inverse stays within 0.5% of `144/62.4 = 2.3077`. Earlier on the same day the Phase F §10.1 NFA hose-friction coefficient closeout landed five new tests asserting that the shared `HOSE_FRICTION_COEFFICIENTS` table from [calc-fire.js](../calc-fire.js) governs every Group F consumer at the bit level: (a) `computeFireFriction` produces the same `friction_loss_psi` as a direct `fireHoseFrictionLoss` primitive call at every diameter (1.75 / 2.5 / 3 / 4 / 5 in), (b) `computeReverseLayFriction` at `n_pumps=1` equals `computeFireFriction` (the n=1 base-case identity) and surfaces the same coefficient field, (c) `computeStandpipeFriction` per-outlet equals `computeFireFriction` at a matched (gpm, length, hose) input, (d) all three consumers reject the same unknown-diameter sentinel with `{ error: "Unknown hose diameter." }` so a future refactor that inlined a default coefficient anywhere would surface immediately, and (e) every entry in `HOSE_FRICTION_COEFFICIENTS` falls inside the NFA-table sanity band (positive, finite, < 200). This closes the Phase F §10.1 shared-computation row for the NFA hose-friction coefficient (the third of the five spec-v14 §10.1 classes), matching the bit-level discipline already in place for AWG->cmils (Group A) and CFM<->m^3/s (Groups C/D/G). The earlier 2026-05-19 IRS-mileage cross-source invariant landed: four new tests pin the `STANDARD_MILEAGE_RATES` JS const in [calc-accounting.js](../calc-accounting.js) against the bundled `data/accounting/standard-mileage-rates.json` data shard at every published year and rate kind to 1e-4 absolute (well below the 0.1 cent IRS publication precision), assert the JS const covers every shard year (bijection in shard->JS direction; the JS const may stay one year ahead during a mid-year update), assert every rate is in the (0, 1.50) USD/mile sanity band per spec-v14 §8.2, and pin the charitable rate to the 26 USC 170(i) statutory 0.14 USD/mile floor so a legislative change cannot slip past CI without a deliberate fixture update. |
| 8. Every active group A through Y has a reviewer signoff row in [docs/audit-trail.md](audit-trail.md) with a date within the prior quarter and a next-renewal date | **partial (structured state machine scaffolded; reviewer signoffs pending)** | Phase H scaffolding landed 2026-05-19; the structured per-group signoff status table landed 2026-05-22 ([docs/audit-trail.md](audit-trail.md) §"Per-group signoff status (v0.14)"). The new table records one row per active catalog group with columns Group / Letter / Credential / Status / Last review / Next renewal / Notes, replacing the prior prose-only solicitations text. Status values: **open** (no reviewer engaged), **under-review** (reviewer engaged; signoff pending), **signed-off**, **renewal-due**, **exempt** (H References and Q Historical per spec-v14 §12.1). Counts at table close (2026-05-22): 24 active groups (22 non-exempt; 2 exempt H / Q); 0 signed-off; 22 open. [scripts/check-audit-trail.mjs](../scripts/check-audit-trail.mjs) reads the file and asserts every active non-exempt group is named at least once -- either standalone (`Group G ...`), parenthesized (`Group G (Cross-trade)`), in the compact comma-list form (`Group A Electrical, B Plumbing, C HVAC, ...`), or in the new structured table. The 2026-05-22 lint extension also parses the structured table itself and reports per-status counts in measurement mode: `per-group signoff status: 24 row(s) in table; open=22, under-review=0, signed-off=0, renewal-due=0, exempt=2`. A missing group row in the table is reported as a warning (does not fail at scaffolding); the warn graduates to error when the first row flips to **under-review** or **signed-off** (so the table cannot lose a row once it starts being maintained). The lint fails on a missing group letter (regression backstop). The "warn if no completed review within 100 days" upgrade per spec-v14 §18.3 lands when the first row flips to signed-off; per spec-v14 §12 the gate flips to pass when every non-exempt row reaches signed-off. Lint wired into `npm run lint`; standalone target `npm run audit:audit-trail`. |
...
| 10. [docs/derivations.md](derivations.md) has one row per tile in TOOLS; the build asserts coverage | **pass (Phase I §13.1 closeout)** | Phase I §13.1 scaffolding landed 2026-05-19; the per-tile derivation-index closeout landed 2026-05-22. [scripts/build-tile-index.mjs](../scripts/build-tile-index.mjs) emits a deterministic per-tile appendix in [docs/derivations.md](derivations.md) (one subsection per group A-Y, one table row per tile carrying tile_id + display name + citation source + fixture pointer) delimited by `<!-- BEGIN tile-index-v14 -->` / `<!-- END tile-index-v14 -->`; `--check` mode is wired into `npm run lint`. [scripts/check-derivation-coverage.mjs](../scripts/check-derivation-coverage.mjs) graduated to fail-on-missing the same day per spec-v14 §16.2: a TOOLS tile without a derivations.md mention fails CI with the tile_id + group letter named, plus a pointer to `npm run audit:tile-index`. **385 / 385 tiles mentioned (100%)** across all 24 active groups (per-group: A 33/33, B 28/28, C 33/33, D 15/15, E 41/41, F 20/20, G 27/27, H 15/15, J 8/8, K 8/8, L 9/9, M 9/9, N 7/7, O 6/6, P 8/8, Q 1/1, R 12/12, S 9/9, T 10/10, U 18/18, V 20/20, W 18/18, X 15/15, Y 15/15). Standalone target `npm run audit:derivation-coverage` (`--verbose` for the per-tile mention status); regenerator at `npm run audit:tile-index`. |
...
- **New tests (Phase E + Phase F scaffolding + follow-ups + calc-module extensions + Phase D scaffolding + Phase D calc-module extensions)**: [test/unit/numerical-stability.test.js](../test/unit/numerical-stability.test.js) (33 tests), [test/unit/cross-tile-invariants.test.js](../test/unit/cross-tile-invariants.test.js) (20 tests), and (2026-05-19) [test/unit/bounds-fuzzer.test.js](../test/unit/bounds-fuzzer.test.js) (283 tests, of which 30 are the pure-math scaffolding, 28 are the calc-fire Phase D extension (14 initial + 14 same-day expansion), 14 are the calc-aviation Phase D expansion (`computePressureAltitude`, `computeCrosswind`, `computeETE`, `computeHypoxiaAltitude`, `computeTopOfDescent`, `computeTrueAirspeed` with identity pins for ISA pressure-altitude, Pythagorean HW/CW, 3-to-1 descent, and TAS = CAS / sqrt(rho_ratio)), 13 are the calc-hvac Phase D expansion (`computeSHR`, `computeSeerEer`, `computeBalancePoint`, `computeCfmPerTon`, `computeCombustionAir`, `computeApproachDeltaT`, `computeEvaporativeCooling`, `computeAffinityLaws`, `computeOutdoorAirMix` with identity pins for fan-affinity Q~N / SP~N^2 / kW~N^3, SEER = 1.12 * EER round-trip, per-climate CFM-per-ton factors, evaporative-cooling Q = m * hfg, and the outdoor-air mass-balance temperature mix), and 17 are the calc-water Phase D expansion (`computePoundsFormula`, `computeFilterLoading`, `computeDetentionTime`, `computeDilution`, `computePumpEfficiency`, `computeCoagulantDose`, `computeSVI`, `computeDisinfectionCT` with identity pins for the AWWA pounds-formula MGD * mg/L * 8.34, C1V1 = C2V2 across every missing-variable case, the serial-dilution per-step divisor chain, water horsepower = (gpm * tdh) / 3960, SVI = SV30 * 1000 / MLSS, and the SWTR `CT_achieved = chlorine * t10` with the 3-log Giardia pass / fail flip at the spec-v9 §E.2 worked-example node), on top of the 3 calc-hvac Phase D scaffolding rows for Manual J + duct sizing. All passing. Test-suite total: 3,602.
- **npm scripts added**: `audit:corpus` (`build-corpus.mjs --check`), `corpus:build` (`build-corpus.mjs`, regenerates the section), (2026-05-19) `audit:cross-validation` (`check-cross-validation.mjs`), (2026-05-19) `audit:dimensions` (`check-dimensions.mjs`), (2026-05-19) `audit:citation-coverage` (`check-citation-coverage.mjs --verbose`), (2026-05-19) `audit:audit-trail` (`check-audit-trail.mjs`), (2026-05-19) `audit:derivation-coverage` (`check-derivation-coverage.mjs --verbose`), and (2026-05-19) `audit:bounds` (`check-bounds.mjs --verbose`).
...
| Figure | Live |
| --- | --- |
| Calculators (tiles) in `TOOLS` | **2082 live tiles** |
| Active catalog groups | **21 live groups** |
| `calc-*.js` modules | **77 live calc modules** |
| Sitemap URLs (one per tile, one per group hub, home, catalog hub) | **2105 live sitemap URLs** |

### Gates

| Figure | Live |
| --- | --- |
| Static gates in the `npm run lint` chain | **59 live lint gates** |
| Unit-test suite files under `test/unit/` | **166 live unit suites** |
