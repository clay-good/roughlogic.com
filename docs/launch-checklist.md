...
- **A.2**: [scripts/check-free-access.mjs](../scripts/check-free-access.mjs) opt-in probe (`npm run check:free-access`). Scans citations.js for URLs under 10 tracked publisher hosts **and every `free_access_url` in scripts/sources-cycle.json**, and HEADs each, falling back to GET when a host refuses or drops the HEAD. Warns rather than fails. Two surfaces, because until 2026-09-09 only the reader-facing one was probed and the ledger's NEC row had been pointing at a 404 (`nfpa.org/free-access`, where the citations all use the working `nfpa.org/freeaccess`). A redirect that lands on a not-found path is a **soft 404** and warns even though the status is 200 -- that is how the retired AASHTO product-ID URL was reading as healthy. Hosts known to 403 every automated fetch (codes.iccsafe.org) still warn, but the line says so, so nobody re-diagnoses a bot wall as a broken link. **Status**: pass.
