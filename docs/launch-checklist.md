...
Five v14 campaigns are complete as of 2026-05-22: Phase C (dimensional-analysis annotation, 658/658), Phase D (bounds-fuzzer coverage, 655/655), Phase E (numerical-stability pins on every spec-v14 §9.1 iterative method), Phase F §10.2 round-trip identities (every SI-base conversion pair pinned at 1e-12 absolute plus three named-constant exact-bit pins for the international foot, the 1959 international pound, and the mechanical horsepower), and Phase I §13.1 (per-tile derivation-index, 385/385 with the lint graduated to fail-on-missing). Phase G citation-to-formula round-trip scaffolding (the [scripts/check-citation-coverage.mjs](../scripts/check-citation-coverage.mjs) inverse-map lint) landed 2026-05-19 and stays in measurement mode pending the per-group reviewer pass; the 2026-05-22 source-pattern expansion (+17 new patterns: OSHA, NIOSH, AWWA, EPA, USDA / USDA NRCS, HUD, FNMA / Fannie Mae, FHFA, CFPB, FRCP, AAFP, ACEP, ACVIM, AVMA, AAEP, AVECCT, NFPA 14 / NFPA 1981 / NFPA 70) moves tracked-edge coverage from 117 -> 201 tile-source edges across 29 -> 46 tracked sources, surfacing the OSHA (16 tiles), AWWA (10), USDA (10), EPA (7), AAFP (4), NIOSH (3) sub-catalogs that were previously aggregated under "other / untracked" in the inverse-map output.

Phase H (per-group reviewer signoff) lands incrementally per the open-solicitations table in [docs/audit-trail.md](audit-trail.md); Phase B's per-row independent-published-source upgrade (gates 2 + 3 at "all rows verified against an independent published worked example") rides the same review pass. Phase F's remaining work is §10.1 shared-computation invariants beyond the four pure-function primitives already pinned (the IRS-mileage invariant is asserted today by [test/unit/cross-tile-invariants.test.js](../test/unit/cross-tile-invariants.test.js) at the JS-const vs JSON-shard pin) and §10.3 monotonicity sweeps for any compute function whose documented relationship is monotonic and not yet covered. The phase order is fixed per spec-v14 §16.1; the per-phase artifact list is in [docs/correctness.md](correctness.md).
