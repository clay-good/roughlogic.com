...
| 8. Every active group A through Y has a reviewer signoff row in [docs/audit-trail.md](audit-trail.md) with a date within the prior quarter and a next-renewal date | **partial** | Phase H scaffolding landed 2026-05-19. [scripts/check-audit-trail.mjs](../scripts/check-audit-trail.mjs) reads [docs/audit-trail.md](audit-trail.md) and asserts every active non-exempt group (A through Y in TOOLS, minus the I-letter skip in the catalog, minus H References and Q Historical exempt per spec-v14 §12.1) is named at least once - either standalone (`Group G ...`), parenthesized (`Group G (Cross-trade)`), or in the compact comma-list form the v0.14 credential-sought block uses (`Group A Electrical, B Plumbing, C HVAC, ...`). Counts at scaffolding close: **24 active groups; 22 non-exempt; all 24 named; 1 completed-review section** (the v0.11 / v0.12 §15 gate-10 seed for Group U and Group V). The lint fails on a missing group letter (regression backstop). The "warn if no completed review within 100 days" upgrade per spec-v14 §18.3 lands when the per-group review pass closes. Lint wired into `npm run lint`; standalone target `npm run audit:audit-trail`. |
