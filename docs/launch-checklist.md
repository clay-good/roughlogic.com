...
| 9. The SPA emits `<link rel="canonical">` to the shell URL when a tile is open | pass | The hash-route handler in [../app.js](../app.js) sets `<link rel="canonical" href="/tools/<id>/">` on tile open and reverts to `/` on return to home. The shell URL is the canonical surface; the hash form is the operational URL for an interactive user. |
