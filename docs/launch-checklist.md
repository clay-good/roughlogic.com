...
| 10. [docs/derivations.md](derivations.md) has one row per tile in TOOLS; the build asserts coverage | **partial** | Phase I §13.1 scaffolding landed 2026-05-19. [scripts/check-derivation-coverage.mjs](../scripts/check-derivation-coverage.mjs) reads the TOOLS array from [app.js](../app.js), searches [docs/derivations.md](derivations.md) for each tile_id, and reports per-tile + per-group coverage. Counts at scaffolding close: **25 / 385 tile(s) mentioned (6.5%)**; current document organizes derivations by formula family (one section per formula covering a tile family), so the per-tile mention rate is well below 100 percent at this measurement step. Per-group coverage today: Group A 6%, B 7%, C 9%, D 13%, E 7%, F 5%, G 15%, J/K/L/M/N/O/U/V/X 0%, P 13%, R 17%, S 11%, T 10%, W 11%, Y 7%. The lint runs in measurement mode at scaffolding close; once the migration to one row per tile lands (an incremental per-group authoring pass), the lint graduates to fail-on-missing per the spec-v14 §16.2 ratchet (mirroring the v10 worked-examples graduation pattern). Lint wired into `npm run lint`; standalone target `npm run audit:derivation-coverage` (`--verbose` for the per-tile mention status). |
