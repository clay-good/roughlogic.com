...
| 6. Every iterative method passes the numerical-stability tests | **partial** | Phase E scaffolding landed 2026-05-18: [test/unit/numerical-stability.test.js](../test/unit/numerical-stability.test.js) covers the pure-math iterative / transcendental primitives (Colebrook, Darcy-Weisbach zero-velocity short-circuit, psychrometric inverse, saturation round-trip, refrigerant P-T interpolation, NaN-poisoning behavior, per-call determinism). **18 passing, 0 skipped** after the 2026-05-18 §9.1 hardening: [pure-math.js](../pure-math.js) `interpLinear` now propagates NaN explicitly via a function-head guard rather than falling through to the upper-tail branch, and the previously-skipped Phase E follow-up test now asserts the guard against single-point, two-point, and multi-point tables. The calc-specific iteratives (Group F pump-discharge elevation loop in [calc-fire.js](../calc-fire.js); Group W density-altitude inversion in [calc-aviation.js](../calc-aviation.js); the simplified Manual J zone loop in [manual-j-worker.js](../manual-j-worker.js)) append as each method's per-row corpus annotation stabilizes. |
...
- **New tests (Phase E + Phase F scaffolding + follow-ups)**: [test/unit/numerical-stability.test.js](../test/unit/numerical-stability.test.js) (18 tests, all passing after the §9.1 NaN-guard hardening) and [test/unit/cross-tile-invariants.test.js](../test/unit/cross-tile-invariants.test.js) (20 tests, all passing after the CFM<->m^3/s cross-group invariant landed). Test-suite total: 3,481 (was 3,443 before Phase E / F scaffolding; +5 since the 2026-05-18 scaffolding close from the two follow-ups).
...
Phases B (cross-validation tests + lint), C (dimensional-analysis lint), D (bounds fuzzer), G (citation-to-formula round-trip), and H (per-group reviewer signoff) land incrementally. Phase E (numerical-stability) and Phase F (cross-tile invariants) carry scaffolding for the pure-math primitives plus the 2026-05-18 follow-ups (§9.1 `interpLinear` NaN-guard; CFM<->m^3/s cross-group invariant); the per-calc-module extensions (Group F pump-discharge loop, Group W density-altitude inversion, simplified Manual J worker, IRS-mileage cross-group invariant) append as each phase's per-row corpus annotation stabilizes. The phase order is fixed per spec-v14 §16.1; the per-phase artifact list is in [docs/correctness.md](correctness.md).
