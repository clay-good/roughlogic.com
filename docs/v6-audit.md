...
### Group X - Real Estate (calc-realestate.js) - priority 19 (v12)

Status: **complete** - citations.js populated for all fifteen Group X tiles (X.1-X.15 per spec-v12 §8). Cites FNMA Single-Family Selling Guide + FHA Handbook 4000.1 + VA Lenders Handbook M26-7 (PITI, DTI, LTV, conforming + FHA + VA loan limits); standard mortgage amortization (loan-payment, amortization-schedule, cost of waiting); 26 USC §1031 + Treas. Reg. 1.1031(k)-1 (1031 timeline); 26 USC §121 (home-sale exclusion); FHFA Conforming Loan Limit Values (annual) + HUD FHA Single-Family Mortgage Limits + Blue Water Navy Vietnam Veterans Act of 2019 (X.8 loan-limits); HUD PD&R Fair Market Rents FY2026 + 24 CFR Part 888 (X.10 hud-fmr); CFPB closing-disclosure form + state recording-fee schedules (closing costs); IRS Schedule E (Form 1040) + 26 USC §469 passive-loss rules (rental worksheet); standard CRE underwriting ratios (cap rate, DSCR, cash-on-cash, commission split). Every tile cites `GOVERNANCE.real_estate` ("lender governs final underwriting; appraiser governs final value").

### Group Y - Educators / K-12 (calc-edu.js) - priority 20 (v12)

Status: **complete** - citations.js populated for all fifteen Group Y tiles (Y.1-Y.15 per spec-v12 §9). Cites Kincaid et al., Naval Technical Training Command Research Branch Report 8-75 (1975) + Flesch, J Appl Psychol 1948 (Flesch-Kincaid readability); McLaughlin 1969 (SMOG) + Coleman-Liau 1975 + Gunning Fog 1952 + Smith-Senter 1967 (alternate readability); state DOE Lexile-band bulletins (Lexile is a registered trademark of MetaMetrics; grade-band targets are public from state DOE); standard US 4.0 / 5.0 GPA scale; standard descriptive statistics (mean / median / SD); Wald + Clopper-Pearson confidence intervals; standard quadratic / 2x2 linear / sig-figs / scientific-notation / base-converter / standards-based-grade / bell-curve identities; IUPAC Standard Atomic Weights 2021 + electron configuration tables (periodic element extension); standard genetic codon table (codon reference). Every tile cites `GOVERNANCE.education` ("teacher / school registrar / district administrator govern").

### v12 expansion close (2026-05-16)

Tile counts at v12 audit close: 385 spec-numbered utilities + visible tiles (was 271 after v5). 86 new visible tiles across Groups U / V / W / X / Y (18 + 20 + 18 + 15 + 15). Every new tile has a `CITATIONS` entry; every entry covers the six §3 fields; every entry uses one of the GOVERNANCE variants (the eighteen pre-v12 variants plus three new v12 variants: `veterinary`, `ems_prehospital`, `aviation` was added in v9, `real_estate`, and `education`). The `test/unit/citations.test.js` "v6 audit complete" coverage check fails the build if any future tile is added without a structured citation, so v12 expansion held the v6 audit invariant throughout. The spec-v12 §13.1 profession-overrides extension is documented in [profession-overrides.md](profession-overrides.md); every Group U / V tile carries the v10 §B.1 limitation banner with the canonical professional-governs replacement copy per the override.
