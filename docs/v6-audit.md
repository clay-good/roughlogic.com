...
5. Run `npm run data:refresh` to regenerate shards if needed, then `npm run audit` (six stages: lint -> test -> build -> check:dist -> check:shells -> data:verify per spec-v12 §G.3 + spec-v13 Phase G). Every stage must pass.
