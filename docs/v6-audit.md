...
Status: **in progress** — citations.js populated as the worked example for the v6 platform; per-tile §3 reference block content authored. Awaiting reference-block UI component to be wired through the renderer.
...
Status: **in progress** — citations.js populated for every Group B tile; per-tile §3 reference block content authored. Awaiting reference-block UI component to be wired through the renderer (shared with Group A; ships now via the dynamic citations.js import in `app.js`).
