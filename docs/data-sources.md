...
`check-verified-on-ledger` prints this set on every run and ratchets it, so **any** unbacked stamp now fails the build. **No shard outside the ledger carries an unbacked `verified_on` any more**: `data/accounting/inventory-benchmarks.json`, the last one, was closed on 2026-09-09 against the Census ARTS 2022 benchmarked tables, and nothing in it survived -- the year, the name of the statistic, the values, and two rows' publishers were all wrong. `data/lab/iupac-atomic-weights.json` was closed that way on 2026-09-09 against ciaaw.org, which also caught a stale zirconium value, `data/lab/centrifuge-rotors.json` the same day against the three manufacturers' own published speed and RCF figures, which caught three radii belonging to a different rotor or a different bucket, and `data/lab/buffer-pka.json` the same day, which caught two pKa values that were 20 °C numbers in a table labelled 25 °C. `data/cross/glossary.json` used to be counted with them and is not: its source is the project author's own definitions, so there is no publisher to check against and no ledger row can exist. It declares `provenance: "author-original"`, the gate reports it separately, and its `verified_on` means *the author last reviewed this prose* -- held as an explicit constant in `build-data.mjs` so the build clock cannot certify a review nobody performed. The stamp stays rather than being deleted because `data/cross` holds only that shard, and `check-manifests` measures a folder's annual cadence from the stamps its shards carry -- removing it would have traded a false stamp for no review check at all. Adding a ledger row for each is open maintainer work, and the way to close one is to read the source and record what was read -- not to re-stamp. Their stamps no longer move on every build, though: `build-data.mjs` compares each regenerated shard against the copy already on disk and, when the content is identical, **carries the committed `verified_on` forward** instead of writing today's date. A stamp the clock rewrites on every run can never age into a recheck, which is the whole point of the field. Only `verified_on` carries -- `fetched`, `built` and `derived_at` say when the generator ran, and on the historical series that is load-bearing, because `calc-historical` measures how far a series has fallen behind its own `fetched` date. Content that genuinely changed still earns today's date. `macrs-tables.json` and `estimated-tax-due-dates.json` were closed that way on 2026-09-03 against `irs.gov/pub/irs-pdf/p946.pdf` and `f1040es.pdf`; both PDFs fetch cleanly, so that check is repeatable in minutes.
...
- Source: U.S. government publications. BLS Producer Price Index series for industrial commodities (copper WPU102502, aluminum WPU102501, structural steel WPU101704, rebar WPU1074051, framing lumber WPU081, OSB WPU083, drywall WPU137, asphalt WPU058102). EIA retail series for diesel (PET.EMD_EPD2D_PTE_NUS_DPG.M), gasoline (PET.EMM_EPMR_PTE_NUS_DPG.M), and natural gas city-gate (NG.N3050US3.M). USDA NASS / FRED series for wheat (WHEAT - PRICE RECEIVED, MEASURED IN $ / BU), corn (CORN, GRAIN - PRICE RECEIVED, MEASURED IN $ / BU), and soybeans (SOYBEANS - PRICE RECEIVED, MEASURED IN $ / BU). Series IDs are reproduced verbatim; the prose / methodology of the issuing publication is not.
...
- Source: IRS publications (Pub 946 MACRS tables, Pub 15-T percentage method, annual Rev. Proc. for the Section 179 cap, Form 1040-ES due dates, annual standard-mileage-rate notice). SSA annual wage-base announcement. U.S. Census Annual Retail Trade Survey (ARTS) 2022 benchmarked tables for the inventory-turnover benchmarks. These are industry **aggregates**, not medians across firms, and ARTS has no later data year: the survey folded into the Annual Integrated Economic Survey, which began collecting in March 2024.
...
- Shards: `macrs-tables.json` (per-class-life percentages), `section-179-limits.json` (per-year cap / phase-out / bonus pct; 2025 onward follows the One Big Beautiful Bill Act -- $2,500,000 / $4,000,000 for 2025 and $2,560,000 / $4,090,000 for 2026, with bonus a permanent 100% for property acquired after 2025-01-19 -- not the TCJA phase-down), `se-tax-parameters.json` (per-year SS wage base + Additional Medicare threshold by filing status; the 2026 base is $184,500 per IRS Topic 751 and the SSA COLA announcement), `estimated-tax-due-dates.json` (per-year four ISO dates), `standard-mileage-rates.json` (per-year business / medical / charitable rates; a year the IRS revises mid-year carries a `periods` list, as 2026 does), `inventory-benchmarks.json` (per-industry turnover aggregate: COGS over average end-of-year inventory, computed from the ARTS 2022 gross-margin, purchases and inventories tables -- 7.0 retail overall, 12.9 grocery, 3.0 apparel, 2.7 auto parts), `pub-15-t-tables.json` (single-filer annualized brackets).
- `pub-15-t-tables.json` carries `edition: 2025` and `verified_on: 2026-09-19`, the date its brackets were read against the IRS's 2025 Pub 15-T PDF (Worksheet 1A standard schedule, single, with the $8,600 line 1g folded in). Before that the 2025-labeled shard held the 2024 schedule ($14,600 floor), which a stamp of 2025-12-01 had never checked. The runtime keys 2024 and 2025 by `tax_year`; single filer only.
...
- **Re-checked 2026-09-04 (24 more rows, 38 of 47 total).** Rhode Island and Nevada were both read against the state's own published text and both thresholds are **correct as bundled** -- $100,000 or 200 transactions, disjunctive. R.I. Gen. Laws 44-18.2-3 states it in the 'remote seller' definition, and the citation is tightened to that subsection. Nevada's citation was **wrong**: it named `Nev. Admin. Code 372.030`, which is titled *"Retail sale" defined* and carries no threshold at all. The test actually comes from the Nevada Tax Commission's approved regulation LCB File No. R189-18 (effective 2018-09-27), whose section 2 paragraph (c) reads "greater than $100,000; or ... 200 or more retail sales" -- read from the enrolled regulation PDF, where new matter is italicised and deleted matter bracketed, so the operative text is unambiguous.

  Maryland and New Jersey followed, both **correct as bundled**. Maryland's citation was wrong in the same way Nevada's was: it named `Md. Code Ann., Tax-Gen. 11-701(b)(2)(iii)`, which is about entering the State to service or repair property. The test is in COMAR 03.06.01.33 B(5)(a)-(b) -- gross revenue over $100,000 **or** 200 or more separate transactions, "either of the following criteria". New Jersey's citation was right; what it lacked was currency, which is why a previous pass declined to stamp it. The Division of Taxation's remote-seller FAQ, last updated 2025-04-24, states the same two tests in the same disjunctive form, so the section is confirmed as the agency applies it today rather than only as enacted.

  California and Texas, the two largest bundled markets and the only two at $500,000, are both **correct** and both **sales-only with no transaction test**, which is the part most easily mis-copied from a chart. CDTFA's Wayfair page states "total combined sales of tangible personal property for delivery in California ... exceed $500,000" under Rev. & Tax. Code 6203, effective 2019-04-01; the Texas Comptroller's Publication 94-108 states "total Texas revenue greater than $500,000 in the preceding 12 calendar months". Both citations were already right and are extended with the source actually read.

  Alabama and Mississippi, the only two at $250,000, are both **correct** and both **sales-only**. Alabama's is the Simplified Sellers Use Tax programme rather than a conventional nexus statute (Ala. Admin. Code r. 810-6-2-.90.03/.90.04, Act 2018-539, effective 2019-01-01). Mississippi's differs from every other bundled row in a way the tile does not model: the Department of Revenue measures "$250,000 over **any twelve month period**", not a calendar year, so a seller can cross it mid-year on a trailing window. That is now in the citation. Mississippi's old citation named Notice 72-19-001, which is no longer findable on the DOR site; the code section stays and the FAQ actually read replaces the notice number rather than being left to look verified.

  Florida and Washington are both **correct on the number and wrong on the pointer**, in two different ways. Florida's figure is right but narrower than the row can express: Fla. Stat. 212.0596(1)(b) counts **taxable** remote sales over the **previous** calendar year only, where most states count gross sales over the previous *or current* year. Washington's citation, `Wash. Rev. Code 82.08.052`, governed a window that closed on 2019-12-31 -- from 2020 the test is RCW 82.04.067(1)(c)(i), "more than one hundred thousand dollars of cumulative gross receipts from this state" in the current or immediately preceding calendar year. Same $100,000, six years of pointing at a superseded provision.

  South Dakota and Indiana were taken specifically because both repealed a transaction prong -- the Washington finding suggested a repeal is where a pointer goes stale. Both rows are **correct and current**. South Dakota's own Remote Seller Bulletin, dated August 2025, states "more than $100,000 in gross sales into South Dakota in the previous or current calendar year" with no transaction test, confirming the 2023-07-01 repeal is reflected; Indiana's DOR FAQ says "Effective January 1, 2024, the 200 transaction threshold in Ind. Code 6-2.5-2-1 was removed". Both citations now carry the repeal date, so the absence of a transaction prong reads as a recorded fact rather than an omission.

  Kansas is the clearest citation error of the set, because the source quotes both subparagraphs side by side. The row cited `K.S.A. 79-3702(h)(1)(F)`, which reads "any retailer who has any other contact with this state that would allow this state to require the retailer to collect and remit tax" -- the constitutional catch-all, no threshold in it. SB 50 (2021) created **new subparagraph (h)(1)(G)**, whose clause (i)(b) is the actual test: "during the current or immediately preceding calendar year, the retailer had in excess of $100,000 of cumulative gross receipts". KDOR Notice 21-17 reproduces the amended statute with the new matter underlined, which is what makes the distinction checkable rather than inferred.

  Pennsylvania is correct on the number -- $100,000 in annual Pennsylvania gross sales, measured by calendar year, and the Department is explicit that "economic presence in Pennsylvania is measured by sales threshold only. There is no transactional count requirement." Its citation pointed at `61 Pa. Code 56.18`, which no longer resolves on the Pennsylvania Code site; the Department names Act 13 of 2019 and Sales & Use Tax Bulletin 2019-01, so those replace it.

  Colorado could not be reached: tax.colorado.gov returns 403 to automated requests. Not stamped.

  Massachusetts is the cleanest row so far: the statute itself was readable and says what the row says. Mass. Gen. Laws ch. 64H sec. 34 reads "if its sales within the commonwealth in the prior taxable year or the current taxable year exceed $100,000" -- note **taxable** year rather than calendar year, which no other bundled row uses. Citation correct as written.

  Tennessee's figure is confirmed at $100,000 with no transaction test, from the Department's out-of-state dealers page and Important Notice #20-14; the threshold was $500,000 until 2020-10-01, so this is a row that would have been wrong had it been written a year earlier. Its citation named `Tenn. Code Ann. 67-6-501(b)(1)`, which I did not open -- with half the citations so far pointing somewhere else, an unread subsection should not stand as though it had been checked, so the citation now names the chapter and the two departmental sources actually read.

  Wisconsin and North Carolina are both correct, both sales-only, and both confirm a repeal the bundled row already reflected: Wisconsin's 200-transaction test went with 2021 Wis. Act 1 effective 2021-02-20, and North Carolina's is likewise gone. North Carolina also surfaced something newer than anything else found here -- NCDOR states that from **2026-07-02** a remote seller crossing the threshold must register from the first day of the first calendar month at least 60 days later. The threshold is unchanged; the timing is not, and it is recorded in the citation because the tile has no field for it.

  Arizona's citation was wrong and the statute it named makes the mistake obvious once opened: `A.R.S. 42-5043` is liability relief for errors in tax payment, with its own phase-down of relief percentages (5% / 3% / 0%) that reads a little like a threshold schedule but is not one. The threshold is A.R.S. 42-5044, and it too phases: $200,000 in 2019, $150,000 in 2020, $100,000 in 2021 and thereafter. The bundled $100,000 is right for today and would have been wrong in either of the first two years.

  Iowa's citation is correct, and reading it turned up a distinction no other row carries: Iowa Code 423.14A(3)(a) says gross revenue "**equal to or exceeding** one hundred thousand dollars", where South Dakota says "more than $100,000" and most others say "exceeds". A seller at exactly $100,000 has nexus in Iowa and not in South Dakota. The tile stores one number and cannot express the boundary, so it is in the citation.

  Missouri's citation was off by one paragraph -- the row named `144.605(2)(d)`, and the remote-vendor definition is `(2)(e)`: "gross receipts from taxable sales from delivery of tangible personal property into this state in the previous calendar year or current calendar year exceeds one hundred thousand dollars". The Department determines it quarterly over a trailing twelve months, which is a second measurement wrinkle the schema cannot hold.

  New Mexico is not a sales tax at all. It is a gross receipts tax, and the threshold is "at least $100,000 of taxable gross receipts in the **previous** calendar year" -- previous only, like Florida. The bundled figure is right; the row's label of 'sales threshold' is the tile's own vocabulary rather than New Mexico's.

  Idaho's citation named subsection `(2)`; the remote-seller definition is `63-3611(h)` -- "cumulative gross receipts from sales delivered into Idaho in excess of one hundred thousand dollars". North Dakota's `57-39.2-02.2` is correct and the section is titled for exactly this purpose: "Certain sellers located outside this state required to collect and remit sales taxes - Criteria". Both are $100,000 over the previous or current calendar year with no transaction test.

  **Of the twenty-four citations checked so far, ten did not point at the operative provision** -- two named the wrong section (NV, MD), one a notice that no longer exists (MS), one a provision superseded in 2020 (WA), and one was right but unverifiable for currency until the agency page was read (NJ). These rows were assembled without each section being opened, so the citation is worth re-reading on every row, not only the numbers.

  Two states could not be reached and are not stamped: michigan.gov returns 403 to automated requests (its row also still cites RAB 2018-16, which a later RAB supersedes), and law.justia.com returns 403, which closes the route to Georgia's O.C.G.A. text. Neither was guessed at from a summary chart.

- **Re-checked 2026-09-03 (14 of 47 rows):** Eight were read against the state's own code text and found **correct**, needing no change -- OH, VA, MN, NE, WV, VT, HI, DC -- with their citations tightened to the subsection that actually carries the two thresholds. Finding a row right is a verification too, and the only kind that lets a stamp move. The other six: LA, UT, IL and KY had transaction thresholds their legislatures repealed (2023 La. Acts 375 eff. 2023-08-01; Utah S.B. 47 eff. 2025-07-01; Ill. P.A. 104-0006 eff. 2026-01-01; 2026 Ky. Acts ch. 161 eff. 2026-08-01). NY and CT gained their conjunctive rule. That pass left 33 rows at `verified_on: 2025-01-15`, because re-stamping an unread row is the failure the freshness gates exist to prevent. **A second pass on 2026-09-04 read 29 more, and a third on 2026-09-09 read the last four -- MI, GA, AR and CO -- so no row carries the 2025-01-15 stamp any more.** The seven this entry originally named as the next place to look (AR, GA, MD, MI, NV, NJ, RI) is long superseded. `data/legal/manifest.json`'s generated `staleness_note` is the surface that always has it right, because it is computed from the stamps rather than written by hand -- which is exactly why this sentence went stale twice and that note never did; it has already re-pivoted to name the 2026-09-03 cohort as the oldest. **New Jersey was deliberately left unstamped**: only the 2018 session law (P.L.2018, c.132) was reachable, and an enacting text does not prove the section has not been amended since. justia.com and dfa.arkansas.gov refuse automated fetches, and the Maryland and Rhode Island pages tried do not carry the threshold text: the trend since 2023 is states repealing it. Read the enrolled act or the code section, not a summary chart -- state bill PDFs bracket deleted text, which is unambiguous.
- Cadence: Quarterly recheck against the state source page (oldest `verified_on` first). **No longer past that cadence:** as of 2026-09-09 no row carries a 2025 stamp -- the oldest cohort is now 2026-09-03, and 14 of the 47 rows still carry `verified_on` 2026-09-03, well inside the quarterly window. `data/legal/manifest.json`'s `staleness_note` re-pivoted to name that cohort on its own. That sentence is kept in this exact shape deliberately: `check-manifests` parses it, and fails the build if it disappears rather than letting the check go blind. That note is **generated from the stamps** by `scripts/staleness-notes.mjs`, not hand-written: it went false the day the first fourteen rows were re-verified and still told readers of the public manifest that none had been. `check-manifests` warns from one cadence period and fails at four unless the manifest's note is exactly what the generator produces from the current stamps -- a note that has drifted no longer buys the folder a pass. The tile prints each row's citation and verified-on date and tells the reader to confirm with the state before filing. The last four were closed on 2026-09-09, and every one was a CITATION defect with correct numbers underneath -- which is the pattern worth expecting when a row has sat unread. **MI** cited RAB 2018-16 where Treasury issued RAB 2021-21 on 2021-11-03, which says on its face that it replaces 2018-16. **GA** cited only O.C.G.A. 48-8-2(8)(M.1), the dollar prong; the 200-transaction prong is the separate subparagraph (M.2), and paragraph (8) is a disjunctive list of ways a person is a dealer, which is what makes the row's `or` right. **AR** cited the section but not the subsection: 26-52-111(a) reads "(1) One hundred thousand dollars ($100,000); or (2) Two hundred (200) transactions", an explicit statutory or. **CO** cited 39-26-102(3)**(b)**, which is the solicitation prong; economic nexus is (3)**(c)(I)**, and it carries a dollar threshold only -- no transaction count -- which is what makes that row's null prong right. Finding a row right is a verification too, and it is the only kind that lets a stamp move. **The method that unblocked all four: a BROWSER.** michigan.gov and justia.com both 403 an automated fetch and both read cleanly in one, which is why these rows sat unread for months against a note saying no source would serve them.

  Each row's `citation` is capped at 140 characters by the build's v6 prose-lint, so it names the operative provision and the measurement period and nothing more. The detail that does not fit lives here:

  | State | Read on 2026-09-04 | What the citation could not hold |
  | --- | --- | --- |
  | SC | `12-36-70`, applied by Rev. Rul. #18-14 | The row previously cited `12-36-1340`, which imposes the duty to collect but states **no threshold at all** -- its subsection (6) says only "meets constitutional standards for economic nexus". South Carolina's $100,000 is administrative, not statutory, and measures **gross revenue including exempt retail sales, wholesale sales and services the state cannot tax**. |
  | OK | `1392(G)` | Previously cited bare section 1392, whose subsection (A) is a **$10,000 election** for marketplace facilitators and referrers, not the remote-seller duty. |
  | WY | `39-15-501(a)(i)` | Subsection `(a)(ii)` now reads, in full, "Repealed by Laws 2024, ch. 67, s. 2" -- the repeal of the 200-transaction prong is visible in the statute itself. |
  | AK | ARSSTC Uniform Code sec. 040(A) | Alaska has **no statewide sales tax**; the threshold is set by an intergovernmental commission for its member municipalities. It is "meets or exceeds" $100,000 (inclusive, like Iowa; unlike South Dakota's "more than"), counting the seller's marketplace facilitator's statewide gross sales too. |
  | ME | `1754-B(1-B)(B)` | Previously cited bare section 1754-B, whose subsections 1 and 1-A were repealed in 2019 and 2021. |
  | ND | `57-39.2-02.2` | Correct as cited. The section sits *before* 57-39.2-02.1 in the chapter, which is why it reads as missing on a first pass. |
...
- Source: IUPAC/CIAAW Abridged Standard Atomic Weights 2024 (the Atomic Weights 2021 report with the 2024 revisions to gadolinium, lutetium and zirconium; of those three only zirconium is bundled, at 91.222). CRC Handbook of Chemistry and Physics 95th ed. for the Tris, phosphate, acetate and bicarbonate pKa values. Good et al. (Biochemistry 5(2): 467, 1966) named the Good's buffers and tabulates them **at 20 °C**; the bundled 25 °C values for HEPES, MES, MOPS and PIPES are the 25 °C column of PanReac AppliChem's Biological buffers IP-022EN, which its own d(pKa)/dT reconciles with Good's 20 °C figures. Manufacturer-published rotor specifications (Eppendorf, Beckman Coulter, Thermo Fisher).
...
- Rotor radii: all six rows verified 2026-09-09 against each manufacturer's published maximum speed and maximum RCF, and against the radius those imply through `RCF = 1.118e-6 x r(mm) x rpm^2` -- the inversion is what settles a radius the vendor does not print. Three were wrong: the FA-45-6-30 carried the FA-45-30-11's 95 mm (Eppendorf publishes 12.3 cm), the A-4-81 carried its MTP/Flex plate bucket's 162 mm under a label naming the rotor generally (18 cm with the buckets it ships with), and the Fiberlite F15-8x50c carried 137 mm against a published 10.4 cm, making every RCF it gave 32% high.
...
- Cadence: 5-year quinquennial release. WMM2025 covers 2025-01-01 through 2038-12-31; the next release (WMM2030) is expected in 2038-12 and the bundle's `expires_on` is set to 2030-01-01. The `scripts/sources-cycle.json` entry for `wmm` drives the freshness lint warning when expiry is within 6 months.
...
- Source: Federal Housing Finance Agency, Conforming Loan Limit Values (annual; 2026 values published November 2025) at [fhfa.gov/data/conforming-loan-limit](https://www.fhfa.gov/data/conforming-loan-limit). HUD Single-Family Mortgage Limits (annual; 2026 values published December 2025) at [entp.hud.gov/idapp/html/hicostlook.cfm](https://entp.hud.gov/idapp/html/hicostlook.cfm). VA full-entitlement no-cap policy per the Blue Water Navy Vietnam Veterans Act of 2019 (Public Law 116-23).
...
- Shard layout: `{ source, edition, fetched, verified_on, free_access, fiscal_year, areas: [{ name, state, fips, fmr_0br, fmr_1br, fmr_2br, fmr_3br, fmr_4br, safmr? }, ...], unknown_area_message, safmr_message, safmr_partial_message }`. Bundled snapshot covers 19 representative HUD Metro FMR Areas / MSAs; canonical per-county lookup is at huduser.gov. The 40th-percentile rent of recent-mover units in the HUD-defined FMR Area, used as the program payment standard for the Housing Choice Voucher (Section 8) program and several HUD subsidies.
- Verification: all 19 rows were read from HUD's FY2026 Fair Market Rent Documentation System on 2026-09-09. huduser.gov serves no automated fetch (curl and WebFetch both get an empty 202), so the rows were read in a browser: `select_Geography.odn` lists every FMR area with its `cbsasub` code, and `2026summary.odn?...&cbsasub=<code>&selection_type=cbsa` gives that area's FMR table. A Small Area FMR area's page shows only the ZIP table; its metro-wide figure is behind `&dallas_sa_override=TRUE&selection_type=hmfa`.
- Small Area FMRs: 11 of the 19 areas are ones where HUD sets every Housing Choice Voucher payment standard by ZIP Code, and one (Houston) has a PHA that opted in. Those rows carry `safmr: "all"` or `safmr: "partial"`, and the tile prints the metro-wide figure with the advisory that it is not the voucher standard there.
