...
- **Re-checked 2026-09-03 (12 of 47 rows):** Six were read against the state's own code text and found **correct**, needing no change -- OH, VA, MN, NE, WV, VT -- with their citations tightened to the subsection that actually carries the two thresholds. Finding a row right is a verification too, and the only kind that lets a stamp move. The other six: LA, UT, IL and KY had transaction thresholds their legislatures repealed (2023 La. Acts 375 eff. 2023-08-01; Utah S.B. 47 eff. 2025-07-01; Ill. P.A. 104-0006 eff. 2026-01-01; 2026 Ky. Acts ch. 161 eff. 2026-08-01). NY and CT gained their conjunctive rule. The remaining 35 rows keep `verified_on: 2025-01-15` and the folder keeps warning, because re-stamping an unread row is the failure the freshness gates exist to prevent. The 11 unread rows that still carry a transaction prong are the highest-yield place to look next (AR, GA, HI, MD, MI, NV, NJ, RI, DC, and the two conjunctive ones are done): the trend since 2023 is states repealing it. Read the enrolled act or the code section, not a summary chart -- state bill PDFs bracket deleted text, which is unambiguous.
- Cadence: Quarterly recheck against the state source page (oldest `verified_on` first). **Currently past that cadence and acknowledged as such:** every row carries `verified_on` 2025-01-15, and `data/legal/manifest.json` holds a `staleness_note` saying why (47 per-state lookups against each department of revenue, driven by legislation rather than a calendar). `check-manifests` warns from one cadence period and fails at four unless that note is present. The tile prints each row's citation and verified-on date and tells the reader to confirm with the state before filing.
