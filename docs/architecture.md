...
|  |  +------------+  +-----------+  +---------------------+   | |
|  |   dynamic-import: 83 calc-* modules (the repo-root        | |
...
- A single index.html, styles.css, app.js, and sw.js at the repository root. `sw.js` precaches the app shell and the data manifests; it does **not** precache the **2146 static shells**. A shell URL opened offline therefore takes the navigation fallback, which redirects to the root and carries a tile id through as the hash (`/tools/ohms-law/` -> `/#ohms-law`). Serving index.html at the shell's own URL, as the fallback did until 2026-08-31, left every relative asset path resolving under `/tools/<id>/`, so the reader got an unstyled home view instead of the calculator they asked for.
...
the original seven trade calc-* modules. The set has since grown to 83
