...
|  |  +------------+  +-----------+  +---------------------+   | |
|  |   dynamic-import: 77 calc-* modules (the repo-root        | |
...
- A single index.html, styles.css, app.js, and sw.js at the repository root. `sw.js` precaches the app shell and the data manifests; it does **not** precache the **2104 static shells**. A shell URL opened offline therefore takes the navigation fallback, which redirects to the root and carries a tile id through as the hash (`/tools/ohms-law/` -> `/#ohms-law`). Serving index.html at the shell's own URL, as the fallback did until 2026-08-31, left every relative asset path resolving under `/tools/<id>/`, so the reader got an unstyled home view instead of the calculator they asked for.
...
- **Exact addresses.** A tile's own id, and a phrase a maintainer curated against it, are not ranking opinions. `resolveQuery` in the same module reads both, and `promoteExactMatch` moves the tile it names to the front of a ranked list, leaving every other result in its order behind it. Until 2026-09-10 `resolveQuery` was reached only from `fallbackSearch`, so on the ranked path the curation was never read: swept over all 22,534 curated terms, three targets never appeared in the browser's twelve-row dropdown at all -- "how much can i build on my lot" (`floor-area-ratio`), "what size wire" (`min-conductor-for-vd`), "what size weld" (`steel-fillet-weld-size`) -- and 58 more appeared but not first, with "wire size" returning `thread-measure-wire`, a machinist's thread-measuring wire, ahead of wire ampacity. All 22,534 now reach their tile, and all of them first. `mcp/catalog.mjs` resolves the same two addresses ahead of its own ranker, so the doors agree on what an address is. Where a curated term is also some other tile's exact name, curation wins -- the call the agent door already made after measuring. Note that `scripts/measure-ranking.mjs` measures `rankTools` **alone**, which no door calls alone, so an exact-term miss in its output is still first in the dropdown; its summary says so.
...
the original seven trade calc-* modules. The set has since grown to 77
