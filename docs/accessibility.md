...
one. The 2,155 prerendered pages load no script at all, so the media query is
...
- **axe-core verification:** this said "the shells pass the same
  ...
  runs 2,134 routes that are all SPA hash routes; the shells are a
  ...
  **all 21 group hubs failed WCAG 1.4.1** -- each tile link sits in a
  ...
  sweep the remaining tile shells -- **2,133 tile shells** exist and the
...
- The structural half of W3C validation is checked offline on all 2,155
