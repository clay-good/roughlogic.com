...
| calc-brewing.js | `computeBeerColorSrm` | `{ base_malt_lb = 0, base_lovibond = 0, crystal_malt_lb = 0, crystal_lovibond ...` | _ | _ | _ |
| calc-brewing.js | `computeBrewhouseEfficiency` | `{ grain_weight_lb = 0, extract_potential_ppg = 0, volume_gal = 0, original_gr...` | _ | _ | _ |
| calc-brewing.js | `computeCarbonationVolumesPressure` | `{ beer_temp_f = 0, gauge_psig = 0, target_volumes = 0, warm_temp_f = 0 } = {}` | _ | _ | _ |
| calc-brewing.js | `computeDryHopBeerLoss` | `{ batch_volume_gal = 0, dry_hop_lb_per_bbl = 0, absorption_gal_per_lb = 0, pa...` | _ | _ | _ |
| calc-brewing.js | `computeFermenterGlycolLoad` | `{ batch_volume_gal = 0, original_gravity = 0, final_gravity = 0, heat_of_ferm...` | _ | _ | _ |
| calc-brewing.js | `computeIbuTinseth` | `{ batch_volume_gal = 0, hop_weight_lb = 0, alpha_acid_pct = 0, boil_minutes =...` | _ | _ | _ |
| calc-brewing.js | `computeKettleBoilOff` | `{ preboil_volume_gal = 0, preboil_gravity = 0, boiloff_pct_per_hour = 0, boil...` | _ | _ | _ |
| calc-brewing.js | `computeMashStrikeWater` | `{ grain_weight_lb = 0, mash_thickness_qt_per_lb = 0, grain_temp_f = 0, target...` | _ | _ | _ |
| calc-brewing.js | `computeMashTunGrainBed` | `{ tun_diameter_ft = 0, grain_weight_lb = 0, mash_thickness_qt_per_lb = 0, gra...` | _ | _ | _ |
| calc-brewing.js | `computePackagingYieldLoss` | `{ brite_volume_gal = 0, transfer_loss_pct = 0, fill_loss_pct = 0, package_gal...` | _ | _ | _ |
| calc-brewing.js | `computeProofGallonYield` | `{ wash_volume_gal = 0, wash_abv_pct = 0, recovery_pct = 0, collection_proof =...` | _ | _ | _ |
| calc-brewing.js | `computeSpargeWaterVolume` | `{ grain_weight_lb = 0, mash_thickness_qt_per_lb = 0, absorption_gal_per_lb = ...` | _ | _ | _ |
| calc-brewing.js | `computeYeastPitchRate` | `{ batch_volume_gal = 0, original_gravity = 0, pitch_rate_million_per_ml_plato...` | _ | _ | _ |
...
| calc-concrete.js | `computeConcreteImmediateDeflection` | `{ ie_in4 = 0, fc_psi = 4000, w_klf = 0, span_ft = 0, support = "simple_udl" }...` | _ | _ | _ |
...
| calc-construction.js | `computeAccessibleRouteWidth` | `{ clear_width_in = 0, pinch_present = "no", pinch_width_in = 0, pinch_length_...` | _ | _ | _ |
...
| calc-construction.js | `computeDrinkingFountainCheck` | `{ units_provided = 0, bi_level = "no", wheelchair_spout_in = 0, standing_spou...` | _ | _ | _ |
| calc-construction.js | `computeDryerDuctLength` | `{ straight_run_ft = 0, elbow_90_count = 0, elbow_45_count = 0, eq_len_90_ft =...` | _ | _ | _ |
...
| calc-construction.js | `computeGuardPostLoad` | `{ post_height_in = 36, post_spacing_ft = 6, concentrated_lb = 200, uniform_pl...` | _ | _ | _ |
| calc-construction.js | `computeHandrailGeometry` | `{ rail_height_in = 0, clearance_in = 0, grip_shape = "circular", diameter_in ...` | _ | _ | _ |
...
| calc-construction.js | `computeTurningAndClearFloorSpace` | `{ cfs_width_in = 0, cfs_depth_in = 0, approach = "forward", alcove_depth_in =...` | _ | _ | _ |
...
| calc-construction.js | `computeWaterClosetLocation` | `{ centerline_in = 0, seat_height_in = 0, clear_side_in = 0, clear_rear_in = 0...` | _ | _ | _ |
...
| calc-construction.js | `computeWindKz` | `{ exposure = "C", z_ft = 0 } = {}` | _ | _ | _ |
...
| calc-corrosion.js | `computeAcInducedVoltagePipeline` | `{ induced_ac_v = 0, soil_resistivity_ohm_m = 0, holiday_area_cm2 = 0, touch_l...` | _ | _ | _ |
| calc-corrosion.js | `computeAnodeBedResistance` | `{ soil_resistivity_ohm_cm = 0, column_length_ft = 0, column_diameter_in = 0, ...` | _ | _ | _ |
| calc-corrosion.js | `computeCloseIntervalSurveyReadings` | `{ survey_length_mi = 0, reading_interval_ft = 0, readings_per_station = 0, sp...` | _ | _ | _ |
| calc-corrosion.js | `computeCoatingBreakdownFactor` | `{ surface_sqft = 0, bare_current_density_ma_per_sqft = 0, initial_breakdown_p...` | _ | _ | _ |
| calc-corrosion.js | `computeCokeBreezeBackfill` | `{ hole_diameter_in = 0, hole_depth_ft = 0, anode_diameter_in = 0, anode_lengt...` | _ | _ | _ |
| calc-corrosion.js | `computeCorrosionRateWeightLoss` | `{ mass_loss_mg = 0, density_g_cm3 = 0, exposed_area_sqin = 0, exposure_hours ...` | _ | _ | _ |
| calc-corrosion.js | `computeCpRectifierSizing` | `{ design_current_a = 0, bed_resistance_ohm = 0, header_length_ft = 0, negativ...` | _ | _ | _ |
| calc-corrosion.js | `computeGalvanicAreaRatio` | `{ anode_potential_v = 0, cathode_potential_v = 0, cathodic_current_density_ma...` | _ | _ | _ |
| calc-corrosion.js | `computeInstantOffIrDrop` | `{ on_potential_v = 0, instant_off_potential_v = 0, native_potential_v = 0, cr...` | _ | _ | _ |
| calc-corrosion.js | `computePipelinePotentialAttenuation` | `{ pipe_od_in = 0, wall_thickness_in = 0, steel_resistivity_ohm_in = 0, coatin...` | _ | _ | _ |
| calc-corrosion.js | `computePolarizationDecayCriterion` | `{ on_potential_v = 0, instant_off_potential_v = 0, native_potential_v = 0, de...` | _ | _ | _ |
| calc-corrosion.js | `computeStrayCurrentBond` | `{ open_circuit_v = 0, solid_bond_current_a = 0, target_bond_current_a = 0, in...` | _ | _ | _ |
| calc-corrosion.js | `computeTankBottomAnodeLayout` | `{ tank_diameter_ft = 0, current_density_ma_per_sqft = 0, grid_spacing_ft = 0,...` | _ | _ | _ |
...
| calc-cross.js | `computeExtensionLadderOverlap` | `{ nominal_length_ft = 0, sections = 2, overlap_override_ft = 0 } = {}` | _ | _ | _ |
...
| calc-doorhardware.js | `computeFireDoorClearance` | `{ head_in = 0, hinge_jamb_in = 0, strike_jamb_in = 0, meeting_edge_in = 0, bo...` | _ | _ | _ |
| calc-doorhardware.js | `computeGateOperatorDuty` | `{ gate_weight_lb = 0, rolling_coefficient = 0.1, grade_pct = 0, gate_length_f...` | _ | _ | _ |
| calc-doorhardware.js | `computeKeyCutMacs` | `{ depth_1 = 0, depth_2 = 0, depth_3 = 0, depth_4 = 0, depth_5 = 0, depth_6 = ...` | _ | _ | _ |
| calc-doorhardware.js | `computeLockBacksetLayout` | `{ backset_in = 2.375, cross_bore_dia_in = 2.125, edge_bore_dia_in = 1, door_t...` | _ | _ | _ |
| calc-doorhardware.js | `computeMaglockLeverage` | `{ rated_holding_lb = 0, lock_distance_from_hinge_in = 0, handle_distance_from...` | _ | _ | _ |
| calc-doorhardware.js | `computeMasterKeyCapacity` | `{ cut_positions = 0, usable_depths = 0, mastered_positions = 0, alternative_m...` | _ | _ | _ |
| calc-doorhardware.js | `computePanicHardwareForce` | `{ release_force_lbf = 0, set_in_motion_lbf = 0, swing_force_lbf = 0, release_...` | _ | _ | _ |
| calc-doorhardware.js | `computeRevolvingDoorThroughput` | `{ rpm = 0, wings = 0, people_per_compartment = 0, utilization_pct = 60, build...` | _ | _ | _ |
| calc-drainage.js | `computeBoxCulvertHeadwater` | `{ span_in = 0, rise_in = 0, flow_cfs = 0, slope = 0, length_ft = 0, manning_n...` | _ | _ | _ |
...
| calc-drainage.js | `computeManningSlope` | `{ pipe_diameter_in = 0, target_flow_gpm = 0, material = "pvc", scour_velocity...` | _ | _ | _ |
...
| calc-elevator.js | `computeElevatorRopeSafetyFactor` | `{ car_weight_lb = 0, rated_load_lb = 0, travelling_cable_lb = 0, rope_count =...` | _ | _ | _ |
| calc-elevator.js | `computeGovernorTrippingSpeed` | `{ rated_speed_fpm = 0, electrical_trip_fpm = 0, mechanical_trip_fpm = 0, code...` | _ | _ | _ |
| calc-elevator.js | `computeGuideRailBracketSpan` | `{ span_ft = 0, horizontal_load_lb = 0, section_modulus_in3 = 0, moment_of_ine...` | _ | _ | _ |
| calc-elevator.js | `computeHoistwayVenting` | `{ hoistway_plan_area_sqft = 0, vent_fraction_pct = 3.5, door_width_in = 0, do...` | _ | _ | _ |
| calc-elevator.js | `computeHydraulicJackPressure` | `{ bore_in = 0, total_load_lb = 0, car_speed_fpm = 0, pump_flow_gpm = 0, relie...` | _ | _ | _ |
| calc-elevator.js | `computeMachineRoomHeat` | `{ input_power_kw = 0, efficiency_pct = 85, duty_cycle_pct = 40, controller_st...` | _ | _ | _ |
| calc-elevator.js | `computeStepChainTension` | `{ total_load_lb = 0, incline_deg = 30, friction_coefficient = 0.03, chain_spe...` | _ | _ | _ |
| calc-elevator.js | `computeTractionRopingRatio` | `{ roping_ratio = 2, car_speed_fpm = 0, sheave_diameter_in = 0, unbalanced_loa...` | _ | _ | _ |
| calc-fab.js | `computeBarNesting` | `{ cut_list = "", stock_length_in = 240, kerf_in = 0.125, end_trim_in = 0 } = {}` | _ | _ | _ |
...
| calc-fab.js | `computeInterpassTemperatureControl` | `{ tau_min = 0, ambient_f = 70, preheat_min_f = 0, interpass_max_f = 0, curren...` | _ | _ | _ |
...
| calc-fab.js | `computeSheetMetalGauge` | `{ gauge = 0, material = "steel" } = {}` | _ | _ | _ |
...
| calc-fab.js | `computeTubeBendWallThinning` | `{ od_in = 0, wall_in = 0, clr_in = 0, bend_angle_deg = 90 } = {}` | _ | _ | _ |
| calc-fab.js | `computeVesselHeadVolume` | `{ inside_diameter_in = 48, head_type = "elliptical", straight_flange_in = 0 }...` | _ | _ | _ |
| calc-fab.js | `computeWeldCoolingRateT85` | `{ heat_input_kj_mm = 0, preheat_c = 20, thickness_mm = 0, f2 = 1.0, f3 = 1.0 ...` | _ | _ | _ |
...
| calc-finish.js | `computeCabinetLinearFeet` | `{ base_wall_ft = 0, wall_cab_ft = 0, tall_ft = 0, appliance_openings_ft = 0, ...` | _ | _ | _ |
...
| calc-finish.js | `computeCountertopOverhangSupport` | `{ overhang_in = 0, total_depth_in = 25.5, thickness_cm = 3, run_length_ft = 0...` | _ | _ | _ |
| calc-finish.js | `computeCrawlSpaceVentilation` | `{ floor_area_sf = 0, base_ratio_denominator = 150, vapor_retarder = "no", ven...` | _ | _ | _ |
| calc-finish.js | `computeDeckBoardTakeoff` | `{ deck_width_ft = 0, deck_length_ft = 0, board_face_width_in = 5.5, gap_in = ...` | _ | _ | _ |
| calc-finish.js | `computeDripEdgeTakeoff` | `{ eave_length_ft = 0, rake_run_ft = 0, rake_count = 0, pitch_rise_per_12 = 0,...` | _ | _ | _ |
...
| calc-hvacservice.js | `computeCondensateOverflowPan` | `{ unit_width_in = 0, unit_length_in = 0, pan_width_in = 0, pan_length_in = 0,...` | _ | _ | _ |
...
| calc-hvacservice.js | `computeDamperAuthority` | `{ damper_dp_inwg = 0, branch_dp_inwg = 0, face_area_sqft = 0, leakage_class_c...` | _ | _ | _ |
...
| calc-hvacservice.js | `computeFlueGasDewPoint` | `{ excess_air_pct = 15 } = {}` | _ | _ | _ |
...
| calc-hvacsystems.js | `computeChillerStagingPoint` | `{ machine_tons = 0, plant_load_tons = 0, kw_per_ton_100 = 0, kw_per_ton_75 = ...` | _ | _ | _ |
| calc-hvacsystems.js | `computeChillerTons` | `{ gpm = 0, ewt_F = 54, lwt_F = 44, fluid = "water", nameplate_tons = null, } ...` | _ | _ | _ |
| calc-hvacsystems.js | `computeCoilCapacityVerification` | `{ airflow_cfm = 0, entering_air_db_f = 0, leaving_air_db_f = 0, enthalpy_drop...` | _ | _ | _ |
...
| calc-kitchen.js | `computeDraftBeerLineBalance` | `{ applied_pressure_psi = 0, rise_ft = 0, tubing_type = "vinyl_316", fixture_p...` | _ | _ | _ |
...
| calc-layout.js | `computeCircularSegmentArea` | `{ chord_in = 0, rise_in = 0 } = {}` | _ | _ | _ |
...
| calc-lineworker.js | `computeMeterCtPtMultiplier` | `{ ct_primary_a = 0, ct_secondary_a = 5, pt_primary_v = 0, pt_secondary_v = 12...` | _ | _ | _ |
...
| calc-lineworker.js | `computeRecloserFuseCoordination` | `{ fault_current_a = 0, fast_curve_s = 0, slow_curve_s = 0, fuse_min_melt_s = ...` | _ | _ | _ |
| calc-lineworker.js | `computeRegulatorTapBandwidth` | `{ base_voltage_v = 120, bandwidth_v = 0, tap_position = 0, load_current_a = 0...` | _ | _ | _ |
...
| calc-lineworker.js | `computeTransformerDiversityLoading` | `{ customers = 0, individual_peak_kva = 0, diversity_factor = 0, continuous_ra...` | _ | _ | _ |
...
| calc-machining.js | `computeBandSawBladePitch` | `{ thickness_in_cut_in = 0, blade_speed_sfm = 0, wheel_diameter_in = 0, cut_ar...` | _ | _ | _ |
...
| calc-machining.js | `computeDrillFeedThrust` | `{ diameter_in = 0, sfm = 0, feed_ipr = 0, unit_power = 1.0, spindle_efficienc...` | _ | _ | _ |
...
| calc-mechanic.js | `computeInclinedPlaneForce` | `{ weight_lb = 0, incline_angle_deg = 0, friction_coefficient = 0 } = {}` | _ | _ | _ |
...
| calc-mechanic.js | `computeToggleMechanismForce` | `{ input_force_lb = 0, toggle_angle_deg = 0 } = {}` | _ | _ | _ |
...
| calc-oilgas.js | `computeAnnularVelocityCleaning` | `{ hole_dia_in = 0, pipe_od_in = 0, flow_gpm = 0, slip_velocity_ft_min = 0, me...` | _ | _ | _ |
...
| calc-rail.js | `computeRailcarLoadLimit` | `{ gross_rail_load_lb = 0, light_weight_lb = 0, lading_net_lb = 0, cubic_capac...` | _ | _ | _ |
| calc-rail.js | `computeTonnageRatingGrade` | `{ tractive_effort_lb = 0, ruling_grade_pct = 0, rolling_resistance_lb_per_ton...` | _ | _ | _ |
...
| calc-refrigerant.js | `computeRefrigerantLeakRate` | `{ full_charge_lb = 0, pounds_added_lb = 0, period_months = 12, threshold_pct ...` | _ | _ | _ |
...
| calc-refrigerant.js | `computeRefrigerantRecoveryTime` | `{ total_charge_lb = 0, liquid_charge_lb = 0, liquid_rate_lb_min = 0, vapor_ra...` | _ | _ | _ |
...
| calc-refrigerant.js | `computeTxvCapacityCheck` | `{ nominal_tons = 0, rated_dp_psi = 100, actual_dp_psi = 0, liquid_temp_factor...` | _ | _ | _ |
...
| calc-shop.js | `computeGasStrutForce` | `{ lid_weight_lb = 0, cg_distance_in = 0, opening_angle_deg = 0, moment_arm_in...` | _ | _ | _ |
| calc-shop.js | `computeHeatTreatSoakTime` | `{ charge_weight_lb = 0, section_thickness_in = 0, soak_temp_f = 0, start_temp...` | _ | _ | _ |
...
| calc-shop.js | `computePlatingTankCurrent` | `{ current_density_asf = 0, part_area_sqft = 0, atomic_weight = 0, valence = 0...` | _ | _ | _ |
| calc-shop.js | `computePneumaticCylinderScfm` | `{ bore_in = 0, rod_in = 0, stroke_in = 0, cycles_per_min = 0, pressure_psig =...` | _ | _ | _ |
| calc-shop.js | `computePowderCoatingCoverage` | `{ specific_gravity = 0, film_thickness_mils = 0, part_area_sqft = 0, transfer...` | _ | _ | _ |
...
| calc-shop.js | `computePyramidFrustumVolume` | `{ bottom_length_ft = 0, bottom_width_ft = 0, top_length_ft = 0, top_width_ft ...` | _ | _ | _ |
| calc-shop.js | `computeQuenchSeverity` | `{ grossmann_h = 0, section_diameter_in = 0, second_grossmann_h = 0 } = {}` | _ | _ | _ |
...
| calc-shop.js | `computeSprayBoothAirflow` | `{ opening_width_ft = 0, opening_height_ft = 0, face_velocity_fpm = 100, indoo...` | _ | _ | _ |
...
| calc-shop.js | `computeTankVolumeDishedHeads` | `{ diameter_ft = 0, shell_length_ft = 0, fill_depth_ft = 0, head_type = "ellip...` | _ | _ | _ |
...
| calc-steel.js | `computeShearFlowConnectorSpacing` | `{ shear_kip = 0, area_in2 = 0, ybar_in = 0, inertia_in4 = 0, connector_kip = ...` | _ | _ | _ |
...
| calc-survey.js | `computeStadiaDistance` | `{ s_ft = 0, theta_deg = 0, k_f = 100, hi_ft = 0, rod_ft = 0, sta_elev = 0, c_...` | _ | _ | _ |
...
| calc-trenchless.js | `computeHddFluidVolume` | `{ ream_diameter_in = 0, pipe_diameter_in = 0, bore_length_ft = 0, fluid_multi...` | _ | _ | _ |
...
| `asymmetrical-fault-xr` | Asymmetrical and Peak Fault Current from X/R | first-cycle fault asymmetry model (DC...; spec-v496 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `awg-wire-geometry` | AWG Conductor Geometry (Diameter, Circular Mils, mm^2) | AWG geometric definition (ASTM B258); spec-v804 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
| `battery-c-rate` | Battery C-Rate: Deliverable Power and Discharge Duration | Battery C-rate definition; spec-v238 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `battery-hydrogen-vent` | Battery Room Hydrogen Ventilation (IEEE 1635) | IEEE 1635 battery-room hydrogen venti...; spec-v518 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `battery-inverter-dc-conductor` | Battery-to-Inverter DC Conductor and OCPD (NEC 690.9 / 706) | battery-inverter DC sizing (NEC 690.8...; I_dc = 4000/(48*0.90) = 92.59; 1.25x = 115.74; next std O... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `battery-peak-shaving` | Battery Peak-Shaving Demand-Charge Savings | Demand-charge peak-shaving method; spec-v237 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `battery-series-parallel` | Battery Bank Series/Parallel Configuration | Battery bank series/parallel configur...; series = round(48/12.8) = 4; bus = 4*12.8 = 51.2; Ah = 2*... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `bifacial-pv-gain` | Bifacial PV Rear-Side Gain | Bifacial PV rear-side gain (bifaciali...; gain = 0.75 x 150/1000 = 0.1125; effective = 400 x 1.1125... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `bonding-jumper` | Bonding Jumper Sizing (Supply-Side and Equipment) | NFPA; spec-v109 section 2.2 pinned example (350 kcmil Cu servic... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+5 more) |
...
| `cable-bend-radius` | Cable Bend Radius Minimum | Southwire; Single-conductor THHN, 1000 V and below: 4D up to 1.0 in ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `capacitor-discharge-time` | Capacitor Discharge Time and Bleed Resistor (NEC 460.6) | NEC 2023 460.6; spec-v495 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `ceiling-speaker-coverage-angle` | Ceiling Speaker Coverage Angle for a Target Spacing | commercial-audio design practice (sol...; spec-v740 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `commercial-lighting-load` | Commercial General-Lighting and Receptacle Load (NEC 220.42(A) / 220.47) | NEC 2023 (NFPA 70); office 1.3 VA/ft2 (Table 220.42(A)): 5,000 ft2 x 1.3 = 6,... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `conductor-short-circuit-withstand` | Conductor Short-Circuit Thermal Withstand (Onderdonk / ICEA) | ICEA / Onderdonk; spec-v125 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `conduit-thermal-expansion` | PVC Conduit Thermal Expansion (NEC 352.44) | NFPA; spec-v126 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `continuous-load-ocpd` | Continuous-Load OCPD and Conductor at 125% (NEC 210.20 / 215.3) | NEC 2023 210.20(A) / 215.3 / 240.6(A); spec-v280 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `dp-level-hydrostatic` | Hydrostatic DP Level Transmitter (Head to Level) | Hydrostatic DP level transmitter (P =...; level = 4.33/(0.433*1.0) = 10 ft; span = 0.433*1.0*20 = 8... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `egc-upsize-proportional` | EGC Proportional Upsize for Increased Conductors (NEC 250.122(B)) | NFPA; spec-v127 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `electric-lock-power-budget` | Electric Strike and Maglock Power, Inrush, and Standby Budget | Project (first-principles); the fire alarm interface governs the duration | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `ev-charge-cost` | EV Charge Cost at the Meter | EV charge-cost-at-the-meter model (en...; spec-v489 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `ev-charge-time` | EV Charge Time (AC Level 2) | AC Level 2 EV charge-time model (SAE ...; spec-v488 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `ev-range-per-hour` | EV Range Added per Hour of Charging | EV range added per hour of AC charging; range/hr = 7.7*0.88*3.5 = 23.716; hours = 100/23.716 = 4.217 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `existing-load-220-87` | Existing-Facility Load by Peak Demand (NEC 220.87) | NEC 2023 220.87; spec-v519 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `feeder-loss-load-factor` | Distribution Feeder I2R Loss and Loss Factor | Project (first-principles); the utility loss study and its metered load data govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `fiber-max-length` | Fiber Max Length for a Loss Budget | TIA-568 / IEEE 802.3 (by name), inverse; 2.6 dB budget, OM4 3.0 dB/km, 2 connectors @ 0.75 dB -> 3... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `grounding-electrode-conductor` | Grounding Electrode Conductor Sizing | NFPA; spec-v109 section 2.1 pinned example (250 kcmil Cu servic... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
...
| `harmonic-resonance` | Harmonic Parallel-Resonance Order | parallel-resonance order of a PF capa...; spec-v523 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `insulation-resistance-pi` | Insulation Resistance PI / DAR (Megger Test) | Insulation resistance PI / DAR (IEEE ...; DAR = 1040/800 = 1.30; PI = 4160/1040 = 4.0 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `lighting-density` | Lighting Power Density | ICC / ASHRAE; 1000 ft^2 office @ 0.64 W/ft^2 -> 640 W interior lighting... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `lighting-light-loss-factor` | Lighting Light-Loss Factor (Maintained/Initial) | IES Lighting Handbook; spec-v365 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `loop-signal-scaling` | 4-20 mA Current-Loop Signal Scaling | 4-20 mA current-loop live-zero scalin...; percent = (12-4)/16*100 = 50; value = 0 + 0.5*(100-0) = 50 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `loop-voltage-budget` | Loop-Powered (2-Wire) 4-20 mA Transmitter Voltage Budget | Loop-powered 2-wire 4-20 mA transmitt...; maxR = (24-10.5)/0.020 = 675; V_at = 24 - 0.020*300 = 18;... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `lv-dc-drop` | Low-Voltage DC Drop | Project (first-principles); 12 V / 10 AWG Cu / 20 ft / 10 A LED lighting -> ~0.486 V ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `maglock-holding-leverage` | Maglock Effective Holding Force at the Door Handle | Project (first-principles); the adopted codes govern electrically locked egress | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `max-circuit-length-for-vd` | Maximum Circuit Length for a Voltage-Drop Target | Max circuit length for a voltage-drop...; VD = 0.03*120 = 3.6; L = 3.6*6530/(2*12.9*20) = 23508/516... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `meter-ct-pt-multiplier` | Watt-Hour Meter CT / PT Multiplier | Project (first-principles); ANSI C12.1, the instrument transformer test reports and t... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `microinverter-branch-count` | Max Microinverters per AC Branch Circuit (NEC 705.60) | max microinverters per AC branch (NEC...; limit = 20*0.80 = 16 A; N = floor(16/1.21) = floor(13.22)... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
...
| `motor-acceleration-time` | Motor Across-the-Line Acceleration Time | Motor across-the-line acceleration ti...; t = WK^2*dN/(308*T_net) = 100*1750/(308*50) = 11.36 s | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `motor-efficiency-upgrade-savings` | Premium Motor Upgrade Energy Savings | motor-efficiency retrofit practice; spec-v471 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `motor-fault-contribution` | Motor Short-Circuit Contribution (First Cycle) | first-cycle motor short-circuit contr...; spec-v521 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `motor-locked-rotor-kva` | Motor Locked-Rotor Current from Code Letter (NEC 430.7(B)) | NEC 2023 Table 430.7(B); spec-v499 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `motor-max-hp-for-starting-current` | Max Motor HP for a Starting-Current Budget (NEC 430.7(B)) | NEC 2023 Table 430.7(B); 300 A budget, code G, 460 V three-phase -> 38.0 hp | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `motor-operating-cost` | Motor Input Power, Annual Energy, and Cost | First-principles motor input power; spec-v123 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `motor-overload-sizing` | Motor Running Overload Protection (NEC 430.32) | NEC 2023 430.32(A)(1) / 430.32(C); spec-v278 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
...
| `neutral-grounding-resistor` | Neutral Grounding Resistor Sizing (IEEE 142) | neutral grounding resistor sizing (IE...; spec-v525 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `open-delta-transformer` | Open-Delta (V-V) Transformer Bank Capacity | Open-delta (V-V) transformer bank cap...; available = sqrt(3) x 25 = 43.30; per unit = 40/sqrt(3) =... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `pid-tuning-ziegler-nichols` | PID Loop Tuning (Ziegler-Nichols Closed-Loop) | Ziegler-Nichols closed-loop PID tunin...; Kp = 0.6*4 = 2.4; Ti = 0.5*2 = 1.0; Td = 0.125*2 = 0.25; ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `power-factor-billing-savings` | Power-Factor Correction Demand-Billing Savings | Power-triangle demand-billing method; spec-v232 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `pull-box-sizing` | NEC Pull and Junction Box Sizing | NEC (NFPA 70) 314.28(A)(1) and (A)(2)...; spec-v101 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `pulse-flowmeter-k-factor` | Pulse Flowmeter K-Factor (Frequency to Flow) | Pulse flowmeter K-factor (turbine/pad...; rate = 100 Hz x 60 / 200 pulses/gal = 30 gpm; x60 = 1800 gph | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `pv-ac-output-circuit` | Inverter AC Output Circuit Conductor and OCPD (NEC 690.8(B)) | inverter AC output sizing (NEC 690.8(B)); I = 9600/240 = 40; 1.25x = 50; next std OCPD >= 50 = 50 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `pv-array-sizing` | PV Array Size from a Target Annual Energy | NREL PVWatts energy model (inverted f...; spec-v647 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `pv-cell-temperature-power` | PV Cell Temperature and Temperature-Derated Power | PV design practice / NREL; spec-v350 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `pv-energy-yield` | PV Annual Energy, Specific Yield, and Capacity Factor | NREL PVWatts energy model; spec-v221 section 2.1 pinned example (average US site) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `pv-inverter-ratio` | PV Inverter Loading Ratio (DC:AC) and Clipping Onset | Inverter loading ratio + NREL guidance; spec-v223 section 2.1 pinned example (in-band) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `pv-performance-ratio` | PV Performance Ratio from Stacked Losses | NREL PVWatts; spec-v351 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `regulator-tap-bandwidth` | Line Voltage Regulator Tap and Bandwidth | Project (first-principles); ANSI C57.15, IEEE 1783 and the regulator control manual g... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `rlc-reactance-resonance` | Series R-L-C Reactance, Impedance, and Resonant Frequency | Series R-L-C reactance / impedance / ...; XL=2pi*60*0.05=18.85; XC=1/(2pi*60*50e-6)=53.05; Z=sqrt(1... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `rolling-sphere-protection` | Lightning Rolling-Sphere Zone of Protection | NFPA 780 (rolling-sphere method); 30 ft mast, 150 ft sphere -> sqrt(9000 - 900) = sqrt(8100... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `room-cavity-ratio` | Room Cavity Ratio (RCR) for CU Lookup | IES zonal-cavity room cavity ratio; RCR = 5*8*(40+30)/(40*30) = 5*8*70/1200 = 2800/1200 = 2.333 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `rotary-phase-converter-sizing` | Rotary Phase Converter Idler Sizing | rotary phase converter idler sizing (...; start = 2*10 = 20; idler = max(20, 15) = 20 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `rtd-resistance-to-temp` | RTD (Pt100 / Pt1000) Resistance to Temperature | IEC 60751 platinum RTD (Callendar-Van...; T = (-A + sqrt(A^2 - 4B(1 - 119.397/100)))/(2B), A=3.9083... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
| `ruling-span` | Ruling (Equivalent) Span for a Line Section | Project (first-principles); the utility stringing charts and construction standards g... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `service-conductor-sizing` | Dwelling Service/Feeder Conductor at 83% (NEC 310.12) | NEC 2023 310.12 / Table 310.16 (75 degC); spec-v279 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `service-load-optional` | Service Load Calculation (NEC 220.82 Optional Method) | NFPA; general demand = 10kVA + 40%*(general-10kVA); + larger HV... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `soil-resistivity-wenner` | Wenner 4-Pin Soil Resistivity | Wenner 4-pin soil resistivity (IEEE 8...; rho = 2*pi*(10*0.3048)*5 = 2*pi*3.048*5 = 95.76 ohm-m; x1... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `transformer-diversity-loading` | Distribution Transformer Diversified Loading | Project (first-principles); the utility transformer loading guide and the manufacture... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `transformer-inrush-point` | Transformer Inrush Coordination Point | transformer energization-inrush coord...; spec-v520 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `transformer-loading-efficiency` | Transformer Loading Efficiency and Losses | transformer loss model; spec-v472 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `transformer-turns-ratio` | Transformer Turns / Voltage / Current / Impedance Ratio | ideal transformer circuit relations; spec-v806 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `transformer-voltage-regulation` | Transformer Voltage Regulation from %R and %X | transformer voltage-regulation approx...; spec-v494 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `voltage-drop` | Voltage Drop | Project (first-principles); Standard single-phase voltage-drop derivation; K=12.9 ohm... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `welder-arc-circuit-conductor` | Arc-Welder Branch-Circuit Conductor and OCPD (NEC 630.11) | arc-welder circuit sizing (NEC 630.11...; mult = sqrt(0.50) = 0.7071; I_eff = 40*0.7071 = 28.28; OC... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `welder-resistance-circuit-conductor` | Resistance / Spot-Welder Branch-Circuit Conductor and OCPD (NEC 630.31) | resistance-welder circuit sizing (NEC...; Table 630.31(A)(2) multiplier at 50% = 0.71 (the code rou... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `wire-pulling-lubricant` | Cable-Pulling Lubricant Quantity | Film-coating lubricant estimate (rule...; gallons = 0.0015*400*3*1.0 = 1.8 gal (Polywater Q = k L D) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `backflow-loss` | Backflow Preventer Pressure Loss | Watts Regulator; 1 in RP at 30 gpm -> 14.8 psi, read from the Watts LF909 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `box-culvert-headwater` | Governing Box Culvert Headwater (HDS-5) | FHWA HDS-5 (FHWA-HIF-12-026); HW_inlet = 4.336 ft (unsubmerged Form 1 inlet control) vs... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `box-culvert-inlet-control` | Box Culvert Headwater by Inlet Control (HDS-5) | FHWA HDS-5 (FHWA-HIF-12-026), Appendix A; B = 6 ft, D (rise) = 4 ft, A = 24 ft2, flow factor Q/(A s... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `box-culvert-outlet-control` | Box Culvert Headwater by Outlet Control (HDS-5) | FHWA HDS-5 (FHWA-HIF-12-026), Chapter 3; A = 6 x 4 = 24 ft2, P = 2(6+4) = 20 ft, R = 1.2 ft, V = 1... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `channel-froude-number` | Open-Channel Froude Number, Regime, and Critical Depth | Chow, Open-Channel Hydraulics; spec-v304 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `channel-normal-depth` | Rectangular Channel Normal Depth (Manning) | Manning normal depth (Chow, Open-Chan...; spec-v641 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `drywell-infiltration` | Dry Well / Infiltration Trench Sizing | Dry well / infiltration trench (void-...; excavation = 200/0.35 = 571.4; footprint = 571.4/4 = 142.... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `expansion-guide-spacing` | Expansion Joint / Loop Guide Spacing (EJMA 4D/14D) | EJMA 4-diameter / 14-diameter guide-p...; spec-v205 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `gas-leak-hole-diameter` | Gas Leak Equivalent Hole Diameter | Project (first-principles, solved for...; 7.72 cfh natural-gas leak at 0.25 psi -> ~0.050 in equiva... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `gas-leak-rate` | Gas Leak Rate (Orifice) | Project (first-principles); 0.05 in orifice / 0.25 psi upstream natural gas -> ~7.72 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `grab-bar-layout` | ADA Grab Bar Layout and Blocking Load | US Access Board / Department of Justice; 604.5.1 asks for a rear-wall bar 36 in minimum extending ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `main-disinfection-chlorine` | Water Main Chlorination Dose | AWWA C651 Disinfecting Water Mains (b...; spec-v103 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `manning-pipe-capacity` | Manning Pipe Capacity (Full-Bore Gravity Flow) | Manning full-bore capacity V = (1.486...; spec-v640 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `orifice-diameter-for-flow` | Orifice Diameter for a Target Flow | Inverse orifice sizing A = Q / (Cd sq...; spec-v639 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `orifice-flow` | Orifice Discharge Flow | Orifice discharge Q = Cd A sqrt(2 g h); spec-v303 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `pressure-tank-drawdown` | Well Pressure-Tank Drawdown and Sizing | Boyle's law on the diaphragm air char...; 44 gal at 40/60 psi (38 psi precharge) -> 11.35 gal drawd... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `roof-drain-sizing` | Roof Drain and Leader Sizing | IPC 2021 Section 1106 (Tables 1106.2 ...; 5000 ft^2 roof, 4 in/hr design rainfall -> 208 GPM, 5 in ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `septic-lpp-squirt-head` | Septic LPP Squirt Head for a Target Orifice Flow | Orifice-discharge equation / universi...; spec-v757 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `sewage-force-main-velocity` | Sewage Force-Main Scour Velocity | Ten States Standards; spec-v427 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `solder-joint-quantity` | Solder and Flux per Sweat-Joint Takeoff | Solder-weight identity (first-princip...; w/in = (PI/4)*0.125^2*0.265 = 0.003252; solder = 200*0.75... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `thermoplastic-temperature-derate` | Thermoplastic Pipe Pressure Derating vs Temperature | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `tr55-time-of-concentration` | Time of Concentration (TR-55 Velocity Method) | USDA NRCS TR-55 (1986), Chapter 3; Sheet n 0.24 dense grass, L 100 ft, P2 3.6 in, s 0.01: Tt... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `velocity-head` | Velocity Head and Dynamic Pressure | fluid mechanics; spec-v371 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `well-shock-chlorination` | Well Shock-Chlorination Dose | AWWA A100 / state private-well shock-...; spec-v103 section 2.2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `air-leak-cost` | Compressed-Air Leak Load and Annual Cost (Load/Unload Test) | US DOE Compressed Air Challenge load/...; spec-v239 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `air-pressure-setpoint-savings` | Compressed-Air Discharge-Pressure Setpoint Savings | Isentropic compression-power ratio (D...; spec-v241 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `baseboard-length-for-load` | Baseboard Length for a Room Load | Slant/Fin (inverse); 4,800 BTU/hr, 180 F water, 1 gpm, Fine/Line 30 (580 BTU/f... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `blower-door-ach50` | Blower-Door Air-Tightness (ACH50, Natural Infiltration, Code Check) | IECC 2021 R402.4.1.2-R402.4.1.3 + LBL...; spec-v218 section 2.1 pinned example (first test, FAIL) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `chimney-height-for-draft` | Chimney Height for a Target Draft | ASHRAE Handbook HVAC Systems / NFPA 2...; 0.1046 in wc net, 60 F ambient, 400 F mean flue, 14.7 psi... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `coil-bypass-factor` | Coil Bypass Factor and Apparatus Dew Point | ASHRAE Fundamentals; spec-v377 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `coil-capacity-verification` | Coil Capacity Verification From Measured Air and Water | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `coil-face-area` | Cooling Coil Face Area for a Target Velocity | coil-selection practice; spec-v701 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `colebrook-friction-factor` | Darcy Friction Factor (Swamee-Jain / Colebrook) | Swamee-Jain / Moody; spec-v387 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `compressor-volumetric-efficiency` | Compressor Volumetric Efficiency (Clearance Re-Expansion) | Reciprocating compressor clearance vo...; ratio = 300/70 = 4.286; VE = 100*(1+0.045-0.045*4.286^(1/... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `condensate-overflow-pan` | Auxiliary Condensate Drain Pan (IRC M1411.3.1) | International Code Council; 'The pan shall have a minimum depth of 1.5 inches, shall ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `condenser-cop-for-heat-rejection` | COP Implied by the Heat of Rejection | THR = Q_evap (1 + 1/COP), solved for COP; spec-v761 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `condenser-heat-rejection` | Condenser Total Heat of Rejection | Total heat of rejection THR = Q_evap ...; spec-v322 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `condensing-flue-condensate` | Condensing Appliance Flue Condensate Rate | Condensing appliance flue condensate ...; water = 1 therm/hr x 9.4 = 9.4 lb/hr; condensate = 9.4 x ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `cooling-coil-total-load` | Cooling Coil Total Load from Enthalpy Difference | ASHRAE Fundamentals; spec-v376 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `dcv-co2-ventilation` | Demand-Controlled Ventilation Rate from a CO2 Setpoint | Steady-state single-zone CO2 mass bal...; spec-v277 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `duct-heat-gain` | Duct Heat Gain/Loss Through Unconditioned Space | ASHRAE Fundamentals; spec-v347 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `duct-leakage` | Duct Leakage Test-and-Balance | SMACNA HVAC Air Duct Leakage Test Man...; 1000 design cfm / 990 measured / 300 ft^2 / 1.0 in WC -> ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `envelope-conduction-load` | Opaque-Envelope Conduction Cooling Load (Sol-Air CLTD) | ASHRAE / ACCA Manual J opaque envelope; spec-v229 section 2.1 pinned example (dark roof) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `erv-sensible-recovery` | ERV/HRV Sensible Effectiveness and Recovered Load | ASHRAE Standard 84 / AHRI Standard 10...; spec-v275 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `erv-total-enthalpy-recovery` | ERV Total Enthalpy Recovery | ASHRAE Fundamentals; spec-v441 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `evaporator-td-dtd` | Evaporator Design TD and Humidity Band | ASHRAE Refrigeration; spec-v434 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `fan-affinity-laws` | Fan Affinity Laws (Speed / Diameter Change) | AMCA / ASHRAE Fundamentals; spec-v384 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `fan-system-effect` | Fan System Effect and Installed Performance | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `flow-hood-correction` | Flow Hood Reading Correction and Diffuser Airflow | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `gas-dp-flow-meter` | Gas (Compressible) Differential-Pressure Flow Meter | ISO 5167-2 (compressible orifice flow...; eps=1-(0.351+0.256*0.0625+0.93*0.00390625)(1-0.99^(1/1.4)... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `heat-pump-seasonal-energy` | Heat-Pump Seasonal Heating Energy and Cost vs Gas and Resistance | AHRI 210/240 HSPF / fuel-cost comparison; spec-v233 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `hood-exhaust` | Commercial Kitchen Hood Exhaust (IMC 507) | ICC; §507.5.2: 400 cfm/ft heavy-duty wall-canopy x 8 ft = 3200... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `hvac-equipment-circuit` | HVAC Equipment Circuit (MCA / MOCP) | NEC 2023 (NFPA 70) 440.33 / 440.22(A)...; spec-v104 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `hydronic-gpm-deltat` | Hydronic System Flow from Load and Delta-T | Water-side heat transport Q = 500 GPM dT; spec-v306 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `hydronic-injection-mixing` | Hydronic Injection-Mixing Loop Flow | hydronic injection-mixing loop (prima...; inj = 10*(110-90)/(180-90) = 10*20/90 = 2.222 gpm; %sec =... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `infiltration-load` | Infiltration Heating / Cooling Load (Sensible + Latent) | ASHRAE Handbook of Fundamentals (air-...; spec-v220 section 2.1 pinned example (winter heating) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `insulation-heat-loss` | Pipe Insulation Heat Loss (bare vs insulated) | Project (first-principles); 2.375 in OD pipe at 200 F into 70 F still air with 1.5 in... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `insulation-thickness` | Pipe Insulation Thickness | Project (first-principles); 1 in OD pipe at 250 F into 75 F ambient, 120 F surface li... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `internal-heat-gains` | Internal Heat Gains: People, Lighting, Equipment | ASHRAE / ACCA Manual J internal gains; spec-v228 section 2.1 pinned example (small office) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `manual-d-friction-rate` | Manual D Friction Rate (Available Static Pressure) | ACCA Manual D; spec-v408 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `moist-air-enthalpy` | Moist Air Enthalpy (ASHRAE Psychrometrics) | ASHRAE Fundamentals; spec-v375 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `outdoor-reset-ratio` | Hydronic Outdoor Reset Ratio and Supply Target | hydronic outdoor reset control (Idron...; ratio = (180-80)/(65-0) = 1.5385; target = 80 + 1.5385*(6... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `outside-air-percent-temps` | Measured Outside-Air Percent from Mixed-Air Temperatures | ASHRAE / AABC-NEBB; spec-v386 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `pump-specific-speed` | Pump Specific Speed and Impeller Type | Hydraulic Institute specific speed; spec-v307 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `pump-suction-specific-speed` | Pump Suction Specific Speed (Nss) | Hydraulic Institute suction specific ...; spec-v629 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `radiant-floor-output` | Radiant Floor Heat Output | EN 1264 basic characteristic (8.92 W/...; spec-v442 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `recovery-cylinder` | Recovery-Cylinder 80% Fill | DOT / AHRI 700 / EPA Section 608 reco...; spec-v102 section 2.2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `refrigerant-mass-flow` | Refrigerant Mass Flow from Capacity and Refrigeration Effect | Vapor-compression cycle (m_dot = Q/(h...; spec-v320 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `valve-actuator-close-off` | Valve Actuator Close-Off Pressure and Torque | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `valve-authority` | Control Valve Authority (Beta) | Control valve authority (beta); beta = 5/(5+3) = 5/8 = 0.625 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `vibration-isolation` | Vibration Isolation Efficiency (ASHRAE) | ASHRAE Handbook -- Fundamentals, Soun...; spec-v483 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `ceiling-water-load` | Trapped Ceiling-Cavity Water Load and Collapse Screen | ANSI/IICRC S500 safety practice; wate...; spec-v137 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `char-depth-capacity` | Fire-Exposed Wood Char Depth and Residual Capacity | AWC National Design Specification cha...; 6x10 (5.5x9.5) 30 min, 2 width + 1 depth faces -> a_eff 1... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `dehumidifier` | Dehumidifier Sizing | IICRC S500 initial dehumidification f...; 6000 ft^3 / Class 2 / 50 -> 120 pints/day AHAM, the recom... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `ac-induced-voltage-pipeline` | AC Induced Voltage and AC Corrosion Density | Project (first-principles); ISO 18086 and a corridor study govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `accessible-parking-count` | Accessible Parking Space Count (2010 ADA Standards 208.2) | US Department of Justice / US Access ...; Table 208.2 row '76 to 100' gives 4. Section 208.2.4: 'Fo... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `accessible-parking-geometry` | Accessible Parking Space Geometry (2010 ADA Standards 502) | US Department of Justice / US Access ...; 502.2: 'Car parking spaces shall be 96 inches wide minimu... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `accessible-route-width` | Accessible Route Width, Pinch Points, and Passing Spaces (2010 ADA Standards 403.5) | US Department of Justice / US Access ...; 403.5.1 Exception: 'The clear width shall be permitted to... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `accessible-shower-check` | Accessible Shower Compartment Types (2010 ADA Standards 608.2) | US Department of Justice / US Access ...; 608.2.2 requires a standard roll-in compartment '30 inche... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `ada-ramp-slope` | ADA Ramp Slope, Runs, and Landings (IBC 1012 / ADA) | IBC 1012 / ADA; spec-v474 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `ada-stair-check` | ADA Stair Treads and Risers (2010 ADA Standards 504) | US Department of Justice / US Access ...; 504.2: 'Risers shall be 4 inches high minimum and 7 inche... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `advance-warning-sign-spacing` | Work Zone Advance Warning Sign Spacing (MUTCD 6C-1) | FHWA MUTCD; rural 500/500/500 -> signs at 500, 1,000, and 1,500 ft up... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `allowable-area` | Allowable Building Area per Story (IBC Chapter 5) | IBC 2021 §506.2 / §506.3 and Table 506.2; spec-v251 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `anchor-embedment` | Anchor Bolt Embedment | Project (ACI 318-19 17.6.2 breakout, ...; 5000 lb uplift / 5/8 in bolt / 3000 psi concrete -> 2.66 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `annular-velocity-cleaning` | Annular Velocity and Hole Cleaning | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `anode-bed-resistance` | Impressed Current Anode Bed Resistance | Project (first-principles); a Wenner survey and the CP designer govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `asce-live-load-reduction` | Live Load Reduction (ASCE 7 Ch. 4) | ASCE 7 minimum design loads; spec-v803 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `asce7-load-combinations` | ASCE 7 ASD Load Combinations: Governing Demand and Net Uplift | ASCE 7 §2.4.1 basic ASD combinations; spec-v225 section 2.1 pinned example (roof uplift) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `asphalt-tack-coat-quantity` | Asphalt Tack / Prime Coat Quantity | Coverage identity / DOT tack-coat pra...; area = 10000/9 = 1,111.1 sy; undiluted = 0.04/0.60 = 0.06... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `at-rest-earth-pressure` | At-Rest Earth Pressure on a Braced Wall (Jaky K0) | Jaky (1944) as compiled in Das / NAVF...; spec-v624 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `atterberg-indices` | Atterberg Plasticity Indices and A-Line Classification | Atterberg limits / USCS A-line; spec-v328 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `attic-ventilation` | Attic Ventilation Net Free Area | IRC R806 attic-ventilation rule; spec-v98 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `blast-burden-spacing` | Blast Pattern Burden, Spacing, and Stiffness Ratio | Project (first-principles); published pattern ratio ranges | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `blast-powder-factor` | Blasting Powder Factor and Explosive Load per Hole | Project (first-principles); the blaster in charge and the site blast plan govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `blast-scaled-distance-ppv` | Blast Vibration Scaled Distance and Peak Particle Velocity | Project (first-principles); site constants entered; a seismograph regression is the d... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `bolt-shear-bearing` | Bolt Shear + Bearing / Tearout Strength (AISC 360 J3) | AISC 360-22 J3.6 / J3.10; spec-v267 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `buffer-stroke-speed` | Elevator Buffer Stroke and Impact Speed | Project (first-principles); ASME A17.1 retardation limits named | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `cantilever-beam` | Cantilever Beam Moment, Shear, and Deflection | Roark / AISC beam diagrams; spec-v341 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `casing-cement-volume` | Well Casing and Annulus Cement Volume | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `cathodic-anode-count-life` | Cathodic Protection Anode Count and Life | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `cement-board-takeoff` | Cement Board (Tile Backer) Sheet and Screw Takeoff | cement-board takeoff (ANSI A108 / TCNA); sheets = ceil(120*1.10/15) = ceil(8.8) = 9; screws = 9*35... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `cipp-liner-thickness` | CIPP Liner Thickness for Groundwater Buckling (ASTM F1216) | Project (first-principles); the fully deteriorated case is a different relation and i... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `cmu-shear-wall` | Reinforced CMU Shear Wall In-Plane Shear (TMS 402 ASD) | TMS 402-16 (ACI 530 / ASCE 5) via Mas...; spec-v270 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `cmu-wall-axial` | Reinforced CMU Wall Axial Compression (TMS 402 ASD) | TMS 402-16 (ACI 530 / ASCE 5) via Mas...; spec-v271 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `cmu-wall-flexure` | Reinforced CMU Wall Out-of-Plane Flexure (TMS 402 ASD) | TMS 402-16 (ACI 530 / ASCE 5) via Mas...; spec-v269 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `coating-breakdown-factor` | Coating Breakdown Factor and CP Current Growth | Project (first-principles); DNV-RP-B401 and the CP designer govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `compound-miter` | Compound Miter (Crown Molding) | First-principles compound-miter trigo...; 38 deg spring crown at a 90 deg corner -> 31.62 deg miter... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `concrete` | Concrete Volume | Project (first-principles); Volume identity; 10% waste typical | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `concrete-anchor-interaction` | Concrete Anchor Tension-Shear Interaction (ACI 318-19 17.8) | ACI 318-19; Nua 6,000 / phiNn 9,831 (0.610), Vua 3,000 / phiVn 5,112 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `concrete-beam-min-flexural-steel` | Minimum Flexural Reinforcement As,min (ACI 318-19 9.6.1.2) | ACI 318-19 9.6.1.2; spec-v394 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `concrete-bearing-strength` | Concrete Bearing Strength (ACI 318-19 22.8) | ACI 318-19 22.8.3; spec-v490 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `concrete-crack-control-spacing` | Crack-Control Bar Spacing (ACI 318-19 24.3.2) | ACI 318-19 24.3.2; spec-v395 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `concrete-cracking-moment` | Concrete Cracking Moment Mcr (ACI 318-19) | ACI 318-19 (Mcr = fr Ig/yt, 19.2.3 mo...; spec-v651 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `concrete-depth-for-cracking-moment` | Concrete Section Depth for a Target Cracking Moment | ACI 318-19 (Mcr = fr b h^2/6, solved ...; spec-v752 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `concrete-effective-inertia` | Concrete Effective Moment of Inertia Ie (ACI 318-19 Bischoff) | ACI 318-19 24.2.3.5 (Bischoff effecti...; Mcr = 31.62 kip-ft; Ma 60 > (2/3)Mcr so cracked; Ie = 401... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `concrete-elastic-modulus` | Concrete Modulus of Elasticity Ec (ACI 318-19 19.2.2) | ACI 318-19 19.2.2; spec-v378 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `concrete-immediate-deflection` | Concrete Immediate Deflection from Ie (ACI 318-19) | ACI 318-19 §24.2.3; Ec = 57000 sqrt(4000) = 3,604,997 psi = 3605 ksi. w = 0.8... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `concrete-longterm-defl` | Long-Term Deflection Multiplier (ACI 318-19 24.2.4) | ACI 318-19 24.2.4.1.1; spec-v497 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `concrete-maturity` | Concrete Maturity and Equivalent Age (ASTM C1074) | ASTM C1074 maturity method (Nurse-Sau...; spec-v476 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `concrete-mix-design` | Concrete Mix Design (Simplified) | ACI; wc = 0.57 (non-air-entrained, 4000 psi); water = 325 lb/y... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `concrete-modulus-of-rupture` | Concrete Modulus of Rupture fr (ACI 318-19 19.2.3) | ACI 318-19 19.2.3; spec-v379 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `concrete-premix-bags` | Bagged Concrete Count for a Small Pour | Bagged (premix) concrete count for a ...; volume = 4 x 4 x 4/12 = 5.333; bags = ceil(5.333 x 1.10 /... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `concrete-pump-line-pressure` | Concrete Pump Line Pressure and Reach Limit | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `concrete-shrinkage-temperature-steel` | Shrinkage and Temperature Reinforcement (ACI 318-19 24.4) | ACI 318-19 24.4; spec-v380 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `concrete-strength-from-modulus` | Concrete f'c from Modulus of Elasticity (ACI 318-19 19.2.2) | ACI 318-19 19.2.2; Ec 3,644,147 psi at 145 pcf -> ~4,000 psi | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `concrete-strength-from-rupture` | Concrete f'c from Modulus of Rupture (ACI 318-19 19.2.3) | ACI 318-19 19.2.3; fr 474.342 psi, lambda 1.0 -> ~4,000 psi | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `concrete-torsion-threshold` | Concrete Threshold and Cracking Torsion (ACI 318-19 22.7) | ACI 318-19 §22.7; spec-v447 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `concrete-yield` | Concrete Yield and Relative Yield (ASTM C138) | ASTM C138 / AASHTO T121 (concrete yield); 3993 lb batch at 148 lb/ft3, 1.0 yd3 design, 564 lb cemen... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `consolidation-degree` | Consolidation Degree from Elapsed Time (Terzaghi) | Terzaghi consolidation theory (invert...; spec-v645 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `consolidation-time-rate` | Consolidation Time Rate (Terzaghi) | Terzaghi consolidation theory; spec-v414 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `control-joint-spacing` | Concrete Control Joint Spacing | ACI 302.1R / 360R slab-on-ground guid...; spec-v96 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `corroded-pipe-b31g` | Corroded Pipe Remaining Strength (ASME B31G) | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `coulomb-earth-pressure` | Coulomb Active Earth Pressure (Wall Friction and Batter) | Coulomb (1776) as compiled in Das / N...; spec-v628 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `countertop-overhang-support` | Stone Countertop Overhang Support Check | stone-industry support guidance; 12-in overhang, 36-in depth, 3 cm, 8-ft run, 170 pcf -> t... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `counterweight-balance` | Elevator Counterweight Balance and Unbalanced Load | Project (first-principles); ASME A17.1 governs traction | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `crusher-reduction-ratio` | Crusher Reduction Ratio and Circuit Stage Check | Project (first-principles); machine ranges entered; the manufacturer's data governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `diaphragm-shear` | Wood Diaphragm Unit Shear and Chord Force (SDPWS) | AWC SDPWS flexible-diaphragm model (A...; spec-v272 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `door-clear-width` | Door Clear Width, Thresholds, and Doors in Series (2010 ADA Standards 404.2) | US Department of Justice / US Access ...; 404.2.3: 'Door openings shall provide a clear width of 32... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `door-closer-opening-force` | Door Closer Size, Opening Force, and Where the Overage Is | Project (first-principles); limits entered from the adopted accessibility and buildin... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `door-closing-energy` | Elevator Door Closing Kinetic Energy and Force | Project (first-principles); ASME A17.1 closing energy limits; both entered from the code | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `door-maneuvering-clearance` | Door Maneuvering Clearance, Front Approach (ADA 404.2.4.1) | US Access Board / Department of Justice; 'Maneuvering space on the pull side extends 18 inches min... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `dryer-duct-length` | Dryer Exhaust Duct Developed Length (IRC M1502.4.6) | International Code Council; 'The maximum length of the exhaust duct shall be 35 feet ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `drywall` | Drywall Sheet Count and Mud | Project (first-principles); sheets = ceil(1.10 * 1800 / 32) = 62; mud = 1800 / 70 = 2... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `dust-collector-air-to-cloth` | Dust Collector Air-to-Cloth Ratio and Bag Derate | Project (first-principles); ranges entered as broad conventions | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `egress-capacity` | Egress Exit Count and Required Width (IBC 1005.3 / 1006.2) | IBC 2021 §1005.3 / §1006.2 / §1010.1.1; spec-v243 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `egress-window-well` | Egress Window Well (IRC R310.4.1) | International Code Council; 'The minimum horizontal area of the window well shall be ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `elastic-settlement-allowable-pressure` | Allowable Bearing Pressure for a Settlement Limit | Theory-of-elasticity immediate settle...; 1 in limit, 6 ft footing, Es 250 ksf, nu 0.3, Is 0.82 -> ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `exterior-opening-protection` | Exterior Wall Opening Limit by Fire Separation Distance (IBC Table 705.8) | IBC 2021 Table 705.8; spec-v253 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `fine-aggregate-grading` | Fine Aggregate Grading Check (ASTM C33) | ASTM C33/C33M §6 (fine-aggregate grad...; passing 100/98/85/68/45/18/5 -> retained 2/15/32/55/82/95... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `fineness-modulus` | Aggregate Fineness Modulus (ASTM C136) | ASTM C136 / C125 (aggregate fineness ...; cumulative retained 2/12/32/57/82/95 -> sum 280 -> FM 2.8... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `fire-door-clearance` | Fire Door Clearance Check (NFPA 80 Annual Inspection) | Project (first-principles); the assembly's own listing governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `footing-eccentric-pressure` | Eccentric Footing Bearing Pressure and Kern Check | Eccentric footing bearing pressure (k...; spec-v309 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `gas-pipeline-flow` | Gas Pipeline Flow (Weymouth and Panhandle A) | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `gate-operator-duty-cycle` | Slide Gate Operator Force, Grade, and Duty Cycle | Project (first-principles); UL 325 governs entrapment protection separately | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `glulam-volume-factor` | Glulam Volume Factor Cv (NDS 5.3.6) | NDS 2018 §5.3.6; spec-v448 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `gutter-downspout` | Gutter and Downspout Sizing | SMACNA / standard residential gutter ...; spec-v98 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `hdd-bend-radius` | Directional Bore Minimum Bend Radius and Entry Setback | Project (first-principles); the pipe manufacturer and the engineer of record govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `hdd-fluid-volume` | HDD Drilling Fluid Volume, Bentonite, and Spoil | Project (first-principles); the mud engineer and the disposal jurisdiction govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `hoistway-venting` | Hoistway Smoke Venting and Pressurization Screen | Project (first-principles); NFPA 92 and the adopted building code named | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `hoop-stress-mawp` | Thin-Wall Vessel Max Allowable Working Pressure | thin-wall / Barlow (inverse); spec-v668 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `hoop-stress-thin-wall` | Thin-Wall Pressure Vessel Hoop and Longitudinal Stress | thin-wall / Barlow; spec-v361 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `instant-off-ir-drop` | Instant-Off Potential and IR Drop | Project (first-principles); NACE SP0169 governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `intersection-sight-triangle` | Intersection Sight Triangle and Departure Distance | Project (first-principles); the AASHTO Green Book, the agency design standards and th... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `joist-cantilever-check` | Joist / Deck Cantilever Ratio Check (IRC R507.6) | IRC 2021 Table R507.6 deck-joist cant...; 2x10 southern pine at a 10 ft backspan: Table R507.6 2-6 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `lateral-earth-pressure` | Lateral Earth Pressure and Thrust (Rankine) | Rankine (1857) as compiled in Das / N...; spec-v261 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `liquefaction-screening` | Liquefaction Triggering Screening (Seed-Idriss CSR) | Seed-Idriss; spec-v416 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `masonry-limited-access-zone` | Masonry Limited Access Zone and Bracing (OSHA 1926.706) | Occupational Safety and Health Admini...; 'The limited access zone shall be equal to the height of ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `membrane-fastener-takeoff` | Mechanically Attached Single-Ply Fastener and Plate Takeoff | mechanically attached single-ply geom...; Usable width = 10 - 6/12 = 9.5 ft, so seam length = 8,000... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `metal-roof-panels` | Metal Roof Panels, Linear Feet, and Fasteners | MCA / MRA install references + manufa...; spec-v216 section 2.1 pinned example (exposed-fastener ag... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `pier-scour-depth` | Local Scour Depth at a Bridge Pier | Project (first-principles); HEC-18 and the bridge hydraulic study govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `pile-group-efficiency` | Pile Group Efficiency (Converse-Labarre) | Converse-Labarre pile-group efficiency; spec-v498 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `pile-group-spacing-for-efficiency` | Pile Group Spacing for a Target Efficiency | Converse-Labarre pile-group efficienc...; spec-v748 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `pipeline-mao-barlow` | Pipeline Maximum Allowable Operating Pressure (Barlow) | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `pipeline-potential-attenuation` | Pipeline CP Potential Attenuation | Project (first-principles); the pipeline's close interval survey governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `polarization-decay-criterion` | 100 mV Polarization Decay Criterion | Project (first-principles); NACE SP0169 governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `pole-embedment-depth` | Embedded Post / Pole Depth for Lateral Load (IBC 1807.3) | IBC; P 200 lb at h 4 ft, b 0.5 ft, 150 psf/ft, isolated (x2), ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `rain-load-ponding` | Roof Rain Load and Secondary-Drainage Flow (ASCE 7 Ch. 8) | ASCE 7 Ch. 8 + IPC drainage; spec-v224 section 2.1 pinned example (typical scupper) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `rc-beam-flexure` | Reinforced Concrete Beam Flexural Capacity (ACI 318-19) | ACI 318-19 (Building Code Requirement...; spec-v257 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `rc-beam-shear` | Reinforced Concrete Beam Shear and Stirrup Spacing (ACI 318-19) | ACI 318-19 (Building Code Requirement...; spec-v258 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `rc-column-axial` | RC Tied Column Axial Capacity (ACI 318-19 22.4) | ACI 318-19 22.4.2 / 22.4.2.1; spec-v284 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `rc-column-steel-for-load` | RC Column Longitudinal Steel for a Target Load | ACI 318-19 22.4.2 (solved for the steel); spec-v753 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `rc-slab-min-thickness` | One-Way Slab / Beam Minimum Thickness for Deflection (ACI 318-19) | ACI 318-19 Table 7.3.1.1 / 9.3.1.1; spec-v299 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `rc-slender-column-magnify` | Slender Column Moment Magnifier, Nonsway (ACI 318-19 6.6.4) | ACI 318-19 Section 6.6.4.5 (nonsway m...; Pu 200, M2 80, M1 -50 (single curvature, ACI 318-19 sign)... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `reach-range` | Reach Ranges for Outlets and Controls (2010 ADA Standards 308) | US Department of Justice / US Access ...; 308.3.2: 'The high side reach shall be 48 inches maximum ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `rebar-lap-splice` | Rebar Lap-Splice Length | ACI 318 development-and-splice basis ...; spec-v96 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `required-section-modulus` | Required Plastic Section Modulus for a Steel Beam | AISC 360-22 Chapter F (Mp = Fy Zx, in...; spec-v634 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `retaining-wall-stability` | Cantilever Retaining Wall Stability (Overturning / Sliding / Bearing) | Das / NAVFAC DM-7.02 stability checks...; spec-v262 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `reverse-curve` | Reverse (S) Curve Between Parallel Tangents | Ghilani & Wolf, Elementary Surveying ...; I = arccos(1 - 60/1000) = arccos(0.94) = 19.9484 deg; T =... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `revolving-door-throughput` | Revolving Door and Turnstile Peak Throughput | Project (first-principles); the adopted codes govern egress credit | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `ridge-cap-fasteners` | Hip / Ridge Cap Bundles and Roofing Nails by the Pound | IRC R905.2.6 asphalt-shingle fastenin...; spec-v217 section 2.1 pinned example (24-square, 40 ft ri... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `scaffold-mudsill-bearing` | Scaffold Mudsill Bearing Pressure and Sill Length | Bearing-pressure identity / OSHA 1926...; area = 9.25*24/144 = 1.542 ft^2; bearing = 4000/1.542 = 2... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `sediment-basin-volume` | Sediment Basin / Trap Storage Volume | Sediment-basin storage identity (cons...; required = 5*3600 = 18,000 cf (667 cy); surface = 18000/3... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `seismic-base-shear` | Seismic Base Shear (ASCE 7 §12.8 Equivalent Lateral Force) | ASCE 7 §12.8 equivalent lateral force; spec-v226 section 2.1 pinned example (short period) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `seismic-design-spectral-acceleration` | Design Spectral Response Accelerations SDS / SD1 (ASCE 7-16 11.4) | ASCE 7-16 11.4; spec-v381 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `seismic-earth-pressure` | Mononobe-Okabe Seismic Active Earth Pressure | Mononobe-Okabe (Okabe 1926; Mononobe ...; psi = arctan(0.15) = 8.5308 deg; Kae = 0.34053 (the publi... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `seismic-overturning-moment` | Seismic Overturning Moment (ASCE 7 §12.8.5) | ASCE 7-22 §12.8.5 (Overturning) with ...; spec-v480 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `seismic-overturning-stability` | Seismic Overturning Stability Ratio at the Foundation | American Society of Civil Engineers; Continues the seismic-overturning-moment tile's own 3-sto... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `seismic-pdelta-stability` | Seismic P-Delta Stability Coefficient (ASCE 7-22 12.8.7) | ASCE 7-22 12.8.7; spec-v383 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `seismic-story-drift` | Seismic Design Story Drift and Allowable Limit (ASCE 7-22 12.8.6 / 12.12) | ASCE 7-22 12.8.6 / 12.12; spec-v382 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `seismic-vertical-distribution` | Vertical Distribution of Seismic Forces (ASCE 7 §12.8.3) | ASCE 7-22 §12.8.3 / §12.8.4 (Eqs. 12....; spec-v477 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `settlement-limit-load` | Allowable Load for a Settlement Limit (NC Clay) | Terzaghi primary consolidation solved...; spec-v648 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `shaft-diameter-for-torsion` | Solid Shaft Diameter for an Allowable Torsion | mechanics of materials (solid shaft, ...; spec-v747 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `shaft-torsion` | Shaft Torsional Shear Stress and Angle of Twist | mechanics of materials; spec-v359 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `shear-flow-connector-spacing` | Shear Flow and Connector Spacing (Built-Up Beam) | Mechanics of materials (shear flow q ...; Q = 6 x 8 = 48 in^3; q = 50 x 48 / 800 = 3.0 kip/in; s = ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `shear-stud-strength` | Composite Shear Stud Strength and Count (AISC 360-22 I8) | AISC 360-22 I8; spec-v411 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `shearwall-deflection` | Wood Shear Wall Deflection (SDPWS Eq 4.3-1) | AWC SDPWS Equation 4.3-1 (AWC/APA des...; spec-v274 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `shore-post-load` | Formwork Shore Post Load and Spacing (ACI 347) | ACI 347 Guide to Formwork for Concrete; spec-v245 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `slab-dowel-schedule` | Slab Load-Transfer Dowel Schedule (ACI 302) | Slab load-transfer dowel schedule (AC...; per joint = floor((40*12-2*6)/12)+1 = floor(468/12)+1 = 3... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `sliding-snow-load` | Sliding Snow Load on a Lower Roof (ASCE 7 7.9) | ASCE 7 §7.9; spec-v469 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `slope-failure-depth-for-fs` | Infinite Slope Critical Depth for a Target FS | Infinite-slope stability (Das / NAVFA...; spec-v749 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `sloped-backfill-earth-pressure` | Sloped-Backfill Earth Pressure (Rankine Inclined Surface) | Rankine sloped-backfill as compiled i...; spec-v626 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `snow-drift-load` | Snow Drift Surcharge at a Roof Step or Parapet (ASCE 7 Ch. 7) | ASCE 7-22 Chapter 7; spec-v297 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `soil-activity` | Soil Activity (Skempton) | Skempton (1953) soil activity; Skempton 1953 activity definition and inactive/normal/act... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `soil-bearing-capacity` | Shallow Foundation Bearing Capacity (Vesic) | Das, Principles of Foundation Enginee...; spec-v260 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `soil-consolidation-settlement` | Primary Consolidation Settlement (NC Clay) | Terzaghi primary consolidation (Das /...; spec-v308 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `soil-permeability` | Soil Permeability (Hydraulic Conductivity from a Permeameter Test) | Darcy's-law constant-head permeameter...; k = 250 x 12 / (78.5 x 30 x 60) = 3000 / 141300 = 0.02123... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `soil-phase-relations` | Soil Phase Relations (Void Ratio, Porosity, Saturation) | Soil phase relations (Das / NAVFAC); spec-v327 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `soil-relative-density` | Relative Density (Density Index) of a Cohesionless Soil | Relative density (density index), dry...; spec-v1014 section 2.1; cross-checked against the void-ra... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `soil-settlement-elastic` | Shallow Foundation Elastic (Immediate) Settlement | Theory-of-elasticity immediate settle...; spec-v287 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `soil-vertical-effective-stress` | Soil Vertical Total and Effective Stress (Terzaghi) | Terzaghi effective-stress principle (...; spec-v1013 section 2.1; buoyant cross-check 120x10 + (125... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `spt-bearing-capacity` | SPT Allowable Bearing on Sand (Meyerhof) | Meyerhof / Das; spec-v415 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
...
| `stair-stringer` | Stair Stringer Length | Project (first-principles); 9 ft rise / 12 ft run -> 180 in stringer (15 ft); 30 BF a... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `steel-b2-amplifier` | Beam-Column Sidesway Moment Amplifier B2 (AISC 360 App. 8) | AISC 360-22 Appendix 8.2.2 sidesway m...; RM=1-0.15*(1200/2000)=0.91; Pe=0.91*100*(14*12)/0.5=30576... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `steel-beam-flexure` | Steel Beam Flexural Capacity (AISC 360 Ch. F, Compact + Braced) | AISC 360-22 Chapter F / Steel Constru...; spec-v254 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-beam-ltb` | Steel Beam Lateral-Torsional Buckling (AISC 360 F2) | AISC 360-22 Section F2 (AISC Manual W...; spec-v281 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `steel-beam-shear` | Steel Beam Web Shear Capacity (AISC 360 Ch. G) | AISC 360-22 Chapter G / Steel Constru...; spec-v255 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-block-shear` | Steel Block Shear Rupture (AISC 360 J4.3) | AISC 360-22 Section J4.3; spec-v282 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-bolt-slip-critical` | Slip-Critical Bolt Design Strength (AISC 360 J3.8) | AISC 360-22 J3.8 / Table J3.1; spec-v294 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-bolt-tension-shear` | Bolt Combined Tension and Shear (AISC 360 J3.7) | AISC 360-22 J3.7; spec-v316 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-camber` | Steel Beam Camber from Dead-Load Deflection | AISC / fabrication practice; spec-v413 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `steel-column-capacity` | Steel Column Compressive Capacity (AISC 360 Ch. E, Flexural Buckling) | AISC 360-22 Chapter E / Steel Constru...; spec-v256 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `steel-effective-length-k` | Column Effective Length Factor K (Alignment Chart) | AISC alignment chart (Dumonteil fit); spec-v315 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-fillet-weld-size` | Fillet Weld Size Limits and Effective Throat (AISC 360 J2.2b) | AISC 360-22 Table J2.4 / J2.2b; spec-v295 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `steel-h1-interaction` | Steel Beam-Column Combined Axial and Flexure (AISC 360 H1.1) | AISC 360-22 H1.1; spec-v314 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-inertia-for-deflection` | Required Moment of Inertia for a Deflection Limit | AISC / mechanics of materials (solved...; spec-v754 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `steel-tau-b-stiffness-reduction` | Direct-Analysis Stiffness Reduction tau_b (AISC 360 C2.3) | AISC 360-22 Section C2.3 (direct-anal...; Py=50*14.4=720; ratio=1.0*400/720=0.5556; tau_b=4*0.5556*... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-tension-member` | Steel Tension Member: Yield and Rupture with Shear Lag (AISC 360 D2/D3) | AISC 360-22 Chapter D (D2 / D3, Table...; spec-v283 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `steel-web-local-strength` | Steel Web Local Yielding and Crippling (AISC 360 J10) | AISC 360-22 J10.2 / J10.3; spec-v293 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `superelevation` | Superelevation / Min Curve Radius (AASHTO) | AASHTO Green Book; spec-v335 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `superelevation-safe-curve-speed` | Safe Curve Speed from Radius and Superelevation | AASHTO Green Book (solved for speed); R 1500 ft, e 0.08, f 0.11 (the AASHTO side-friction facto... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `t-beam-effective-flange-width` | T-Beam Effective Flange Width (ACI 318-19 6.3.2) | ACI 318-19 6.3.2; spec-v393 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `tank-bottom-anode-layout` | Tank Bottom Cathodic Protection Anode Layout | Project (first-principles); API RP 651 and the ribbon rating govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `thermal-stress-max-deltat` | Max Temperature Change for a Stress Limit | mechanics of materials (inverse); spec-v674 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `thermal-stress-restrained` | Restrained Thermal Stress and Force | mechanics of materials; spec-v360 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `thinset-coverage` | Thin-Set Mortar Coverage | Manufacturer thin-set coverage charts...; spec-v95 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `tilt-up-brace-load` | Tilt-Up Panel Temporary Brace Load and Count | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `tilt-up-lift-stress` | Tilt-Up Panel Lifting Stress and Insert Layout | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `traction-roping-ratio` | Elevator Traction Roping Ratio, Torque, and Rope Tension | Project (first-principles); ASME A17.1 and the equipment manufacturer named | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `traffic-taper-length` | Work-Zone Merging Taper Length and Device Count (MUTCD) | MUTCD merging-taper identity (public-...; S >= 45: L = 12*55 = 660 ft; devices = ceil(660/40)+1 = 1... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `vertical-curve-sight-distance` | Crest Vertical Curve Length for SSD (AASHTO) | AASHTO Green Book; spec-v336 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `water-cement-ratio` | Water-Cementitious Ratio and Exposure Cap (ACI 318) | ACI 318 Table 19.3.2.1; ACI 211.1 (wa...; 282 lb water / (470 cement + 94 fly ash = 564) = 0.50, ex... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `wind-cc-pressure` | Wind Components and Cladding Pressure (ASCE 7 Ch. 30) | ASCE 7-22 Chapter 30; spec-v296 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `wind-mwfrs-pressure` | MWFRS Wall Pressure (ASCE 7 Ch. 27) | ASCE 7-22 Chapter 27; spec-v298 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `wind-velocity-pressure-exposure-coefficient` | Velocity Pressure Exposure Coefficient Kz (ASCE 7-22 §26.10) | ASCE 7-22 §26.10.1 velocity pressure ...; Kz = 2.41*(50/2460)^(2/9.8) = 2.41*(0.020325)^0.20408 = 2... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `wood-beam-bending` | Wood Bending Member (NDS Beam Stability Factor CL and Adjusted Fb') | NDS 3.3.3 (National Design Specificat...; spec-v263 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `wood-beam-shear` | Wood Bending Member Shear (fv and the NDS Tension-Side End-Notch Reduction) | NDS 3.4.2 / 3.4.3.2 (National Design ...; spec-v264 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `wood-bearing-perpendicular` | Wood Bearing Perpendicular to Grain (NDS 3.10) | NDS 2018 3.10.2 / 3.10.4; spec-v290 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `wood-bolt-connection` | Single-Shear Bolted / Dowel Lateral Design Value (NDS Yield-Limit Z) | NDS Table 12.3.1A (National Design Sp...; spec-v265 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `wood-lag-withdrawal` | Lag Screw Withdrawal Design Value (NDS 12.2.1) | NDS 2018 12.2.1; spec-v333 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `fall-arrest-clearance` | Required Fall-Arrest Clearance (ANSI Z359) | ANSI/ASSP Z359.13 / OSHA 1926 Subpart M; free fall 6 + deceleration 4 (Z359.13 48 in for a 6 ft fr... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `fire-pump-curve` | Fire Pump Rated / Churn / Overload Curve Check (NFPA 20) | NFPA 20 (Standard for the Installatio...; spec-v248 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
...
| `hydrant-available-flow` | Hydrant Rated Flow at 20 psi (NFPA 291) | NFPA 291; spec-v389 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `jockey-pump-sizing` | Jockey (Pressure-Maintenance) Pump Sizing (NFPA 20) | jockey pump sizing (NFPA 20); jockey = max(0.01*750, 1) = 7.5; stop = 120+50 = 170; sta... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `required-fire-flow` | Required Fire Flow | ISO / NFPA 1142 area-based simplified...; 5000 ft^2 / ordinary / occupancy 1.0 / no exposure or com... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `smooth-bore-diameter-for-flow` | Smooth-Bore Tip Diameter for a Target Flow | IFSTA Pumping Apparatus Driver/Operat...; spec-v738 section 2.1 pinned example (250 gpm, 50 psi -> ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `smooth-bore-flow` | Smooth-Bore Nozzle Flow (GPM) | IFSTA Pumping Apparatus Driver/Operat...; spec-v114 section 2.1 pinned example (1.125 in, 50 psi ->... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `sprinkler-pressure-demand` | Sprinkler Pressure Demand at the Base of Riser (NFPA 13) | NFPA 13 (Standard for the Installatio...; spec-v479 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `belt-conveyor-tension-power` | Belt Conveyor Effective Tension and Drive Power | CEMA; Te = f L (2 Wb + Wm + Wrot) + H Wm | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `belt-hp-transmitted` | Belt Power from Tension and Speed | belt-drive power relation (Machinery'...; spec-v807 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `bolt-proof-load` | Bolt Proof, Yield, and Tensile Load (SAE J429) | SAE J429 (ASME B1.1 tensile stress area); spec-v503 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `casting-pour-yield` | Casting Pour Weight, Gating Yield, and Melt Energy | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `cone-flat-pattern` | Cone Flat-Pattern Development (Radial Line) | sheet-metal radial-line layout; spec-v400 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `corrosion-rate-weight-loss` | Corrosion Rate from Coupon Weight Loss | Project (first-principles); ASTM G1 and the coupon's pit depth govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `extrusion-output-rate` | Extrusion Output Rate, Line Speed, and Draw-Down | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `galvanic-area-ratio` | Galvanic Corrosion and Area Ratio | Project (first-principles); the materials engineer governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `hdpe-fusion-pressure-time` | HDPE Butt Fusion Gauge Pressure and Drag | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `injection-clamp-tonnage` | Injection Mould Clamp Tonnage From Projected Area | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `mold-shrinkage-dimension` | Moulded Part Shrinkage and the Cavity Dimension to Cut | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `plating-tank-current` | Electroplating and Anodizing Tank Current and Time | Project (first-principles); mass rate = I x AW / (n x 96,485) x efficiency | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `plume-rise-briggs` | Stack Plume Rise and Effective Stack Height (Briggs) | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `press-fit-pressure` | Interference Press-Fit Pressure and Holding Force (Lame) | Lame interference-fit model (Machiner...; spec-v511 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `pyramid-frustum-volume` | Truncated Pyramid (Rectangular Frustum) Volume | Project (first-principles); V=(h/3)(A1+A2+sqrt(A1 A2)) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `quench-severity` | Quench Severity, Biot Number, and the Agitation Screen | Project (first-principles); Bi = 2 H r | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `radiant-heat-exchange` | Radiant Heat Exchange (Stefan-Boltzmann) | Stefan-Boltzmann law (SI cross-checked); Ts = 659.67 R, Tsurr = 529.67 R; q = 0.9 x 1 x 0.1714e-8 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `rainwater-catchment-area` | Catchment Area for a Target Harvest | ARCSA / NOAA (inverse); 11,593 gal target, 30 in, 0.62 efficiency -> about 1,000 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `thermoforming-draw-ratio` | Thermoforming Draw Ratio and Wall Thinning | Project (first-principles); the standards named in the tile citation govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `tolerance-stack-rss` | Tolerance Stack-Up: Worst-Case and RSS | mechanical design / GD&T; spec-v399 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `wind-chill-wind-speed` | Wind Speed from Wind Chill and Temperature | NWS (2001 formula, solved for wind sp...; spec-v758 pinned example: 5 F air, -19 F wind chill -> ~2... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `bridge-formula-min-spacing` | Bridge Formula Minimum Axle Spread | FHWA; Figure 6: 80,000 lb on 5 axles is the Bridge Table row fo... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
...
| `driver-pay-cpm-vs-percentage` | Driver Pay: Cents-per-Mile vs Percentage | carrier settlement practice; spec-v424 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `invoice-factoring-cost` | Invoice Factoring Cost and Effective APR | freight-factoring practice; spec-v425 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `stopping-sight-distance` | Stopping Sight Distance (AASHTO) | Illinois DOT (AASHTO Green Book values); 55 mph, t 2.5 s, a 11.2 ft/s2 (f = 11.2/32.2 = 0.348): br... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `aircraft-weight-balance` | Aircraft Weight and Balance (CG Envelope) | station-moment weight and balance (FA...; spec-v516 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `anchor-rode-scope` | Anchor Rode Scope and Swing Radius | anchor rode scope and swing radius (s...; spec-v505 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `ballnose-scallop-height` | Ballnose Milling Scallop Height from Stepover | Ballnose scallop geometry; spec-v319 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `bearing-defect-frequencies` | Rolling-Element Bearing Defect Frequencies | Project (first-principles); the bearing manufacturer published frequencies and a qual... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `bearing-equivalent-load` | Rolling-Bearing Dynamic Equivalent Load P (ISO 281) | Project (first-principles); P = X Fr + Y Fa | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `bearing-l10-life` | Rolling-Bearing L10 Rating Life (ISO 281) | ISO 281 basic rating life; spec-v504 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `bearing-max-load` | Max Bearing Load for a Target L10 Life | ISO 281 basic rating life (inverse); spec-v672 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `boring-bar-deflection` | Boring Bar / Tool Overhang Deflection and L/D Limit | Cantilever tool deflection; spec-v318 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `boring-bar-max-overhang` | Boring Bar Max Overhang for a Deflection Limit | Cantilever tool deflection (inverse); 0.75 in steel bar, 100 lb, 0.01545 in allowable -> 6.0 in... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `brake-pedal-hydraulic` | Brake Pedal Ratio and Line Pressure | hydraulic brake force chain (Pascal's...; spec-v514 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `countersink-diameter-from-depth` | Countersink Diameter from a Plunge Depth | Machinery's Handbook countersinking (...; spec-v733 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `coupling-alignment-tolerance` | Coupling Alignment Tolerance by Speed | Project (first-principles); the plant precision maintenance standard governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `crosswind-component` | Crosswind and Headwind Component | runway wind-component resolution (FAA...; spec-v501 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `dynamic-compression-ratio` | Dynamic Compression Ratio | dynamic compression ratio (slider-cra...; Vc = 47.833/(10.5-1) = 5.035; piston@IVC = 3.036; Veff = ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `endurance-limit-marin` | Corrected Endurance Limit (Shigley Marin Factors) | Project (first-principles); Se = ka kb kc kd ke Se | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `gear-identification` | Gear Identification (Pitch from Teeth and OD) | Machinery's Handbook / AGMA (inverse ...; spec-v649 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `glidepath-descent-rate` | Glidepath Rate of Descent | FAA Instrument Flying Handbook; TERPS...; 120 kt on a 3.0 deg glidepath -> 637 ft/min, 318 ft/nm (T... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `grinding-wheel-rpm` | Grinding Wheel Surface Speed and Max Safe RPM | grinding wheel surface-speed identity...; max_rpm = 6500*12/(pi*7) = 3547; actual_sfpm = pi*7*3450/... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `hull-speed` | Displacement Hull Speed and Speed/Length Ratio | displacement hull-speed relation (Fro...; spec-v502 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `hydraulic-drive-flow-limit` | Hydraulic Flow Limit from Drive Power | fluid-power engineering (inverse); 13.73 drive HP, 2000 psi, 0.85 efficiency -> 10 GPM (roun... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `hydraulic-motor-torque-speed` | Hydraulic Motor Torque and Speed | fluid-power engineering; spec-v397 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `hydraulic-pump-flow` | Hydraulic Pump Output Flow | fluid-power engineering; spec-v642 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `hydraulic-pump-horsepower` | Hydraulic Pump Drive Horsepower | fluid-power engineering; spec-v396 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `inclined-plane-force` | Inclined Plane Push and Hold Force | Project (first-principles); F=W(sin+mu cos); slides above atan(mu)=16.7 deg | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `injector-max-hp` | Injector Max Horsepower Capacity | Fuel injector power capacity (inverse...; spec-v661 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `injector-size` | Fuel Injector Size from Horsepower, BSFC, and Duty Cycle | Fuel injector sizing (HP x BSFC / (n ...; spec-v323 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `keyseat-key-size` | Shaft Key and Keyseat Size (ANSI B17.1) | ANSI B17.1 Keys and Keyseats; spec-v513 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `max-rpm-from-piston-speed` | Max RPM from a Piston-Speed Limit | Mean piston speed (engine building), ...; spec-v660 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `mean-piston-speed` | Mean Piston Speed and RPM-Limit Reading | Mean piston speed (engine building); spec-v324 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `paint-mix-ratio` | 2K Paint Mix Ratio | Paint manufacturer technical data she...; spec-v100 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `reaming-drill-allowance` | Reaming Prebore (Drill) Allowance | machine-reaming stock allowance (Mach...; 0.5 in is in the 1/4-1/2 band (0.015); drill = 0.500 - 0.... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `receiver-pump-up-time` | Air Receiver Pump-Up and Pump-Down Time | Project (first-principles); the compressor and receiver manufacturers data and the pr... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `roller-chain-length` | Roller Chain Length in Pitches (ANSI B29.1) | ANSI B29.1 roller-chain length; spec-v512 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `sacrificial-anode-life` | Sacrificial Anode Service Life | Sacrificial-anode life by Faraday's l...; 5 lb zinc (354 A-h/lb), 0.85 utilization, 0.15 A -> 1.14 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `sailboat-performance-ratios` | Sailboat Performance Ratios (SA/D and DLR) | Sailboat performance ratios (SA/D and...; SA/D = 500 / (10000/64)^(2/3) = 500/29.0 = 17.24; DLR = (... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `shaft-alignment-rim-face` | Shaft Alignment Offset and Angularity (Rim-and-Face) | Project (first-principles); the machine manufacturer alignment specification governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `single-plane-field-balance` | Single-Plane Field Balance Trial Weight | Project (first-principles); the machine manufacturer balancing instructions and a qua... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `sprocket-pitch-diameter` | Sprocket Pitch Diameter (ANSI B29.1) | ANSI B29.1 sprocket geometry; spec-v801 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `spur-gear-geometry` | Spur Gear Tooth Geometry (Diametral Pitch) | Machinery's Handbook / AGMA; spec-v401 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `stub-acme-thread-depth` | Stub Acme (29-degree) Thread Depth and Dimensions | Stub Acme thread (Machinery's Handboo...; pitch = 1/5 = 0.200; depth = 0.3 x 0.200 = 0.060; pitch d... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `tailstock-setover` | Tailstock Setover for Taper Turning | Machinery's Handbook (Industrial Press); spec-v805 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `thick-wall-cylinder-stress` | Thick-Wall Cylinder Stress (Lame) | Project (first-principles); Lame thick-wall | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `tire-contact-patch` | Tire Contact Patch from Load and Pressure | first-order pneumatic-tire relation A...; spec-v808 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `toggle-mechanism-force` | Toggle Mechanism Clamping Force | Project (first-principles); F_out = F_in/(2 tan theta) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `torque-adapter-correction` | Torque Wrench Extension / Crowfoot Correction | Standard torque-adapter correction (S...; spec-v485 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `trap-speed-horsepower` | Horsepower from Quarter-Mile Trap Speed | Hale quarter-mile trap-speed relation; spec-v325 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `turbo-max-boost-for-charge-temp` | Max Boost Before a Charge-Air Temperature Limit | turbocharger charge-air-temperature m...; 250 F limit, 80 F inlet, 70% eff, 14.7 psia -> 14.8 psi m... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `turbo-pressure-ratio` | Turbocharger Pressure Ratio and Charge-Air Temp | turbocharger pressure-ratio and charg...; spec-v506 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `vibration-severity-zone` | Vibration Severity Zone (ISO 20816) | Project (first-principles); ISO 20816 and the plant condition monitoring programme go... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `waterline-for-hull-speed` | Waterline Length for a Target Hull Speed | displacement hull-speed relation (Fro...; 8 kn target displacement hull -> 35.6 ft waterline | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| tile_id | name | citation source | fixture |
| --- | --- | --- | --- |
| `anhydrous-ammonia-rate` | Anhydrous Ammonia Rate from Target Nitrogen | anhydrous ammonia rate (82-0-0); product = 180/0.82 = 219.5 lb; gal/ac = 219.5/5.15 = 42.6... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `cattle-heart-girth-weight` | Cattle Live Weight from Heart Girth | Cattle live weight from heart girth (...; weight = 70^2 x 55 / 300 = 4900 x 55 / 300 = 269,500 / 30... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `cattle-stocking-rate` | Cattle Stocking Rate (AUM) | USDA NRCS; available = 1500*160*0.40 = 96,000 lb; AUMs = 96,000/790 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `corn-yield-estimate` | Pre-Harvest Corn Yield (Yield Component Method) | Pre-harvest corn yield (yield compone...; kernels/ear = 16 x 35 = 560; bu/ac = 32 x 560 / 90 = 17,9... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `drawbar-pull` | Tractor Drawbar Pull from Power | ASABE (inverse); 75 PTO hp at 4.5 mph on firm soil (0.72) -> 54 drawbar hp... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `dressing-percentage` | Carcass Dressing Percentage and Freezer Yield | University of Wisconsin-Madison Divis...; p. 2: 1200 lb x 62% = 744 lb hot carcass; 744 x (100% - 3... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `drip-zone-flow` | Drip Zone Flow and Valve Capacity | Irrigation Association low-volume / m...; spec-v209 section 2.1 pinned example (inline dripline) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `fan-pad-evaporative-cooling` | Greenhouse Fan and Pad Evaporative Cooling | Project (first-principles); pad manufacturer data and the design wet bulb govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `fertigation-injection-rate` | Fertigation / Chemigation Injection Rate | Fertigation / chemigation injection rate; total = 5*40 = 200; injection = 200/6 = 33.333 gph = 0.55... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `grain-drying-energy` | Grain Drying Energy and Fuel | grain-handling practice; spec-v418 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `grain-shrink-moisture` | Grain Drying Shrink and Net Bushels | USDA / land-grant extension; spec-v338 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `leaching-fraction-runoff-ec` | Leaching Fraction and Runoff Conductivity | Project (first-principles); a consistent pour-through and the crop range govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `livestock-dry-matter-intake` | Livestock Dry-Matter Intake and As-Fed Ration | NRC Nutrient Requirements; spec-v339 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `log-limb-weight` | Green Log and Limb Weight | USDA FPL Technical Note 218 green wei...; 16 in butt / 16 in top, 8 ft red oak (Q. borealis, 63 lb/... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `mad-irrigation-trigger` | Available Water and MAD Irrigation Trigger | FAO-56 / NRCS soil-water reservoir (MAD); TAW = (0.30-0.12)*24 = 4.32; RAW = 0.5*4.32 = 2.16; inter... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `manure-application-rate` | Nutrient-Based Manure Application Rate | USDA NRCS Code 590; spec-v340 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `manure-nutrient-application` | Manure Nutrient Application Rate | USDA NRCS Code 590; spec-v419 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `mulch-topsoil-volume` | Mulch, Topsoil, and Aggregate Volume | landscape material take-off; spec-v417 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `photoperiod-blackout-schedule` | Photoperiod Blackout and Night Interruption | Project (first-principles); the crop's critical photoperiod governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `plant-spacing-count` | Plant Spacing Count (Square and Triangular) | Nursery / landscape estimating refere...; spec-v210 section 2.1 pinned example (1 ft on center) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `ppfd-daily-light-integral` | PPFD to Daily Light Integral | Project (first-principles); the crop's published DLI requirement governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `shade-cloth-transmission` | Shade Cloth Transmission and Solar Heat | Project (first-principles); cloth manufacturer data and the crop DLI govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `sod-takeoff` | Sod Takeoff (Slabs and Pallets) | Turfgrass producer / landscape estima...; spec-v211 section 2.1 pinned example (residential lawn) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `sprayer-calibration` | Sprayer 1/128-Acre Calibration | USDA Cooperative Extension Service; course = 43560/128 / nozzle spacing (the width ONE nozzle... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `sprinkler-gpm-for-precip` | Sprinkler Zone Flow for a Target Precip Rate | Irrigation Association / Rain Bird / ...; spec-v736 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `sprinkler-precip-rate` | Sprinkler Precipitation Rate | Irrigation Association / Rain Bird / ...; spec-v207 section 2.1 pinned example (rotor lawn zone) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `tractor-ballast` | Tractor Ballast for a Target Weight-to-Power Ratio | tractor ballasting rule (ASABE); target = 125*180 = 22500; change = 22500 - 18000 = 4500 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `chlorine-demand` | Chlorine Demand and Dose for a Target Residual | Standard Methods 4500-Cl / AWWA M14 (...; spec-v116 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `clarifier-area-for-loading` | Clarifier Surface Area for a Target SOR | Ten States Standards / Metcalf & Eddy...; spec-v742 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `clarifier-surface-loading` | Clarifier Surface, Weir, and Solids Loading | Ten States Standards / Metcalf & Eddy; spec-v405 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+4 more) |
...
| `fluoride-feed-dose` | Fluoride Feed Dose (Available Fluoride Ion) | Water-fluoridation feed (available-fl...; pure = 0.6 x 2 x 8.34 = 10.008; feed = 10.008 / (0.792 x ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `pool-salt-dose` | Pool Salt Dose | Mass-balance identity (NSPF CPO / ANS...; spec-v93 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `tds-from-conductivity` | Total Dissolved Solids from Conductivity | Standard Methods 2510; spec-v407 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `two-source-blend` | Flow-Weighted Two-Source Water Blend | Flow-weighted two-source water blend; blend = (500x4 + 300x12)/800 = 5600/800 = 7.0; low-source... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `uv-dose` | UV Dose and Target Check | USEPA UV Disinfection Guidance Manual...; spec-v116 section 2.2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `well-max-yield` | Well Sustainable Yield from Specific Capacity | AWWA / USGS (inverse); 1.0 GPM/ft, 30 ft allowable drawdown -> 30 GPM (round-tri... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| tile_id | name | citation source | fixture |
| --- | --- | --- | --- |
| `acoustic-gain-pag-nag` | Potential / Needed Acoustic Gain (Feedback Stability) | Potential / needed acoustic gain (PAG...; PAG = 20log8 + 20log30 - 20log2 - 20log12 - 10log1 - 6 = ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `amp-power-spl` | Amplifier Power to SPL | first-principles loudspeaker SPL (ANS...; 90 dB sensitivity, 100 W, 1 m -> 110 dB | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `cardioid-sub-array` | End-Fire and Cardioid Subwoofer Spacing | Project (first-principles); c / (4 x spacing) | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `gobo-image-size` | Gobo Projected Image Size and Keystone | Project (first-principles); 2 x throw x tan(field/2); long axis throw sin(phi) [1/cos... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `lighting-beam` | Stage Lighting Beam and Throw | first-principles theatrical photometr...; 20 deg beam, 30 ft throw, 100000 cd -> 10.58 ft pool, 111... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `lighting-throw-for-pool` | Throw Distance for a Target Beam Pool | first-principles theatrical photometr...; 10.58 ft pool from a 20 deg beam -> 30 ft throw (round-tr... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `mired-gel-shift` | Color-Temperature Correction in Mireds | Project (first-principles); mired = 1,000,000 / kelvin | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `neutral-imbalance` | Three-Phase Neutral Imbalance and Distro | Project (first-principles); Unbalanced 100 / 85 / 70 A: I_N = sqrt(100^2+85^2+70^2 - ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `projector-brightness` | Projector Brightness and Throw | Standard AV screen-luminance identity...; spec-v92 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `projector-max-screen-size` | Max Screen Size for a Projector | Standard AV screen-luminance identity...; spec-v727 section 2 pinned example: 5,000 lm, gain 1.0, 1... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `rf-antenna-cable-loss` | RF Antenna Cable Loss and Amplifier Budget | Project (first-principles); loss = length/100 x loss per 100 ft | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `room-absorption-target` | Absorption Needed for a Target RT60 | W.C. Sabine reverberation equation (p...; spec-v664 section 2 pinned example: 5,000 ft^3 targeting ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `room-acoustics` | Room Acoustics: RT60 and Axial Modes | W.C. Sabine reverberation equation (p...; spec-v120 section 2 pinned example: 5,000 ft^3 / 500 sabi... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `spl-distance` | SPL and Inverse Square Law | Project (first-principles); Free-field SPL attenuation; 110 dB at 1 m -> ~80.5 dB at ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `spl-distance-for-level` | Distance for a Target SPL | Project (inverse-square law, inverse); 110 dB at 1 ft free-field falls to 84 dB at about 20 ft (... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `truss-capacity` | Truss Point Load and Span Capacity | Tomcat USA; 16 in box / 40 ft span / 200 + 400 + 200 lb point loads -... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `winch-fleet-angle` | Winch Drum Fleet Angle | Winch drum fleet angle (Wire Rope Use...; 6 in offset over a 240 in lead -> atan(0.025) = 1.43 deg,... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `wireless-intermod` | Two-Transmitter Intermodulation Screen | Project (first-principles); 2f1-f2, 2f2-f1, 3f1-2f2, 3f2-2f1 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |

### Group O Kitchen (49 tiles)

| tile_id | name | citation source | fixture |
| --- | --- | --- | --- |
| `abv-from-gravity` | Alcohol by Volume from Gravity | Alcohol by volume from gravity (stand...; ABV = (1.055 - 1.012) x 131.25 = 0.043 x 131.25 = 5.64%; ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `as-purchased-quantity` | As-Purchased Quantity from Edible Portion | As-purchased quantity from edible por...; AP = EP / yield = 20 / 0.75 = 26.67 lb (always divide; th... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `beer-color-srm` | Beer Color SRM from the Malt Bill | Project (first-principles); a spectrophotometer reading governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `brewhouse-efficiency` | Brewhouse Efficiency and Grain Bill | Project (first-principles); the malt analysis and measured gravities govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `carbonation-volumes-pressure` | Carbonation Volumes and Head Pressure | Project (first-principles); a dissolved-CO2 measurement governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `dough-water-temperature` | Desired Dough Temperature (Mixing Water) | Desired dough temperature (DDT) mixin...; water = 75 x 3 - (68 + 72 + 24) = 225 - 164 = 61 F | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `draft-beer-line-balance` | Draft Beer Line Balancing | Draft-beer line balancing (Brewers As...; 12 psi, 4 ft rise, 3/16 in vinyl (3.0 psi/ft), no tower -... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `dry-hop-beer-loss` | Dry Hop Absorption and Beer Loss | Project (first-principles); the brewery's measured absorption governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `fermenter-glycol-load` | Fermenter Glycol Cooling Load | Project (first-principles); the chiller manufacturer's capacity governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `food-cost-percentage` | Period Food-Cost Percentage | Standard restaurant-accounting identi...; spec-v90 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `ibu-tinseth` | Hop Bitterness IBU (Tinseth) | Project (first-principles); measured IBU on the brewery's own system governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `kettle-boil-off` | Kettle Boil-Off Rate and Post-Boil Gravity | Project (first-principles); the kettle's measured boil-off rate governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `mash-strike-water` | Mash Strike Water Temperature and Thickness | Project (first-principles); the brewery's own measured mash temperatures govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `mash-tun-grain-bed` | Mash Tun Capacity and Grain Bed Depth | Project (first-principles); the lauter tun manufacturer's rated depth governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `overrun-percent` | Ice Cream Overrun | Goff & Hartel, Ice Cream, 7th ed.; FD...; Mix 9.0 lb/gal frozen to 4.5 lb/gal -> 100% overrun, 50% ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `packaging-yield-loss` | Brewery Packaging Yield and Loss | Project (first-principles); the brewery's packaging records govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `prime-cost` | Restaurant Prime Cost | Standard restaurant P&L prime-cost de...; spec-v90 section 2 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `proof-gallon-yield` | Distillation Charge, Cut, and Proof Gallon Yield | Project (first-principles); the TTB gauging tables govern | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `sous-vide-pasteurization` | Sous-Vide Pasteurization Time | FDA Food Code 3-401.11(B)(2) / one-te...; one-term slab centerline, Fo = 2.245 for a 99.5% approach... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
| `sparge-water-volume` | Sparge Water and Pre-Boil Volume | Project (first-principles); the brewery's own measured runoff governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `yeast-pitch-rate` | Yeast Pitch Rate and Cell Count | Project (first-principles); a viability count on the day governs | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `azimuth-bearing-conversion` | Azimuth and Quadrant Bearing Conversion | standard surveying references (Ghilan...; An azimuth of 138.5 deg lies between 90 and 180 deg, so i... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `backcountry-needs` | Backcountry Water and Caloric Requirement | Project (first-principles) over a res...; 170 lb / moderate / moderate / 3 days / solo -> 3.5 L/day... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `differential-leveling` | Differential Leveling (HI Method) and Loop Misclosure | Height-of-instrument leveling (Ghilan...; spec-v311 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `level-loop-adjustment` | Level-Loop Misclosure Distribution (Compass Rule) | Compass-rule level-loop adjustment (G...; spec-v631 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `stadia-distance` | Stadia Tacheometry (Distance and Elevation) | Stadia tacheometry (K = 100); spec-v312 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `taping-corrections` | Steel Tape Distance Corrections (Temperature, Slope, Tension, Sag) | Steel-tape corrections (Ghilani/Wolf); spec-v313 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `change-order-markup` | Change Order Price with Overhead and Profit | construction estimating (AIA G701); spec-v391 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `eoq-order-quantity` | Economic Order Quantity (Wilson EOQ) | Wilson economic order quantity (EOQ) ...; spec-v529 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `equipment-hourly-rate` | Equipment Owning and Operating Hourly Rate | CAT / AED method; spec-v363 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `overhead-recovery-rate` | Overhead Recovery Rate | contractor cost accounting; spec-v364 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `reorder-point` | Reorder Point and Safety Stock (Service Level) | reorder point and safety stock (servi...; spec-v530 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `retainage-tracker` | Retainage Withheld and Net Payment (AIA G702/G703) | construction billing (AIA G702/G703); spec-v392 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `surety-bond-premium` | Surety Bond Premium (Tiered Rate) | surety pricing (tiered rate schedule); spec-v444 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
| `units-of-production-depr` | Units-of-Production Depreciation | units-of-production (activity) deprec...; spec-v531 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `wip-percent-complete` | Work-in-Progress Percent Complete and Over/Under Billing | construction accounting (cost-to-cost...; spec-v390 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `ideal-gas-law` | Ideal Gas Law (PV = nRT) | Ideal gas law (general chemistry); V = nRT/P = 1 x 0.0820573 x 298.15 / 1 = 24.465 L; molar ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `molarity-dilution` | Molarity and Dilution (C1V1=C2V2) | Project (first-principles); C1 = 1.0 M / C2 = 0.1 M / V2 = 0.05 L (50 mL) -> V1 = 0.0... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) |
...
| `nernst-equation` | Nernst Equation (Cell Potential vs Concentration) | Nernst equation (physical chemistry, ...; E = 1.10 - (8.314 x 298.15 / (2 x 96485)) x ln(0.01) = 1.... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `osmolarity` | Osmolarity and Osmotic Pressure (van't Hoff) | van't Hoff colligative properties (ph...; Osm = 2 x 0.154 = 0.308 Osmol/L = 308 mOsm/L; Pi = 0.308 ... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `blended-mortgage-rate` | Blended Mortgage Rate (Two Loans) | blended mortgage rate (weighted-avera...; spec-v528 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `break-even-occupancy` | Break-Even Occupancy | CRE underwriting; spec-v345 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `commercial-load-factor` | Rentable/Usable Load Factor (BOMA) | BOMA rentable/usable load factor (ANS...; spec-v527 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `debt-yield` | Debt Yield | CRE lender underwriting; spec-v344 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `fix-flip-profit` | Fix-and-Flip Profit and Return | real-estate-investing practice; spec-v402 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
| `floor-area-ratio` | Floor Area Ratio (Zoning) | Floor area ratio (municipal zoning in...; 30,000 SF building on a 20,000 SF lot -> FAR 1.5; 2.0 cap... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `max-offer-70-rule` | Fix-and-Flip Maximum Offer (70% Rule) | real-estate-investing heuristic; spec-v346 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `net-effective-rent` | Net Effective Rent (Lease Concessions) | net effective rent (straight-line con...; spec-v526 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `required-face-rent` | Required Face Rent from a Target Net Effective Rent | required face rent (inverse of the ne...; spec-v646 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `one-way-anova` | One-Way ANOVA (Compare 3+ Group Means) | scipy.stats.f_oneway (cross-verified); SSB/2 over SSW/12; scipy.stats.f_oneway returns F=32.2359... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `significant-figures` | Significant Figures (count + round) | NIST SP 811 §7 (public); Worked example 0.00347 -> 3 sig figs; rounded to 2 -> 0.0035 | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
...
| `crane-power-line-clearance` | Crane Power Line Clearance (OSHA 1926.1408) | Occupational Safety and Health Admini...; Table A: up to 50 kV, 10 ft. Having determined the line i... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
...
| `guy-wire-tension` | Guy-Wire / Down-Guy Tension and Mast Download | Guy-wire / down-guy tension and mast ...; theta = atan(20/20) = 45 deg; tension = 500/cos45 = 707.1... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `spanline-sag-tension` | Spanned Cable Sag and Tension | Shallow-cable parabola statics (by name); spec-v484 section 2.1 pinned example | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+2 more) |
...
| `wire-rope-clips` | Wire-Rope Clip Count and Spacing (OSHA Table H-2) | wire-rope clip requirements (OSHA Tab...; OSHA Table H-2: 3/4 in -> 4 clips; spacing = 6 x 0.75 = 4... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+3 more) |
| `wire-rope-diameter-for-wll` | Wire-Rope Diameter for a Required WLL | Wire Rope Users Manual rule-of-thumb ...; 5 ton WLL, cf 46, DF 5 -> 0.737 in exact, next standard 3... | [test/fixtures/worked-examples.json](../test/fixtures/worked-examples.json) (+1 more) |
