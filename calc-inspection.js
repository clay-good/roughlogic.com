...
// AWS D1.1 Table 8.1 permits a fillet to underrun its nominal size by 1/16 in
// up to 3/16 in, 3/32 in at 1/4 in, and 1/8 in at 5/16 in and over, on no
// more than 10% of the length. This 3/8 in fillet takes 1/8 in (the example
// carried 1/16 until 2026-09-19); it still fails on length and on the crack.
export const weldVisualAcceptanceExample = { inputs: { nominal_leg_in: 0.375, measured_leg_in: 0.3125, undersize_length_in: 3, weld_length_in: 20, allowed_undersize_in: 0.125, allowed_undersize_fraction: 0.10, measured_undercut_in: 0.030, allowed_undercut_in: 0.03125, crack_present: "yes" } };
...
// (34 thou is more than half a 1/16 in corrosion allowance) and the
...
export function computeRtExposureTime({
  ...
  if (has_geometry && !(base_distance_in > material_thickness_in && (!(new_distance_in > 0) || new_distance_in > material_thickness_in))) return { error: "The source-to-film distance must exceed the material thickness." };
  // ASME V T-274 / SE-1030: Ug = F d / D, with D the source-to-OBJECT
  // distance -- the source-to-film distance less the thickness. Until
  // 2026-09-19 the whole source-to-film distance was used, understating Ug.
  const ug_base_in = has_geometry ? source_size_in * material_thickness_in / (base_distance_in - material_thickness_in) : 0;
  const ug_new_in = has_geometry && has_new ? source_size_in * material_thickness_in / (new_distance_in - material_thickness_in) : 0;
...
INSPECTION_RENDERERS["mt-yoke-coil-amperage"] = _simpleRenderer({
  citation: "Citation: circular field current = part diameter × amperes per inch (commonly 300 to 800 A/in, entered); coil shot amp-turns = 35,000 / (L/D + 2) at high fill factor or 45,000 / (L/D) at low with the part against the coil wall, with L/D bounded between 2 and 15 as the standard formulae require; and the yoke lift check of 10 lb on AC or 40 lb on DC at maximum pole spacing. A circular field finds LONGITUDINAL discontinuities and a longitudinal field finds TRANSVERSE ones, so one shot examines one orientation. Field adequacy is verified with a gauss meter or quantitative quality indicator, not by the amperage. It does not write a technique, select particles or lighting, evaluate indications, or set acceptance criteria. ASTM E1444 / E709 and the certified examiner govern.",
...
export function computePwhtHoldingTime({
  ...
  const _cycleFor = (t) => {
    ...
    const coolRate = Math.min(cooling_rate_constant_f_hr_in / t, cooling_rate_constant_f_hr_in);
