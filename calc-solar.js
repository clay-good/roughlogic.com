...
export function computePvEnergyYield({ dc_kw = 0, psh = 5.0, perf_ratio = 0.77 } = {}) {
  ...
  return {
    ...
    note: "NREL PVWatts energy model: annual energy = DC nameplate x peak-sun-hours x 365 x performance ratio. The peak-sun-hours is the plane-of-array daily irradiation from NREL NSRDB / PVWatts for the site, tilt, and azimuth (not a fixed 5). The performance ratio (default 0.77, a typical whole-system ratio that includes temperature loss; it equals the retired PVWatts V1 derate -- current PVWatts takes 14% system losses and a 96% inverter, 0.826, and models temperature separately) rolls up soiling, shading, mismatch, wiring, inverter, and availability losses and is the single biggest lever; the first-year figure degrades roughly half a percent a year. A pre-design estimate, not a bankable production model.",
  };
}
function renderPvEnergyYield(inputRegion, outputRegion, citationEl) {
  citationEl.textContent = "Citation: NREL PVWatts energy model E = Pdc x PSH x 365 x PR, with specific yield = E / Pdc and capacity factor = E / (Pdc x 8760) (by name). The peak-sun-hours is the plane-of-array daily irradiation from NREL NSRDB / PVWatts for the site, tilt, and azimuth; the performance ratio (default 0.77, a typical whole-system ratio that includes temperature loss; it equals the retired PVWatts V1 derate -- current PVWatts takes 14% system losses and a 96% inverter, 0.826, and models temperature separately) is the single biggest lever. A pre-design estimate, not a bankable production model.";
