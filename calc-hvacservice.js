...
function _condensateDrainSize(tons) {
  ...
  if (tons <= 250) return 2.0;
  return null; // IMC Table 307.2.2 stops at 250 tons
}
...
export function computeCondensateDrain({ tons = 0, pints_per_ton_hr = 3, run_ft = 0, slope_in_per_ft = 0.125 } = {}) {
  ...
  if (min_size_in === null) return { error: "IMC Table 307.2.2 stops at 250 tons (2 in); size a larger combined drain by engineering design." };
  const fall_in = run_ft * slope_in_per_ft;
  return {
    rate_pints_hr, rate_gph, min_size_in, fall_in,
    note: "Condensate production tracks the LATENT load and indoor humidity - the per-ton rate here is a field estimate (about 2 to 4 pints per ton-hour is common in humid cooling), not a code value. The drain-size steps are IMC Table 307.2.2 by equipment capacity, which applies where the drains of more than one unit are manifolded together (a single unit's drain is not less than 3/4 in and does not reduce); the line slopes not less than 1/8 in per foot toward the discharge (IMC 307.2.1). A draw-through coil needs a proper trap to break the negative pressure. The AHJ and the equipment manual govern.",
  };
}
...
HVACSERVICE_RENDERERS["condensate-drain"] = _simpleRenderer({
  citation: "Citation: IMC 2021 Table 307.2.2 condensate drain size by capacity (manifolded drains, up to 250 tons; 3/4 in minimum for any drain) and the 307.2.1 not-less-than-1/8-in-per-foot slope (by name); 8 pints per gallon. The per-ton condensate rate is an editable field estimate, not a code value.",
...
HVACSERVICE_RENDERERS["blower-door-ach50"] = _simpleRenderer({
  citation: "Citation: ACH50 = CFM50 x 60 / conditioned volume (the blower-door normalization) and the LBL natural-infiltration divide-by-N rule (by name); IECC 2021 R402.4.1.3 sets the prescriptive air-leakage limit in ACH50 (<= 3 in CZ 3-8, <= 5 in CZ 0-2) and R402.4.1.2 caps every path at 5.0; ASTM E779 / E1827 are the test methods. The N-factor varies with climate zone, building height, and wind shielding -- roughly 10 in a cold, windy, exposed, taller situation to over 30 in a mild, sheltered, single-storey one -- so entering a low and high N shows the threefold spread a single figure hides. Natural infiltration on this model is a SEASONAL AVERAGE, not an hourly or design-condition rate; the seasonal energy uses the standard 1.08 x cfm x 24 x heating degree days. A field normalization, not a rater's signed report.",
...
HVACSERVICE_RENDERERS["duct-leakage-cfm25"] = _simpleRenderer({
  citation: "Citation: Residential duct leakage (IECC R403.3.5): normalized = leakage CFM25 / conditioned floor area x 100, compared to the IECC 2021 R403.3.6 limit (4 CFM25 per 100 ft^2 total / post-construction, 3 for a rough-in without the air handler, 8 with all ducts inside the thermal envelope). A field aid; the adopted energy code, the required test type, and the rater govern.",
