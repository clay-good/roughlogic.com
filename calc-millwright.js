...
export function computeShaftAlignmentReverseDial({ tir_a_in = 0, tir_b_in = 0, plane_spacing_in = 0, front_foot_distance_in = 0, rear_foot_distance_in = 0, coupling_center_distance_in = 0 } = {}) {
  ...
  // Movable-shaft centerline position (+ = high) at each plane. The plane-A indicator rides on the MOVABLE shaft and
  // reads the stationary hub, the plane-B indicator rides on the stationary shaft and reads the movable hub, so the
  // two readings describe the movable shaft with OPPOSITE signs: y_A = +TIR_A / 2, y_B = -TIR_B / 2. The foot move is
  // the negative of the extrapolated position (a low foot is raised). Rexnord 538-214: -.020 and +.010 over 10.5 in
  // -> add .004 at the front feet (13 in) and .001 at the rear (18.25 in). Until 2026-09-26 both halves carried the
  // same sign and the move equalled the position, reversing the shim direction.
...
MILLWRIGHT_RENDERERS["shaft-alignment-reverse-dial"] = _simpleRenderer({
  citation: "Citation: the reverse-dial alignment relations by name -- movable-shaft position +TIR_A / 2 at plane A and -TIR_B / 2 at plane B (each indicator reads the other shaft), slope = (y_B - y_A) / the plane spacing, and the move at a foot = -(y_A + slope x the distance from plane A to that foot), measured in the same sense as the plane spacing. Sign convention stated explicitly: positive raises the foot and adds shim. Bracket sag must be measured and subtracted before the readings are used. The machine manufacturer's alignment specification and the plant's precision maintenance procedure govern.",
  example: shaftAlignmentReverseDialExample.inputs,
  fields: [
    { key: "tir_a_in", label: "TIR at plane A: indicator on the MOVABLE shaft reading the stationary hub (in, signed)", kind: "number", default: -0.014 },
    { key: "tir_b_in", label: "TIR at plane B: indicator on the STATIONARY shaft reading the movable hub (in, signed)", kind: "number", default: 0.022 },
...
export function computeAlignmentThermalGrowth({ stationary_support_height_in = 0, stationary_alpha_per_f = 0.0000065, stationary_operating_temp_f = 70, movable_support_height_in = 0, movable_alpha_per_f = 0.0000065, movable_operating_temp_f = 70, ambient_temp_f = 70, tolerance_offset_mils = 2 } = {}) {
  ...
  if ([arguments[0]?.stationary_alpha_per_f, arguments[0]?.movable_alpha_per_f].some((v) => Number(v) > 1e-4)) return { error: "Enter the expansion coefficient per degree F as a small number (0.0000063), not in micro-inches (6.3)." };
...
MILLWRIGHT_RENDERERS["bearing-defect-frequencies"] = _simpleRenderer({
  ...
  outputs: [
    ...
    { key: "b", id: "bdf-out-b", label: "Ball spin (BSF; a ball defect also shows at 2 x BSF)", value: (r) => fmt(r.bsf_hz, 2) + " Hz at " + fmt(r.bsf_order, 2) + "x (2x: " + fmt(2 * r.bsf_order, 2) + "x)" },
...
export function computeAirCompressorCfmSizing({ tool1_qty = 0, tool1_cfm = 0, tool1_duty = 0, tool2_qty = 0, tool2_cfm = 0, tool2_duty = 0, tool3_qty = 0, tool3_cfm = 0, tool3_duty = 0, tool4_qty = 0, tool4_cfm = 0, tool4_duty = 0, leak_allowance_pct = 15, growth_allowance_pct = 20, cfm_per_hp = 4 } = {}) {
  const _g = _finiteGuard(arguments[0]); if (_g) return _g;
  // Unit / range guard added 2026-09-26 after printed-example probing.
  if ([arguments[0]?.leak_allowance_pct, arguments[0]?.growth_allowance_pct].some((v) => Number(v) > 0 && Number(v) < 1)) return { error: "Enter the leak and growth allowances as percents (15 for 15%), not fractions." };
...
export function computeVacuumEvacuationTime({ chamber_volume_ft3 = 0, pump_speed_cfm = 0, start_pressure_torr = 760, target_pressure_torr = 0, leak_rate_torr_cfm = 0, conductance_efficiency = 1 } = {}) {
  ...
  // With a leak the time is (V/S) ln((p0 - pu)/(pt - pu)) (Leybold); an unreachable target has no time. Until
  // 2026-09-26 the leak was ignored in the time, which read 5.36 min for a target the verdict called unreachable.
  const evacuation_minutes = reaches_target
    ? (chamber_volume_ft3 / effective_speed_cfm) * Math.log((start_pressure_torr - ultimate_pressure_torr) / (target_pressure_torr - ultimate_pressure_torr))
    : null;
  const outs = [minutes_per_decade, decades, effective_speed_cfm].concat(evacuation_minutes === null ? [] : [evacuation_minutes]);
...
MILLWRIGHT_RENDERERS["vacuum-evacuation-time"] = _simpleRenderer({
  citation: "Citation: the isothermal volume pump-down relation by name -- t = (V / S) ln(p1 / p2), or (V / S) ln((p1 - pu) / (p2 - pu)) with a leak-limited ultimate pu, so each leak-free decade costs t = 2.303 V / S and the time is spread evenly across the decades -- with the leak-limited ultimate pressure = leak rate / effective pumping speed. Rated speed is corrected by an entered conductance efficiency because the connecting line never delivers it in full. Real pumps lose speed near the ultimate, so this runs optimistic at the low end. The pump manufacturer's speed curve and the system designer govern.",
  ...
  outputs: [
    { key: "t", id: "vet-out-t", label: "Evacuation time", value: (r) => r.evacuation_minutes === null ? "Not reached: the leak holds the chamber at " + fmt(r.ultimate_pressure_torr, 4) + " torr, above the target" : fmt(r.evacuation_minutes, 2) + " minutes from " + fmt(r.start_pressure_torr, 0) + " to " + fmt(r.target_pressure_torr, 3) + " torr" },
